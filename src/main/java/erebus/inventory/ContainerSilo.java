...
public class ContainerSilo extends Container {
    ...
    public ContainerSilo(InventoryPlayer playerInventory, TileEntitySiloTank tile) {
        ...
        for (j = 0; j < numRows; ++j)
            for (k = 0; k < 13; ++k) addSlotToContainer(new Slot(tile, k + j * 13, 12 + k * 18, 17 + j * 18));

        for (j = 0; j < 3; ++j) for (k = 0; k < 9; ++k)
            addSlotToContainer(new Slot(playerInventory, k + j * 9 + 9, 48 + k * 18, 102 + j * 18 + i));

        for (j = 0; j < 9; ++j) addSlotToContainer(new Slot(playerInventory, j, 48 + j * 18, 160 + i));
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int par2) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (par2 < numRows * 13) {
                if (!mergeItemStack(is1, numRows * 13, inventorySlots.size(), true)) return null;
            } else if (!mergeItemStack(is1, 0, numRows * 13, false)) return null;
