...
public class ContainerComposter extends Container {
    ...
    public ContainerComposter(InventoryPlayer player, TileEntityComposter composter) {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 9; j++) addSlotToContainer(new Slot(player, j + i * 9 + 9, 8 + j * 18, 84 + i * 18));

        for (int i = 0; i < 9; i++) addSlotToContainer(new Slot(player, i, 8 + i * 18, 142));
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex == 2) {
                ...
                } else if (slotIndex >= 30 && slotIndex < 39 && !mergeItemStack(itemstack1, 3, 30, false)) return null;
