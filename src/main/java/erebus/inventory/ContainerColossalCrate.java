...
public class ContainerColossalCrate extends Container {
    ...
    public void addSlots() {
        ...
        for (j = slotNo; j < slotNo + 72; ++j) {
            ...
            addSlotToContainer(
                new Slot(crateList.get(crateNo), getSlotIDFromSlotNo(j), getSlotXFromSlotNo(j), getSlotYFromSlotNo(j)));
        }

        for (j = 0; j < 3; ++j) for (k = 0; k < 9; ++k)
            addSlotToContainer(new Slot(playerInventory, k + j * 9 + 9, 35 + k * 18, 138 + j * 18));

        for (j = 0; j < 9; ++j) addSlotToContainer(new Slot(playerInventory, j, 35 + j * 18, 196));
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int par2) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (par2 < 72) {
                if (!mergeItemStack(is1, 72, inventorySlots.size(), true)) return null;
            } else if (!mergeItemStack(is1, 0, 72, false)) return null;
