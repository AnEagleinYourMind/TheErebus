...
public class ContainerSmoothieMaker extends Container {
    ...
    public ItemStack transferStackInSlot(EntityPlayer player, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            stack = stack1.copy();
            if (slotIndex > 4) {
                if (stack1.getItem() == ModItems.materials
                    && stack1.getItemDamage() != ItemMaterials.DATA.SMOOTHIE_GLASS.ordinal()
                    || stack1.getItem() != ModItems.materials) {
                    if (!mergeItemStack(stack1, 0, 4, false)) return null;
                } else if (stack1.getItem() == ModItems.materials
                    && stack1.getItemDamage() == ItemMaterials.DATA.SMOOTHIE_GLASS.ordinal())
                    if (!mergeItemStack(stack1, 4, 5, false)) return null;
            } else if (!mergeItemStack(stack1, 5, inventorySlots.size(), false)) return null;
            if (stack1.stackSize == 0) slot.putStack(null);
            else slot.onSlotChanged();
            if (stack1.stackSize != stack.stackSize) slot.onPickupFromSlot(player, stack1);
            else return null;
        }
        return stack;
    }
    ...
    public void detectAndSendChanges() {
        ...
        if (tile != null) {
            ...
            for (Object crafter : crafters) if (crafter instanceof EntityPlayerMP)
    ...
    public void readPacketData(NBTTagCompound nbt) {
        if (tile != null && nbt != null) tile.readGUIData(nbt);
    }
