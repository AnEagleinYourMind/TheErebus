...
public class ItemMaterials extends Item {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (side == 1 && stack.getItemDamage() == DATA.BAMBOO_SHOOT.ordinal()
            && player.canPlayerEdit(x, y, z, side, stack)
            && player.canPlayerEdit(x, y + 1, z, side, stack)) {
            Block soil = world.getBlock(x, y, z);

            if (soil != null
                && soil.canSustainPlant(world, x, y, z, ForgeDirection.UP, (BlockBambooShoot) ModBlocks.bambooShoot)
        ...
        if (side == 0 && stack.getItemDamage() == DATA.DARK_FRUIT_SEEDS.ordinal()
            && player.canPlayerEdit(x, y, z, side, stack)
            && player.canPlayerEdit(x, y - 1, z, side, stack)) {
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (!world.isRemote) {
            ...
            if (damage == DATA.BIO_VELOCITY.ordinal() || damage == DATA.SUPERNATURAL_VELOCITY.ordinal()) {
                ...
                if (currentSpeed == null || damage == DATA.BIO_VELOCITY.ordinal() && currentSpeed.getAmplifier() < 1
                    || damage == DATA.SUPERNATURAL_VELOCITY.ordinal() && currentSpeed.getAmplifier() < 3) {
                    player.addPotionEffect(
                        new PotionEffect(
                            Potion.moveSpeed.id,
                            damage == DATA.BIO_VELOCITY.ordinal() ? 280 : 210,
                            damage == DATA.BIO_VELOCITY.ordinal() ? 1 : 3,
                            true));
                    PacketPipeline.sendToAll(
                        new PacketSound(
                            PacketSound.SOUND_VELOCITY_USE,
                            player.posX,
                            player.posY,
                            player.posZ,
                            1.2F,
            ...
            if (damage == DATA.CAMO_POWDER.ordinal()) {
                ...
                if (currentVisibility == null
                    || damage == DATA.CAMO_POWDER.ordinal() && currentVisibility.getAmplifier() < 1) {
                    player.addPotionEffect(new PotionEffect(Potion.invisibility.id, 280, 1, true));
                    PacketPipeline.sendToAll(
                        new PacketSound(PacketSound.SOUND_CAMO_USE, player.posX, player.posY, player.posZ, 1.2F, 1F));
                    if (!player.capabilities.isCreativeMode) stack.stackSize--;
                } else return stack;
            } else return stack;
        }
    ...
    public void registerIcons(IIconRegister reg) {
        ...
        for (DATA data : DATA.values()) icons[data.ordinal()] = reg.registerIcon(
            "erebus:" + data.name()
                .toLowerCase());
    }
    ...
    public void getSubItems(Item stack, CreativeTabs tab, List list) {
        for (DATA data : DATA.values()) list.add(new ItemStack(stack, 1, data.ordinal()));
    }
