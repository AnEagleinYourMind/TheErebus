...
public class ItemBambucket extends Item {
            ...
            if (pos.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                if (flag) {
                    ...
                    if (tryPlaceContainedLiquid(world, x, y, z) && !player.capabilities.isCreativeMode) {
                        stack.stackSize--;
                        if (stack.stackSize <= 0) return new ItemStack(ModItems.bambucket);
                        else player.inventory.addItemStackToInventory(new ItemStack(ModItems.bambucket));
                        return stack;
                    }
                }
            }
    ...
    private ItemStack addBucketToPlayer(ItemStack stack, EntityPlayer player, Item item) {
        if (player.capabilities.isCreativeMode) return stack;
        else if (--stack.stackSize <= 0) return new ItemStack(item);
        else {
    ...
    private boolean tryPlaceContainedLiquid(World world, int x, int y, int z) {
        if (fluid == Blocks.air) return false;
        else {
            ...
            if (!world.isAirBlock(x, y, z) && !flag) return false;
                ...
                if (world.provider.isHellWorld && fluid == Blocks.flowing_water) {
                    ...
                    for (int i = 0; i < 8; i++) world.spawnParticle(
                        "largesmoke",
                        x + Math.random(),
                        y + Math.random(),
                        z + Math.random(),
