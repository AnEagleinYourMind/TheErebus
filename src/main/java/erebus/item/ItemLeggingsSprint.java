...
public class ItemLeggingsSprint extends ItemArmor {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean showAdvancedInfo) {
        list.add(
            EnumChatFormatting.GRAY + StatCollector.translateToLocalFormatted(
                "tooltip.erebus.sprintleggingstier",
                1 + (!stack.hasTagCompound() ? 0 : stack.stackTagCompound.getByte("upgradeTier"))));
    }

    @Override
    public String getArmorTexture(ItemStack is, Entity entity, int slot, String type) {
        if (is.getItem() == ModItems.sprintLeggings) {
            if (armtick >= 0 && armtick <= 20) texture = "erebus:textures/models/armor/centipede0.png";
            if (armtick > 20 && armtick <= 40) texture = "erebus:textures/models/armor/centipede1.png";
            if (armtick > 40 && armtick <= 60) texture = "erebus:textures/models/armor/centipede2.png";
        }
        return texture;
    }
    ...
    public void onArmorTick(World world, EntityPlayer player, ItemStack is) {
        ...
        armtick++;
        if (armtick > 60 || player.isSprinting()) armtick = 0;
    }
