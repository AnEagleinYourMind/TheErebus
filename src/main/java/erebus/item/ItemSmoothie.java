...
public class ItemSmoothie extends ItemFood {
    ...
    public ItemStack onEaten(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (stack.stackSize >= 0) if (!player.inventory.addItemStackToInventory(getContainerItem(stack)))
    ...
    protected void onFoodEaten(ItemStack stack, World world, EntityPlayer player) {
        if (!world.isRemote) {
            ...
            if (effects != null && effects.length > 0)
                for (PotionEffect effect : effects) player.addPotionEffect(new PotionEffect(effect));
    ...
    public String getUnlocalizedName(ItemStack stack) {
        String type = SmoothieType.values()[stack.getItemDamage()].toString()
            .toLowerCase();
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean isComplex) {
        ...
        if (extraEffect != null) list.add(StatCollector.translateToLocal("erebus.smoothie.effect." + extraEffect));
        ...
        if (effects != null && effects.length > 0) {
            ...
            for (PotionEffect effect : effects) {
                String str = StatCollector.translateToLocal(effect.getEffectName())
                    .trim();
                Potion potion = Potion.potionTypes[effect.getPotionID()];
                Map<IAttribute, AttributeModifier> map = potion.func_111186_k();

                if (map != null && map.size() > 0) for (Entry<IAttribute, AttributeModifier> entry : map.entrySet()) {
                    ...
                    attributes.put(
                        ...
                        new AttributeModifier(
                            modifier.getName(),
                            potion.func_111183_a(effect.getAmplifier(), modifier),
                            modifier.getOperation()));
                }

                if (effect.getAmplifier() > 0) str = str + " "
                    + StatCollector.translateToLocal("potion.potency." + effect.getAmplifier())
                        .trim();
                if (effect.getDuration() > 20) str = str + " (" + Potion.getDurationString(effect) + ")";

                if (potion.isBadEffect()) list.add(EnumChatFormatting.RED + str);
                else list.add(EnumChatFormatting.GRAY + str);
            }
            ...
            if (!attributes.isEmpty()) {
                ...
                for (Entry<String, AttributeModifier> entry : attributes.entries()) {
                    ...
                    if (modifier.getOperation() != 1 && modifier.getOperation() != 2) amount1 = modifier.getAmount();
                    else amount1 = modifier.getAmount() * 100.0D;

                    if (amount0 > 0.0D) list.add(
                        EnumChatFormatting.BLUE + StatCollector.translateToLocalFormatted(
                            "attribute.modifier.plus." + modifier.getOperation(),
                            new Object[] { ItemStack.field_111284_a.format(amount1),
                        ...
                        list.add(
                            EnumChatFormatting.RED + StatCollector.translateToLocalFormatted(
                                "attribute.modifier.take." + modifier.getOperation(),
                                new Object[] { ItemStack.field_111284_a.format(amount1),
                                    StatCollector.translateToLocal("attribute.name." + entry.getKey()) }));
                    }
                }
    ...
    public void getSubItems(Item item, CreativeTabs tab, List list) {
        for (int i = 0; i < SmoothieType.values().length; i++) list.add(new ItemStack(item, 1, i));
    }
    ...
    public void registerIcons(IIconRegister icon) {
        ...
        for (int i = 0; i < icons.length; i++) icons[i] = icon.registerIcon(
            "erebus:smoothie_" + SmoothieType.values()[i].toString()
                .toLowerCase());
    }
    ...
    public static enum SmoothieType {
        ...
        BRYUFS_BREW(2, 0.0F, new PotionEffect(Potion.damageBoost.id, 1000, 2),
            new PotionEffect(Potion.jump.id, 1000, 2), new PotionEffect(Potion.moveSpeed.id, 1000, 2),
            new PotionEffect(Potion.nightVision.id, 1000, 1));
