...
public class ItemBowMaxSpeed extends ItemBow {
    ...
    public static final String[] bowAnimationIcon = new String[] { "_standby", "_pulling_0", "_pulling_1",
        "_pulling_2" };
    ...
    public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
        ...
        if (usingItem != null) if (time >= 4) return getItemIconForUseDuration(3);
        else if (time > 2) return getItemIconForUseDuration(2);
        else if (time > 0) return getItemIconForUseDuration(1);
        return itemIcon;
    }
    ...
    public void onUsingTick(ItemStack stack, EntityPlayer player, int count) {
        ...
        if (ticks > speed) player.stopUsingItem();
    }
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World world, EntityPlayer player, int inUseCount) {
        ...
        if (canShoot || player.inventory.hasItem(Items.arrow)) {
            ...
            if (maxUseDuration < minRelease || power < 0.1D) return;
            ...
            if (power >= 1.0F) entityArrow.setIsCritical(true);

            int powerEnchant = EnchantmentHelper.getEnchantmentLevel(Enchantment.power.effectId, stack);

            if (powerEnchant > 0) entityArrow.setDamage(entityArrow.getDamage() + powerEnchant + 0.5D);

            int punchEnchant = EnchantmentHelper.getEnchantmentLevel(Enchantment.punch.effectId, stack);

            if (punchEnchant > 0) entityArrow.setKnockbackStrength(punchEnchant * 2);
            ...
            world.playSoundAtEntity(
                ...
                "random.bow",
                1.0F,
                soundPitchModifier * (1.0F / (itemRand.nextFloat() * 0.4F + 1.2F) + power * 0.3F));

            if (canShoot) entityArrow.canBePickedUp = 2;
            else {
