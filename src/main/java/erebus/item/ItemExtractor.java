...
public class ItemExtractor extends Item {
    ...
    public int getMaxItemUseDuration(ItemStack stack) {
        float x = hasTag(stack) ? stack.getTagCompound()
            .getFloat("blockHardness") * 30 : 0;
        if (x >= 150) x = 150;
        return (int) x;
    }
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (hasTag(stack) && stack.getTagCompound()
            .getInteger("coolDown") <= 0) getBlockInfo(world, player, stack);
        if (hasTag(stack)) {
            Block block = Block.getBlockById(
                stack.getTagCompound()
                    .getInteger("blockID"));
            if (block != null) player.setItemInUse(stack, getMaxItemUseDuration(stack));
        }
        return stack;
    }
    ...
    public void getBlockInfo(World world, EntityPlayer player, ItemStack stack) {
        if (!world.isRemote && hasTag(stack)) {
            ...
            while (world.isAirBlock(
                ...
                MathHelper.floor_double(targetZ)) && range <= 15) { // range of 16
                range++;
                targetX += vec3.xCoord;
                targetY += vec3.yCoord;
                targetZ += vec3.zCoord;
                if (!world.isAirBlock(
                    ...
                    stack.getTagCompound()
                        .setInteger("targetX", MathHelper.floor_double(targetX));
                    stack.getTagCompound()
                        .setInteger("targetY", MathHelper.floor_double(targetY));
                    stack.getTagCompound()
                        .setInteger("targetZ", MathHelper.floor_double(targetZ));
                    stack.getTagCompound()
                        .setInteger(
                            "blockID",
                    ...
                    Block block = Block.getBlockById(
                        stack.getTagCompound()
                            .getInteger("blockID"));
                    if (block != null) stack.getTagCompound()
                        .setFloat(
                            "blockHardness",
    ...
    public ItemStack onEaten(ItemStack stack, World world, EntityPlayer player) {
        if (!world.isRemote && hasTag(stack)) {
            Block block = Block.getBlockById(
                stack.getTagCompound()
                    .getInteger("blockID"));
            if (block != null && canExtract(block)) extractBlock(stack, world, player);
        }
        resetStats(stack);
    ...
    protected void extractBlock(ItemStack stack, World world, EntityPlayer player) {
        if (!world.isRemote) {
            ...
            int targetX = stack.getTagCompound()
                .getInteger("targetX");
            int targetY = stack.getTagCompound()
                .getInteger("targetY");
            int targetZ = stack.getTagCompound()
                .getInteger("targetZ");
            ...
            entityExtractedBlock.setBlock(
                Block.getBlockById(
                    stack.getTagCompound()
                        .getInteger("blockID")),
                stack.getTagCompound()
                    .getInteger("blockMeta"));
    ...
    private boolean canExtract(Block block) {
        return !(block instanceof BlockContainer) && !(block instanceof BlockFluidBase)
            ...
            && block != Block.getBlockFromName("bedrock")
            && block.getBlockBoundsMaxX() - block.getBlockBoundsMinX() >= 0.7F
    ...
    public void resetStats(ItemStack stack) {
        stack.getTagCompound()
            .setInteger("blockID", 0);
        stack.getTagCompound()
            .setInteger("coolDown", 20);
    }
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int aNumber, boolean aBoolean) {
        if (hasTag(stack) && stack.getTagCompound()
            .getInteger("coolDown") >= 0) stack.getTagCompound()
                .setInteger(
                    "coolDown",
                    stack.getTagCompound()
                        .getInteger("coolDown") - 1);
    }
}
