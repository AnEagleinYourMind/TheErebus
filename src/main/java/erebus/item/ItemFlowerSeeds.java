...
public class ItemFlowerSeeds extends Item {
    ...
    public void getSubItems(Item id, CreativeTabs tab, List list) {
        for (int i = 0; i < SEED_TYPE.values().length; i++) list.add(new ItemStack(id, 1, i));
    }

    @Override
    public String getItemStackDisplayName(ItemStack stack) {
        String colour;
        if (stack.getItemDamage() == SEED_TYPE.RAINBOW.ordinal()) colour = "rainbow";
        else colour = EnumColour.values()[Utils.getFlowerMetadata(stack)].getUnlocalisedName();
