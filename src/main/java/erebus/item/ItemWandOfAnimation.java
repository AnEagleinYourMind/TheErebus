...
public class ItemWandOfAnimation extends Item {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (!player.canPlayerEdit(x, y, z, side, stack)) return false;
            ...
            if (!world.isRemote && block != null && canAnimate(block, world, x, y, z)) {
                ...
                if (block == Blocks.chest) entityAnimatedBlock = new EntityAnimatedChest(world)
                    .setContents(Utils.getTileEntity(world, x, y, z, TileEntityChest.class));
                else if (block == ModBlocks.bambooCrate) entityAnimatedBlock = new EntityAnimatedBambooCrate(world)
                    .setContents(Utils.getTileEntity(world, x, y, z, TileEntityBambooCrate.class));
                else entityAnimatedBlock = new EntityAnimatedBlock(world);
                ...
                entityAnimatedBlock.setOwnerName(
                    player.getUniqueID()
                        .toString());
    ...
    private boolean canAnimate(Block block, World world, int x, int y, int z) {
        return block == Blocks.chest
            || !(block instanceof BlockContainer) && block.getBlockHardness(world, x, y, z) >= 0
