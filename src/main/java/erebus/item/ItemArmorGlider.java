...
public class ItemArmorGlider extends ItemArmor {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean flag) {
        if (canFly()) {
            ...
            list.add(
                StatCollector.translateToLocal("tooltip.erebus.gliderPoweredKey") + ": "
                    + Keyboard.getKeyName(KeyBindingHandler.poweredGlide.getKeyCode()));
        }
        list.add(
            StatCollector.translateToLocal("tooltip.erebus.gliderGlideKey") + ": "
                + Keyboard.getKeyName(KeyBindingHandler.glide.getKeyCode()));
    }
    ...
    public boolean hasColor(ItemStack stack) {
        return !stack.hasTagCompound() ? false
            : !stack.getTagCompound()
                .hasKey("display", Constants.NBT.TAG_COMPOUND) ? false
                    : stack.getTagCompound()
                        .getCompoundTag("display")
                        .hasKey("color", Constants.NBT.TAG_INT);
    }
    ...
    public int getColor(ItemStack stack) {
        ...
        if (nbt == null) return 0xFFFFFF;
            ...
            return displayNBT == null ? 0xFFFFFF
                : displayNBT.hasKey("color", 3) ? displayNBT.getInteger("color") : 0xFFFFFF;
        }
    }
    ...
    public void removeColor(ItemStack stack) {
        ...
        if (nbt != null) {
            ...
            if (displayNBT.hasKey("color")) displayNBT.removeTag("color");
        }
    }
    ...
    public void func_82813_b(ItemStack stack, int colour) {
        ...
        if (!nbt.hasKey("display", 10)) nbt.setTag("display", displayNBT);
    ...
    public ModelBiped getArmorModel(EntityLivingBase player, ItemStack stack, int slot) {
        if (canFly()) {
            ...
            if (stack.hasTagCompound()) {
                model.isGliding = stack.getTagCompound()
                    .getBoolean("isGliding");
                model.isPowered = stack.getTagCompound()
                    .getBoolean("isPowered");
            }
            ...
            if (stack.hasTagCompound()) model.isGliding = stack.getTagCompound()
                .getBoolean("isGliding");
    ...
    public void onUpdate(ItemStack stack, World world, Entity entity, int par4, boolean par5) {
        if (world.isRemote) {
            ...
            if (stack.getTagCompound()
                .getBoolean("isGliding") && (!KeyBindingHandler.glide.getIsKeyPressed() || entity.onGround)) {
                stack.getTagCompound()
                    .setBoolean("isGliding", false);
                PacketPipeline.sendToServer(new PacketGlider(false));
            }

            if (canFly()) if (stack.getTagCompound()
                .getBoolean("isPowered") && (!KeyBindingHandler.poweredGlide.getIsKeyPressed() || entity.onGround)) {
                    stack.getTagCompound()
                        .setBoolean("isPowered", false);
    ...
    public void onArmorTick(World world, EntityPlayer player, ItemStack stack) {
        ...
        if (nbt.getBoolean("isGliding")) if (!player.onGround) {
            player.motionX *= 1.05D;
            player.motionZ *= 1.05D;
            player.motionY *= 0.5D;
        }

        if (nbt.getBoolean("isPowered") && canFly() && hasGemOrIsCreative(player)) if (!player.onGround) {
            player.motionX *= 1.05D;
            player.motionZ *= 1.05D;
            player.motionY += 0.1D;
    ...
    public void onPlayerRenderPre(RenderPlayerEvent.Pre event) {
        ...
        if (chestPlate != null && chestPlate.getItem() instanceof ItemArmorGlider && chestPlate.hasTagCompound())
            if (chestPlate.getTagCompound()
                .getBoolean("isGliding") && !player.onGround
                || chestPlate.getTagCompound()
                    .getBoolean("isPowered") && !player.onGround) {
