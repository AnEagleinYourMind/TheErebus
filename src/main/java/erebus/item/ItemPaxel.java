...
public abstract class ItemPaxel extends ItemTool {
    ...
    public float getDigSpeed(ItemStack stack, Block block, int meta) {
        if (isToolEffective(block, meta)) return efficiencyOnProperMaterial;
        return 1.0F;
    }

    public boolean isToolEffective(Block block, int meta) {
        return block.isToolEffective("pickaxe", meta) || block.isToolEffective("axe", meta)
            || block.isToolEffective("shovel", meta);
    }
}
