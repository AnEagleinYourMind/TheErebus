...
public class ItemFoodStagHeart extends ItemFood {
    ...
    public void addInformation(ItemStack stack, EntityPlayer player, List list, boolean flag) {
        if (type.equals("raw")) list.add(StatCollector.translateToLocalFormatted("tooltip.erebus.heals"));
        else list.add(StatCollector.translateToLocalFormatted("tooltip.erebus.feeds"));
    }

    @Override
    protected void onFoodEaten(ItemStack is, World world, EntityPlayer player) {
        if (type.equals("raw")) player.heal(20);
        else super.onFoodEaten(is, world, player);
    }
}
