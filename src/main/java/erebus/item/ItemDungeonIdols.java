...
public class ItemDungeonIdols extends Item {
    ...
    public void registerIcons(IIconRegister reg) {
        ...
        int i = 0;
        for (IDOL idol : IDOL.values()) icons[i++] = reg.registerIcon(
            "erebus:idol_" + idol.name()
                .toLowerCase());
    }
    ...
    public void getSubItems(Item item, CreativeTabs tab, List list) {
        for (int i = 0; i < IDOL.values().length; i++) list.add(new ItemStack(item, 1, i));
    }
    ...
    @Override
    public boolean onItemUse(ItemStack stacl, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (stacl.getItemDamage() >= 4) {
            if (!world.isRemote) {
                ...
                entityUmberGolem.setHealth(entityUmberGolem.getMaxHealth()); // hack because of stupid attributes
                                                                             // setting
                entityUmberGolem.setPosition(x + 0.5D, y + 1, z + 0.5D);
                world.spawnEntityInWorld(entityUmberGolem);
            }
            if (!player.capabilities.isCreativeMode) stacl.stackSize--;
            return true;
        }
        return false;
    }
