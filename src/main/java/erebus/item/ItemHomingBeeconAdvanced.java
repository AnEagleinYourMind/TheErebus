...
public class ItemHomingBeeconAdvanced extends Item {
        ...
        if (hasTag(stack) && stack.stackTagCompound.hasKey("homeX")) {
            list.add(
                StatCollector.translateToLocalFormatted(
                    "tooltip.erebus.dimension",
                    stack.getTagCompound()
                        .getString("dimName")));
            list.add(
                StatCollector.translateToLocalFormatted(
                    "tooltip.erebus.targetx",
                    stack.getTagCompound()
                        .getInteger("homeX")));
            list.add(
                StatCollector.translateToLocalFormatted(
                    "tooltip.erebus.targety",
                    stack.getTagCompound()
                        .getInteger("homeY")));
            list.add(
                StatCollector.translateToLocalFormatted(
                    "tooltip.erebus.targetz",
                    stack.getTagCompound()
                        .getInteger("homeZ")));
        } else {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (!world.isRemote && hasTag(stack) && player.isSneaking()) {
            ...
            if (!world.isRemote && block != null) {
                stack.getTagCompound()
                    .setInteger("chipDim", player.dimension);
                stack.getTagCompound()
                    .setString("dimName", player.worldObj.provider.getDimensionName());
                stack.getTagCompound()
                    .setInteger("homeX", x);
                stack.getTagCompound()
                    .setInteger("homeY", y);
    ...
    public ItemStack onItemRightClick(ItemStack stack, World world, EntityPlayer player) {
        if (!world.isRemote && !player.isSneaking() && stack.stackTagCompound.hasKey("homeX")) {
            int x = stack.getTagCompound()
                .getInteger("homeX");
            int y = stack.getTagCompound()
                .getInteger("homeY");
            int z = stack.getTagCompound()
                .getInteger("homeZ");
