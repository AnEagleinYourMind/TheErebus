...
public class ItemErebusFood extends ItemFood {
    ...
    protected void onFoodEaten(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (!world.isRemote && effect != null) player.addPotionEffect(effect);
    ...
    public void registerIcons(IIconRegister reg) {
        ...
        int i = 0;
        for (FoodType type : FoodType.values()) icons[i++] = reg.registerIcon(
            "erebus:food_" + type.toString()
                .toLowerCase());
    }
    ...
    public void getSubItems(Item item, CreativeTabs tab, List list) {
        for (int i = 0; i < FoodType.values().length; i++) list.add(new ItemStack(item, 1, i));
    }

    @Override
    public String getUnlocalizedName(ItemStack stack) {
        return super.getUnlocalizedName() + "_"
            + FoodType.values()[stack.getItemDamage()].toString()
                .toLowerCase();
    }
    ...
    public String getPotionEffect(ItemStack stack) {
        return stack != null && stack.getItemDamage() == FoodType.GRASSHOPPER_LEG_RAW.ordinal() ? "+0+1-2+3&4-4+13"
            : super.getPotionEffect(stack);
    }
