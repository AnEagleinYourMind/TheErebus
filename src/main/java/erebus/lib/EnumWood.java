...
public enum EnumWood {
    ...
    public static void initRecipes() {
        for (EnumWood wood : values()) {
            ...
            if (wood.hasSapling) OreDictionary.registerOre("treeSapling", saplings.get(wood));
            if (wood.hasPlanks) {
                Block stair = stairs.get(wood);
                OreDictionary.registerOre("stairWood", stair);
                GameRegistry.addRecipe(
                    new ItemStack(stair, 4),
                    new Object[] { "x  ", "xx ", "xxx", 'x', new ItemStack(ModBlocks.planks, 1, wood.ordinal()) });

                Block slab = slabs.get(wood);
                OreDictionary.registerOre("slabWood", slab);
                GameRegistry.addRecipe(
                    new ItemStack(slab, 6),
                    new Object[] { "xxx", 'x', new ItemStack(ModBlocks.planks, 1, wood.ordinal()) });

                Block fence = fences.get(wood);
                OreDictionary.registerOre("fenceWood", fence);
                GameRegistry.addRecipe(
                    new ItemStack(fence, 2),
                    new Object[] { "sps", "s s", 's', Items.stick, 'p',
                        new ItemStack(ModBlocks.planks, 1, wood.ordinal()) });

                Block fenceGate = fenceGates.get(wood);
                OreDictionary.registerOre("fenceGate", fenceGate);
                GameRegistry.addRecipe(
                    new ItemStack(fenceGate, 1),
                    new Object[] { "sps", "sps", 's', Items.stick, 'p',
                        new ItemStack(ModBlocks.planks, 1, wood.ordinal()) });
