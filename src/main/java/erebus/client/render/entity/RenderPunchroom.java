...
public class RenderPunchroom extends RenderLiving {
    ...
    protected void preRenderCallback(EntityLivingBase entityliving, float partialTickTime) {
        ...
        int i = 1;
        float f1 = (punchroom.prevSquishFactor
            + (punchroom.squishFactor - punchroom.prevSquishFactor) * partialTickTime) / (i * 0.5F + 1.0F);
    ...
    protected ResourceLocation getEntityTexture(Entity entity) {
        if (((EntityLiving) entity).hasCustomNameTag())
            if (((EntityLiving) entity).getCustomNameTag() == "Bryuf") return texture1;
        return texture;
    }
}
