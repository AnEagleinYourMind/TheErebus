...
public class TileEntityGaeanKeystoneRenderer extends TileEntitySpecialRenderer {
    ...
    private static final ResourceLocation texture = new ResourceLocation(
        "erebus:textures/special/items/portalStaff.png");

    @Override
    public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float partialTick) {
        if (!GaeanKeystone.isGemActive(tile.getBlockMetadata())) return;

        final Minecraft mc = Minecraft.getMinecraft();
        mc.getTextureManager()
            .bindTexture(texture);
        double now = 0;
        if (mc.theWorld != null) now = (mc.theWorld.getTotalWorldTime() % Short.MAX_VALUE) + partialTick;
        GL11.glPushMatrix();
