...
public class TextureHomingBeecon extends TextureAtlasSprite {
    ...
    public void updateAnimation() {
        ...
        if (minecraft.theWorld != null && minecraft.thePlayer != null) updateCompass(
            minecraft.theWorld,
            ...
            false,
            false);
        else updateCompass((World) null, 0.0D, 0.0D, 0.0D, true, false);
    }
    ...
    public void updateCompass(World world, double x, double z, double rotation, boolean flag1, boolean flag2) {
        ...
        if (!framesTextureData.isEmpty()) {
            ...
            if (flag2) currentAngle = angle;
            else {
                double angle2;
                for (angle2 = angle - currentAngle; angle2 < -Math.PI; angle2 += Math.PI * 2D);

                while (angle2 >= Math.PI) angle2 -= Math.PI * 2D;

                if (angle2 < -1.0D) angle2 = -1.0D;
    ...
    private void getBeeconHome() {
        ...
        if (minecraft.theWorld != null && minecraft.thePlayer != null) {
            ...
            boolean isCompassItem = stack.getItem() == ModItems.homingBeecon
                || stack.getItem() == ModItems.homingBeeconAdvanced
                || stack.getItem() == ModItems.deathCompass;

            if (isCompassItem && stack.hasTagCompound() && stack.stackTagCompound.hasKey("homeX")) {
                targetX = stack.getTagCompound()
                    .getInteger("homeX");
                targetZ = stack.getTagCompound()
                    .getInteger("homeZ");
            }
        }
    }
