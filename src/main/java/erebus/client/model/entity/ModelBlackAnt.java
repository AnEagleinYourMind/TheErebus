...
public class ModelBlackAnt extends ModelBase {
    ...
    public void render(Entity entity, float limbSwing, float prevLimbSwing, float entityTickTime, float rotationYaw,
        ...
        if (ant.getDataWatcher()
            .getWatchableObjectByte(16) == Byte.valueOf((byte) 3)) {
        ...
        if (ant.getDataWatcher()
            .getWatchableObjectByte(16) == Byte.valueOf((byte) 4)) {
            RightShears.render(unitPixel);
            LeftShears.render(unitPixel);
        }

        if (ant.getDataWatcher()
            .getWatchableObjectByte(16) == Byte.valueOf((byte) 2)) {
            LeftPack.render(unitPixel);
            StrapPack.render(unitPixel);
            RightPack.render(unitPixel);
            GL11.glPushMatrix();
        ...
        if (ant.getDataWatcher()
            .getWatchableObjectByte(16) == Byte.valueOf((byte) 5)) {
            LeftPack.render(unitPixel);
            StrapPack.render(unitPixel);
            RightPack.render(unitPixel);
    ...
    public void setRotationAngles(float limbSwing, float prevLimbSwing, float entityTickTime, float rotationYaw,
        ...
        HatTop.rotateAngleX = HatBrimF.rotateAngleX = HatBrimL.rotateAngleX = HatBrimMain.rotateAngleX = HatBrimR.rotateAngleX = Head1.rotateAngleX
            + 0.5F;
