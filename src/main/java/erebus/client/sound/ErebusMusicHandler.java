...
public class ErebusMusicHandler {
    ...
    public void onClientTick(ClientTickEvent event) {
        ...
        if (event.phase == Phase.START) {
            ...
            if (mc.theWorld != null) {
                if (mc.theWorld.provider.dimensionId == ConfigHandler.INSTANCE.erebusDimensionID
                    && prevMusicTicker == null) {
                    prevMusicTicker = getMcMusicTicker();
                    setMcMusicTicker(new BLMusicTicker(mc));
                } else if (mc.theWorld.provider.dimensionId != ConfigHandler.INSTANCE.erebusDimensionID
                    && prevMusicTicker != null) {
        ...
        public void update() {
            if (currentSound != null) {
                ...
                if (!mc.getSoundHandler()
                    ...
                    currentSound = null;
                    timeUntilMusic = Math
                        .min(MathHelper.getRandomIntegerInRange(RNG, MIN_WAIT, MAX_WAIT), timeUntilMusic);
                }
            }
