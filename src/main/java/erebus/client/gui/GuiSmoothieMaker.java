...
public class GuiSmoothieMaker extends GuiErebus {
    ...
    private static final ResourceLocation TEXTURE = new ResourceLocation(
        "erebus:textures/gui/container/smoothieMaker.png");
    public static final Rectangle[] tankPositions = new Rectangle[] { new Rectangle(8, 6, 9, 73),
        new Rectangle(25, 6, 9, 73), new Rectangle(142, 6, 9, 73), new Rectangle(159, 6, 9, 73) };
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTickTime, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < tankPositions.length; i++) if (tanks[i].getFluidAmount() > 0) {
            Rectangle rect = tankPositions[i];
            drawFluid(
                tanks[i].getFluid(),
                (int) (rect.height * (float) tanks[i].getFluidAmount() / tanks[i].getCapacity()),
                rect.x + guiLeft,
                rect.y + guiTop,
                rect.width,
                rect.height);
