...
public abstract class GuiErebus extends GuiContainer {
    ...
    protected void drawTexturedModalRectFloat(float p_73729_1_, float p_73729_2_, float p_73729_3_, float p_73729_4_,
        float p_73729_5_, float p_73729_6_) {
        float f = 0.00390625F;
        ...
        tessellator.addVertexWithUV(
            ...
            (p_73729_4_ + 0) * f1);
        tessellator
            .addVertexWithUV(p_73729_1_ + 0, p_73729_2_ + 0, zLevel, (p_73729_3_ + 0) * f, (p_73729_4_ + 0) * f1);
        tessellator.draw();
    }
    ...
    protected void drawFluid(FluidStack fluid, int fluidHeight, int x, int y, int width, int height) {
        ...
        if (fluid.getFluid()
            .getDensity(fluid) < 0) y += fluidHeight - height;
        ...
        for (int i = 0; i < fullX; i++) for (int j = 0; j < fullY; j++)
            if (j >= fullLvl) drawCutIcon(icon, x + i * 16, y + j * 16, 16, 16, j == fullLvl ? lastLvl : 0);
        for (int i = 0; i < fullX; i++)
            drawCutIcon(icon, x + i * 16, y + fullY * 16, 16, lastY, fullLvl == fullY ? lastLvl : 0);
        for (int i = 0; i < fullY; i++)
            if (i >= fullLvl) drawCutIcon(icon, x + fullX * 16, y + i * 16, lastX, 16, i == fullLvl ? lastLvl : 0);
