        ...
        if (!mc.thePlayer.capabilities.isCreativeMode) {
            ...
            if (event.type == RenderGameOverlayEvent.ElementType.HOTBAR) {
                mc.getTextureManager()
                    .bindTexture(antiVenomTexture);

                for (int i = 0; i < 10; i++) {
                    int offsetY = mc.thePlayer.isInsideOfMaterial(Material.water) ? -10 : 0;

                    if (mc.thePlayer.getEntityData()
                        .hasKey("antivenomDuration")) {
                        int duration = mc.thePlayer.getEntityData()
                            .getInteger("antivenomDuration") / 9;
                        GL11.glEnable(GL11.GL_BLEND);
                        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
                        GL11.glColor4f(1, 1, 1, 1);
                        if (duration <= 3 && mc.theWorld.getTotalWorldTime() % 60 == 0)
                            offsetY += random.nextInt(3) - 1;
                        if (i * 2 + 1 < duration) drawTexturedModalRect(startX - i * 8, startY + offsetY, 0, 0, 9, 11);
                        if (i * 2 + 1 == duration) drawTexturedModalRect(startX - i * 8, startY + offsetY, 9, 0, 9, 11);
