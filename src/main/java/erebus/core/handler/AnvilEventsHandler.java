...
public class AnvilEventsHandler {
    ...
    public void onAnvilChange(AnvilUpdateEvent event) {
        if (Enchantment.sharpness.canApply(event.left) && event.right != null
            && event.right.getItem() == ModItems.whetstone) {
            ...
            for (Integer enchantID : currentEnchantments.keySet()) {
                ...
                if (!enchantment.canApplyTogether(Enchantment.sharpness)
                    || !Enchantment.sharpness.canApplyTogether(enchantment)) return;
            }

            // Test if whetstone sharpness level is greater than the current one
            if (EnchantmentHelper.getEnchantmentLevel(Enchantment.sharpness.effectId, event.left)
                >= event.right.getItemDamage()) return;
