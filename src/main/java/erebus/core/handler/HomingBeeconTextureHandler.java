...
public class HomingBeeconTextureHandler {
    ...
    public void onItemIconRegister(TextureStitchEvent.Pre evt) {
        if (evt.map.getTextureType() == 1) {
            evt.map.setTextureEntry(
                "erebus:homing_beecon",
                HomingBeeconTextureHandler.beecon = new TextureHomingBeecon("erebus:homing_beecon"));
            evt.map.setTextureEntry(
                "erebus:death_compass",
                HomingBeeconTextureHandler.deathCompasss = new TextureHomingBeecon("erebus:death_compass"));
        }
    }
}
