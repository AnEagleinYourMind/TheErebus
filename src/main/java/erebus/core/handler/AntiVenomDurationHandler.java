    ...
    public void playerAntiVenomTick(PlayerTickEvent event) {
        ...
        if (event.phase == Phase.END) {
            if (event.player.getEntityData()
                ...
                int duration = event.player.getEntityData()
                    .getInteger("antivenomDuration");
                if (duration > 0) {
                    ...
                    for (PotionEffect effect : (Collection<PotionEffect>) event.player.getActivePotionEffects()) {
                        ...
                        for (int count = 0; count < potionIds.length; count++)
                            if (potion.getId() == potionIds[count]) toRemove.add(potion);
                    }
                    for (Potion potion : toRemove) event.player.removePotionEffect(potion.getId());

                    if (world.getTotalWorldTime() % 20 == 0) {
                        duration--;
                        event.player.getEntityData()
                            .setInteger("antivenomDuration", duration);
