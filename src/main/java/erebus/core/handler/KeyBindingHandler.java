...
public class KeyBindingHandler {
    ...
    public static KeyBinding poweredGlide = new KeyBinding(
        "key.erebus.poweredGlide",
        Keyboard.KEY_F,
        Reference.MOD_NAME);
    public static KeyBinding beetleRam = new KeyBinding("key.erebus.beetleRam", Keyboard.KEY_R, Reference.MOD_NAME);
    public static KeyBinding beetleMine = new KeyBinding(
        "key.erebus.beetleMine",
        Keyboard.KEY_LMENU,
        Reference.MOD_NAME);
    ...
    public void onKey(KeyInputEvent e) {
        if (glide.isPressed()) {
            ...
            if (chestPlate != null && chestPlate.getItem() == ModItems.armorGlider
                || chestPlate != null && chestPlate.getItem() == ModItems.armorGliderPowered) {
                if (!chestPlate.hasTagCompound()) chestPlate.stackTagCompound = new NBTTagCompound();

                chestPlate.getTagCompound()
                    .setBoolean("isGliding", true);
