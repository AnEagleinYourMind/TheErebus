...
public class PacketPipeline {
    ...
    private void load() {
        ...
        try {
            ...
            for (ClassInfo clsInfo : path.getTopLevelClasses("erebus.network.client")) {
                ...
                if (!cls.getName()
                    .endsWith("__")) registerPacket(++id, cls);
            }
    ...
    private void readPacket(FMLProxyPacket fmlPacket, Side side) {
        ...
        try {
            AbstractPacket packet = idToPacket.get(buffer.readByte())
            ...
            switch (side) {
                ...
                case SERVER:
                    packet.handle(
                        Side.SERVER,
                        ((NetHandlerPlayServer) fmlPacket.handler()).playerEntity.worldObj,
                        ((NetHandlerPlayServer) fmlPacket.handler()).playerEntity);
                    break;
            }
        } catch (Exception e) {
            e.printStackTrace();
