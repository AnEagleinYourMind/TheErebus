...
public class EntityCrushroom extends EntityMob implements IRangedAttackMob, IBossDisplayData {
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 200D : 200D * ConfigHandler.INSTANCE.mobHealthMultipier);
        ...
        if (!worldObj.isRemote && getAttackTarget() != null) {
            ...
            if (distance > 5.0D && distance <= 12) {
                ...
                if (getSmashCount() >= 1) {
                    ...
                    setStanding((byte) 3);
                    if (getSmashCount() == 0) setStanding((byte) 0);
                }
            }
    ...
    protected void dropFewItems(boolean recentHit, int fortune) {
        ...
        if (fortune > 0) amount += rand.nextInt(fortune + 1);
    ...
    public boolean canAttackClass(Class entity) {
        return EntityCrushroom.class != entity && EntityCrushling.class != entity
            && EntityZombieAnt.class != entity
            && EntityZombieAntSoldier.class != entity
            && EntityPunchroom.class != entity;
    }
    ...
    private void meleeAttackPlayer() {
        if (!worldObj.isRemote && getAttackTarget().boundingBox.maxY >= boundingBox.minY - 1.0D
            ...
            getAttackTarget().attackEntityFrom(
                ...
                (float) (ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 6D
                    : 6D * ConfigHandler.INSTANCE.mobAttackDamageMultiplier));
            getAttackTarget().addVelocity(
                -MathHelper.sin(rotationYaw * 3.141593F / 180.0F) * 0.5D,
                0.2D,
                MathHelper.cos(rotationYaw * 3.141593F / 180.0F) * 0.5D);
        }
    }
