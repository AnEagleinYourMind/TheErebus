...
public class EntityGrasshopper extends EntityCreature {
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        for (amount = 0; amount < chance; ++amount)
            if (isBurning()) entityDropItem(new ItemStack(ModItems.food, 1, 3), 0.0F);
            else entityDropItem(new ItemStack(ModItems.food, 1, 2), 0.0F);
    }
    ...
    @Override
    protected void jump() {
        currentJumpTarget = new ChunkCoordinates(
            (int) posX + rand.nextInt(3) - rand.nextInt(3),
            (int) posY,
            (int) posZ + rand.nextInt(3) - rand.nextInt(3));
