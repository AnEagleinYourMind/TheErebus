...
public class EntityWisp extends EntityMob {
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 5D : 5D * ConfigHandler.INSTANCE.mobHealthMultipier);
    ...
    public void onLivingUpdate() {
        ...
        particleSpawnTick++;
        if (particleSpawnTick <= 50) particleSize = particleSpawnTick / 25;
        ...
        if (getEntityToAttack() != null
            && getEntityToAttack().posY + getEntityToAttack().getEyeHeight() > posY + getEyeHeight() + heightOffset) {
        ...
        if (getEntityToAttack() == null) flyAbout();
    }
    ...
    public void onUpdate() {
        super.onUpdate();
        if (!worldObj.isRemote) findNearEntity();
        if (worldObj.isRemote && isGlowing()) lightUp(
            worldObj,
            MathHelper.floor_double(posX),
            MathHelper.floor_double(posY),
            MathHelper.floor_double(posZ));
        if (worldObj.isRemote && !isGlowing()) switchOff();
        if (!worldObj.isRemote) if (worldObj
            .getBlock(MathHelper.floor_double(posX), MathHelper.floor_double(posY) - 1, MathHelper.floor_double(posZ))
            == Blocks.water) motionY += 0.1D;
    }
    ...
    private void lightUp(World world, int x, int y, int z) {
        ...
        for (int i = -1; i < 2; i++) for (int j = -1; j < 2; j++)
            for (int k = -1; k < 2; k++) if (x + i != lastX || y + j != lastY || z + k != lastZ || isDead) {
    ...
    protected Entity findNearEntity() {
        List<EntityLivingBase> list = worldObj.getEntitiesWithinAABB(
            EntityPlayer.class,
            AxisAlignedBB.getBoundingBox(posX - 0.5D, posY - 0.5D, posZ - 0.5D, posX + 0.5D, posY + 0.5D, posZ + 0.5D)
                .expand(16D, 16D, 16D));
        for (int i = 0; i < list.size(); i++) {
            Entity entity = list.get(i);
            if (entity != null && !getIsNearEntity()) setIsNearEntity(true);
        }
        if (list.isEmpty() && getIsNearEntity()) setIsNearEntity(false);
        return null;
    }
    ...
    public void setDead() {
        super.setDead();
        if (worldObj.isRemote) switchOff();
    }
        ...
        if (super.attackEntityAsMob(entity)) {
            ...
            if (entity instanceof EntityLivingBase) {
                ...
                if (worldObj.difficultySetting.ordinal() > EnumDifficulty.EASY.ordinal() && rand.nextInt(19) == 0)
                    if (worldObj.difficultySetting == EnumDifficulty.NORMAL) duration = 5;
                    else if (worldObj.difficultySetting == EnumDifficulty.HARD) duration = 10;

                if (duration > 0) ((EntityLivingBase) entity)
                    .addPotionEffect(new PotionEffect(Potion.confusion.id, duration * 20, 0));
            }
            return true;
        } else return false;
    }
