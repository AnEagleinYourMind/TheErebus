...
public class EntityCicada extends EntityCreature {
    ...
    public boolean getCanSpawnHere() {
        ...
        for (int p1 = n; p1 < o; p1++) for (int q1 = p; q1 < q; q1++) for (int n2 = n1; n2 < o1; n2++) {
            ...
            if (o2 == null) continue;
            if (o2 == EnumWood.Cypress.getLog()) return true;
        }
        return false;
    }
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        if (recentlyHit) {
            ...
            int amount;
            for (amount = 0; amount < chance; ++amount) entityDropItem(ItemMaterials.DATA.REPELLENT.makeStack(), 0.0F);
        }
    }
    ...
    public void onUpdate() {
        ...
        findEnemyToAttack();
        if (sonics <= 20) sonics++;
        if (sonics > 20) sonics = 0;
        if (sonics > 10) entityToAttack = null;
        ...
        if (!worldObj.isRemote) {
            if (rand.nextInt(100) == 0) if (cicadaFlying) setCicadaFlying(false);

            if (cicadaFlying) flyAbout();
            else land();
        }
    ...
    protected Entity findEnemyToAttack() {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (entity instanceof EntityPlayer && !((EntityPlayer) entity).capabilities.isCreativeMode) {
                if (sonics == 20) {
                    if (worldObj.isRemote) spawnSonicParticles();
                    ...
                    entity.addVelocity(
                        -MathHelper.sin(rotationYaw * 3.141593F / 180.0F) * 2.0D,
                        0D,
                        MathHelper.cos(rotationYaw * 3.141593F / 180.0F) * 2.0D);
    ...
    public void spawnSonicParticles() {
        for (int a = 0; a < 360; a += 6) {
            ...
            Erebus.proxy.spawnCustomParticle(
                "repellent",
                worldObj,
                posX + -MathHelper.sin((float) ang) * 1.0,
                posY + 0.5D,
                posZ + MathHelper.cos((float) ang) * 1.0,
                -MathHelper.sin((float) ang) * 0.3,
                0D,
                MathHelper.cos((float) ang) * 0.3);
        }
