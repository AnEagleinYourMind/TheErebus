...
public class EntityScorpion extends EntityMob {
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(
            ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 3D
                : 3D * ConfigHandler.INSTANCE.mobAttackDamageMultiplier);
    ...
    public void onUpdate() {
        ...
        if (!sting && stingticks > 0F && poisoned) {
            stingticks = stingticks - 0.16F;
            if (stingticks <= 0.0F) setHasBeenStung(false);
        }
    }
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        for (amount = 0; amount < chance; ++amount) {
            ...
            if (pincerChance == 0) entityDropItem(ItemMaterials.DATA.SCORPION_PINCER.makeStack(), 0.0F);
    ...
    public void onCollideWithPlayer(EntityPlayer player) {
        ...
        if (!player.capabilities.isCreativeMode && !worldObj.isRemote && !captured())
            if (getEntitySenses().canSee(player)) player.mountEntity(this);
    }
    ...
    protected void attackEntity(Entity entity, float distance) {
        ...
        if (distance < 1.0F && entity.boundingBox.maxY > boundingBox.minY
            && entity.boundingBox.minY < boundingBox.maxY
            && cooldown == 0) attackEntityAsMob(entity);
    }
}
