...
public class EntityMucusBombPrimed extends Entity {
    ...
    public void onUpdate() {
        ...
        if (fuse-- <= 0) {
            ...
            if (worldObj.isRemote) spawnGooeyTypeParticles();

        } else worldObj.spawnParticle("smoke", posX, posY + 0.5D, posZ, 0.0D, 0.0D, 0.0D);
    }
    ...
    public void spawnGooeyTypeParticles() {
        ...
        for (int a = 0; a < 360; a += 4) {
            ...
            Erebus.proxy.spawnCustomParticle(
                ...
                posY + 0.5D,
                posZ + MathHelper.cos((float) ang) * 1.0,
                -MathHelper.sin((float) ang) * 0.4,
                0D,
                MathHelper.cos((float) ang) * 0.4);
        }
    }
}
