...
public class EntityAnimatedBlock extends EntityMobBlock implements IEntityAdditionalSpawnData {
    ...
    protected final EntityAINearestAttackableTarget aiAttackNearestTarget = new EntityAINearestAttackableTarget(
        this,
        EntityMob.class,
        0,
        true);
    protected final EntityAIAttackOnCollide aiAttackOnCollide = new EntityAIAttackOnCollide(
        this,
        EntityMob.class,
        0.5D,
        false);
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 10.0D : 10D * ConfigHandler.INSTANCE.mobHealthMultipier);
    ...
    public void onLivingUpdate() {
        ...
        if (worldObj.isRemote) if (worldObj.getSunBrightness(1.0F) < 0.5F) lightUp(
            worldObj,
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (!worldObj.isRemote && is != null && is.getItem() == ModItems.wandOfAnimation) {
            ...
            player.openGui(
                Erebus.instance,
                CommonProxy.GuiID.PETRIFIED_CRAFT.ordinal(),
                player.worldObj,
                (int) player.posX,
                (int) player.posY,
                (int) player.posZ);
    ...
    protected void attackEntity(Entity entity, float distance) {
        if (distance > 0.0F && distance < 2.0F) attackEntityAsMob(entity);
    }

    @Override
    public boolean attackEntityAsMob(Entity entity) {
        boolean atk = false;
        if (super.attackEntityAsMob(entity)) if (entity instanceof EntityMob) atk = true;
        else atk = false;
        return atk;
    }
    ...
    public void writeEntityToNBT(NBTTagCompound data) {
        ...
        if (getOwnerName() == null) data.setString("OwnerUUID", "");
        else data.setString("OwnerUUID", getOwnerName());
    }
    ...
    public void readEntityFromNBT(NBTTagCompound data) {
        ...
        if (data.hasKey("OwnerUUID", 8)) s = data.getString("OwnerUUID");
        else {
            String s1 = data.getString("Owner");
            s = PreYggdrasilConverter.func_152719_a(s1);
        }

        if (s.length() > 0) setOwnerName(s);
    }
