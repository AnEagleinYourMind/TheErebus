...
public class EntityHoneyPotAnt extends EntityTameable {
    ...
    public void onUpdate() {
        ...
        if (worldObj.isRemote) setSize(0.9F + getHoneyBelly(), 0.4F);
    }

    @Override
    public boolean interact(EntityPlayer player) {
        ItemStack stack = player.inventory.getCurrentItem();
        if (stack == null) return super.interact(player);
        ...
        } else if (item == ModItems.antTamingAmulet) if (!isTamed()) {
            player.swingItem();
            setTamed(true);
            playTameEffect(true);
            return true;
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        if (isTamed()) {
            if (getHoneyBelly() > 0)
                entityDropItem(ItemMaterials.DATA.NECTAR.makeStack((int) (getHoneyBelly() * 10)), 0.0F);
        } else entityDropItem(ItemMaterials.DATA.NECTAR.makeStack(1), 0.0F);
    }
