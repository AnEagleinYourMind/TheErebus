...
public class EntityAIAntlionBossAttack<T extends Entity> extends EntityAIBase {
    ...
    public boolean continueExecuting() {
        ...
        return entitylivingbase == null ? false
            : !entitylivingbase.isEntityAlive() ? false
                : !longMemory ? !attacker.getNavigator()
                    .noPath()
    ...
    public void startExecuting() {
        attacker.getNavigator()
            .setPath(entityPathEntity, speedTowardsTarget);
        attackTimer = 0;
    }
    ...
    public void updateTask() {
        ...
        attacker.getLookHelper()
            .setLookPositionWithEntity(entitylivingbase, 20.0F, 20.0F);
        attacker.faceEntity(entitylivingbase, 50.0F, 50.0F);
        if ((longMemory || attacker.getEntitySenses()
            .canSee(entitylivingbase)) && --attackTimer <= 0) {
            attackTimer = failedPathFindingPenalty + 4
                + attacker.getRNG()
                    .nextInt(7);
            attacker.getNavigator()
                .tryMoveToEntityLiving(entitylivingbase, speedTowardsTarget);
            if (attacker.getNavigator()
                .getPath() != null) {
                PathPoint finalPathPoint = attacker.getNavigator()
                    .getPath()
                    .getFinalPathPoint();
                if (finalPathPoint != null && entitylivingbase
                    .getDistanceSq(finalPathPoint.xCoord, finalPathPoint.yCoord, finalPathPoint.zCoord) < 1D)
        ...
        if (attacker.getDistanceSq(entitylivingbase.posX, entitylivingbase.boundingBox.minY, entitylivingbase.posZ)
            <= distance) if (attackTick <= 0) {
                attackTick = 20;
                ...
                entitylivingbase.attackEntityFrom(
                    ...
                    (float) (ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 8D
                        : 8D * ConfigHandler.INSTANCE.mobAttackDamageMultiplier));
                entitylivingbase.addVelocity(
                    -MathHelper.sin(attacker.rotationYaw * 3.141593F / 180.0F) * 0.3F,
                    0.1D,
                    MathHelper.cos(attacker.rotationYaw * 3.141593F / 180.0F) * 0.3F);
            }

        if (attacker.getDistanceSq(entitylivingbase.posX, entitylivingbase.boundingBox.minY, entitylivingbase.posZ)
            > distance + 9D
            && attacker.getDistanceSq(entitylivingbase.posX, entitylivingbase.boundingBox.minY, entitylivingbase.posZ)
                < distance + 256.0D)
            if (attackTick <= 0) {
                ++shouldDo;
                if (shouldDo <= 2) attackTick = 5;
                else {
                    attackTick = 0;
                    shouldDo = 0;
                }
                if (shouldDo == 1) {
                    ...
                    thrownsand.setPosition(
                        attacker.posX - Math.sin(direction) * 3.5,
                        attacker.posY + attacker.height,
                        attacker.posZ + Math.cos(direction) * 3.5);
            ...
            if (entity != null) if (entity instanceof EntityLivingBase && !(entity instanceof EntityAntlionBoss)) {
                entity.attackEntityFrom(
                    ...
                    (float) (ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 8D
                        : 8D * ConfigHandler.INSTANCE.mobAttackDamageMultiplier));
                entity.addVelocity(
                    -MathHelper.sin(attacker.rotationYaw * 3.141593F / 180.0F) * 1D,
                    0.4D,
                    MathHelper.cos(attacker.rotationYaw * 3.141593F / 180.0F) * 1D);
