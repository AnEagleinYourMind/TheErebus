...
public class EntityAIBlockFollowOwner extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        else if (animatedBlock.getDistanceSqToEntity(entitylivingbase) < (double) (minDist * minDist)) return false;
        else {
    ...
    public void startExecuting() {
        distanceCounter = 0;
        avoidWater = animatedBlock.getNavigator()
            .getAvoidsWater();
        animatedBlock.getNavigator()
    ...
    public void resetTask() {
        ...
        animatedBlock.getNavigator()
            .setAvoidsWater(avoidWater);
    }

    @Override
    public void updateTask() {
        animatedBlock.getLookHelper()
            .setLookPositionWithEntity(theOwner, 10.0F, (float) animatedBlock.getVerticalFaceSpeed());
                        ...
                        for (int l = 0; l <= 4; ++l) {
                            for (int i1 = 0; i1 <= 4; ++i1) {
                                if ((l < 1 || i1 < 1 || l > 3 || i1 > 3)
                                    ...
                                    && !theWorld.getBlock(i + l, k, j + i1)
                                        .isNormalCube()
                                    && !theWorld.getBlock(i + l, k + 1, j + i1)
                                        .isNormalCube()) {
                                    animatedBlock.setLocationAndAngles(
                                        (double) ((float) (i + l) + 0.5F),
                                        (double) k,
                                        (double) ((float) (j + i1) + 0.5F),
                                        animatedBlock.rotationYaw,
                                        animatedBlock.rotationPitch);
                                    petPathfinder.clearPathEntity();
                                    return;
                                }
                            }
