...
public class EntityAIEatCrops extends EntityAIEatBlock {
    ...
    public boolean continueExecuting() {
        return entity instanceof EntityAnimal ? !((EntityAnimal) entity).isInLove() && !entity.isChild()
    ...
    protected void moveToLocation() {
        ...
        if (!grasshopper.isEating) if (!entity.getNavigator()
            .tryMoveToXYZ(cropX + 0.5D, cropY, cropZ + 0.5D, moveSpeed))
    ...
    protected void afterEaten() {
        ...
        grasshopper.setIsEating(false);
        reproCap++;
        if (reproCap == 6) if (grasshopper.worldObj.countEntities(EntityGrasshopper.class) < 80) {
