...
public class EntityAIAntPlantCrops extends EntityAIAntsBlock {
    ...
    protected boolean canEatBlock(Block block, int blockMeta) {
        ...
        if (block == Blocks.dirt || block == Blocks.grass) return true;
        else if (block.hasTileEntity(blockMeta)) return false;
    ...
    protected void moveToLocation() {
        PathEntity pathentity = blackAnt.worldObj
            .getEntityPathToXYZ(blackAnt, cropX, cropY + 1, cropZ, 16.0F, true, false, false, true);
        if (pathentity != null) {
            blackAnt.setPathToEntity(pathentity);
            blackAnt.getNavigator()
                .setPath(pathentity, 0.5D);
        }

        if (blackAnt.getDistance(cropX, cropY, cropZ) < 1.5D) blackAnt.getMoveHelper()
            .setMoveTo(cropX + 0.5D, cropY + 1, cropZ + 0.5D, 0.5D);
    }
    ...
    protected void afterEaten() {
        if (!blackAnt.worldObj.isRemote) {
            ...
            if (getTargetBlock() != Blocks.farmland) {
                ...
                blackAnt.worldObj.playSoundEffect(
                    (double) cropX + 0.5F,
                    (double) cropY + 0.5F,
                    (double) cropZ + 0.5F,
            ...
            if (!isFilterSlotEmpty() && !isAntInvSlotEmpty()) {
                ...
                if (filterItem == invItem && filterItemMeta == invItemMeta) {
                    Utils.rightClickItemAt(
                        blackAnt.worldObj,
                        cropX,
                        cropY,
                        cropZ,
                        1,
                        new ItemStack(invItem, invItemMeta));
                    blackAnt.setInventorySlotContents(
                        INVENTORY_SLOT,
                        new ItemStack(invItem, getAntInvSlotStack().stackSize - 1, invItemMeta));
                    if (getAntInvSlotStack().stackSize < 1) blackAnt.setInventorySlotContents(INVENTORY_SLOT, null);
                }
            }
        }
    }
