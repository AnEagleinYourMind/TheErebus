...
public abstract class EntityAIAntsBlock extends EntityAIBase {
    ...
    public EntityAIAntsBlock(EntityLiving entity, Block block, int maxGrowthMetadata, ItemStack seed, double moveSpeed,
        int eatSpeed) {
        this.entity = entity;
    ...
    public void updateTask() {
        ...
        for (int i = 0; i < CHECKS_PER_TICK; i++) if (!hasTarget) {
            ...
            for (int y = -2; y < 1; y++) if (canEatBlock(
            ...
            if (flag) {
                ...
                if (!canEatBlock(
                    ...
                    if (seed != null) Utils.dropStack(entity.worldObj, cropX, cropY, cropZ, seed.copy());
                    hasTarget = false;
                    eatTicks = 0;
                    afterEaten();
                }
