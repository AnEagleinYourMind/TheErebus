...
public class EntityAIAntBonemealCrops extends EntityAIAntsBlock {
    ...
    protected boolean canEatBlock(Block block, int blockMeta) {
        ...
        if (block instanceof BlockCrops && blockMeta < 7) return true;
    ...
    protected void moveToLocation() {
        PathEntity pathentity = blackAnt.worldObj
            .getEntityPathToXYZ(blackAnt, cropX, cropY, cropZ, 16.0F, true, false, false, true);
        ...
        if (blackAnt.getDistance(cropX, cropY, cropZ) < 1.5D) blackAnt.getMoveHelper()
            .setMoveTo(cropX + 0.5D, cropY, cropZ + 0.5D, 0.5D);
    }
    ...
    protected void afterEaten() {
        ...
        if (!blackAnt.worldObj.isRemote) if (!isFilterSlotEmpty() && !isAntInvSlotEmpty()) {
