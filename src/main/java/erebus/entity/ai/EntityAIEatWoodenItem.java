...
public class EntityAIEatWoodenItem extends EntityAIEatBlock {
    ...
    protected boolean canEatBlock(Block block, int blockMeta) {
        ...
        if (block == ModBlocks.scorchedWood) return false;

        if (block == ModBlocks.planks && blockMeta == EnumWood.Rotten.ordinal()) return false;

        if (block == EnumWood.Bamboo.getSlab()) return false;

        if (block == EnumWood.Bamboo.getStair()) return false;

        if (block == ModBlocks.planks && blockMeta == EnumWood.Bamboo.ordinal()) return false;

        if (ConfigHandler.INSTANCE.beetleLarvaEating == 2) return true;
        else if (block.getMaterial() != Material.wood || block instanceof BlockLog
            ...
            || block instanceof BlockBambooTorch) return false;
        else if (ConfigHandler.INSTANCE.beetleLarvaEating == 0 && block.hasTileEntity(blockMeta)) return false;
    ...
    protected void moveToLocation() {
        ...
        if (!beetleLarva.isEating) entity.getMoveHelper()
            .setMoveTo(cropX + 0.5D, cropY, cropZ + 0.5D, moveSpeed);
    }
