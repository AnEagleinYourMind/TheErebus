...
public class EntityAITarantulaMinibossAttack extends EntityAIBase {
    ...
    public EntityAITarantulaMinibossAttack(EntityCreature entityCreature, Class<? extends Entity> entityClass,
        double moveSpeed, boolean memory) {
    ...
    public boolean shouldExecute() {
        ...
        if (entitylivingbase == null) return false;
            ...
            findAttemptCount = 4 + attacker.getRNG()
                .nextInt(7);
    ...
    public void startExecuting() {
        if (attacker.getHealth() <= 150) attacker.getNavigator()
            .setPath(entityPathEntity, speedTowardsTarget);
        else attacker.getNavigator()
            .setPath(entityPathEntity, 0);
        findAttemptCount = 0;
    }
    ...
    public void updateTask() {
        ...
        if ((longMemory && attacker.getHealth() <= 150 || attacker.getEntitySenses()
            .canSee(entitylivingbase)) && --findAttemptCount <= 0 && attacker.getHealth() <= 150) {
            findAttemptCount = failedPathFindingPenalty + 4
                + attacker.getRNG()
                    .nextInt(7);
            attacker.getNavigator()
        ...
        if (attacker.getDistanceSq(entitylivingbase.posX, entitylivingbase.boundingBox.minY, entitylivingbase.posZ)
            ...
            && attacker.getDistanceSq(entitylivingbase.posX, entitylivingbase.boundingBox.minY, entitylivingbase.posZ)
                < d0 + 256.0D
            && attacker.getHealth() < 150) if (attackTick <= 0) {
                ++shouldDo;
