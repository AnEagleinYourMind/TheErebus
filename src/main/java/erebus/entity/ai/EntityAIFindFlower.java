...
public abstract class EntityAIFindFlower extends EntityAIBase {
    ...
    public boolean continueExecuting() {
        return entity.worldObj
            .getEntitiesWithinAABBExcludingEntity(
                entity,
                AxisAlignedBB.getBoundingBox(flowerX, flowerY + 1, flowerZ, flowerX + 1, flowerY + 2, flowerZ + 1))
            .isEmpty();
    }
    ...
    public void updateTask() {
        ...
        for (int i = 0; i < CHECKS_PER_TICK; i++) if (!hasTarget) {
            ...
            for (int y = -8; y < 8; y++) if (canPolinate(
            ...
            entity.getLookHelper()
                .setLookPosition(flowerX + 0.5D, flowerY + 0.5D, flowerZ + 0.5D, 30.0F, 8.0F);
            ...
            if (flag) {
                prepareToPollinate();
                collectTicks++;
                entity.worldObj.destroyBlockInWorldPartially(
                    entity.getEntityId(),
                    flowerX,
                    flowerY,
                    flowerZ,
                    getScaledcollectTicks());
                if (!canPolinate(
                    entity.worldObj.getBlock(flowerX, flowerY, flowerZ),
                    entity.worldObj.getBlockMetadata(flowerX, flowerY, flowerZ))) hasTarget = false;
