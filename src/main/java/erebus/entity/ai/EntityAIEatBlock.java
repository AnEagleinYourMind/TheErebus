...
public abstract class EntityAIEatBlock extends EntityAIBase {
    ...
    public void updateTask() {
        ...
        for (int i = 0; i < CHECKS_PER_TICK; i++) if (!hasTarget) {
            increment();

            Point p = getNextPoint();
            for (int y = -2; y < 2; y++) if (canEatBlock(
                entity.worldObj.getBlock(xCoord + p.x, yCoord + y, zCoord + p.y),
                entity.worldObj.getBlockMetadata(xCoord + p.x, yCoord + y, zCoord + p.y))) {
                    cropX = xCoord + p.x;
                    cropY = yCoord + y;
                    cropZ = zCoord + p.y;
                    hasTarget = true;
            ...
            entity.getLookHelper()
                .setLookPosition(cropX + 0.5D, cropY + 0.5D, cropZ + 0.5D, 30.0F, 8.0F);
            AxisAlignedBB blockbounds = getBlockAABB(cropX, cropY, cropZ);
            boolean flag = entity.boundingBox.maxY >= blockbounds.minY && entity.boundingBox.minY <= blockbounds.maxY
                && entity.boundingBox.maxX >= blockbounds.minX
                && entity.boundingBox.minX <= blockbounds.maxX
                && entity.boundingBox.maxZ >= blockbounds.minZ
                && entity.boundingBox.minZ <= blockbounds.maxZ;

            if (flag) {
                prepareToEat();
                eatTicks++;
                entity.worldObj
                    .destroyBlockInWorldPartially(entity.getEntityId(), cropX, cropY, cropZ, getScaledEatTicks());
                if (!canEatBlock(
                    entity.worldObj.getBlock(cropX, cropY, cropZ),
                    entity.worldObj.getBlockMetadata(cropX, cropY, cropZ))) hasTarget = false;
                else if (EAT_SPEED <= eatTicks) {
                    entity.worldObj.playAuxSFXAtEntity(
                        null,
                        2001,
                        cropX,
                        cropY,
                        cropZ,
                        Block.getIdFromBlock(entity.worldObj.getBlock(cropX, cropY, cropZ))
                            + (maxGrowthMetadata << 12));
                    if (getTargetBlock() != Blocks.tallgrass) Utils.dropStack(
                        entity.worldObj,
                        cropX,
                        cropY,
                        cropZ,
                        new ItemStack(
                            getTargetBlock().getItemDropped(
                                entity.worldObj.getBlockMetadata(cropX, cropY, cropZ),
    ...
    private void increment() {
        spiralIndex++;
        if (spiralIndex >= spiral.size()) spiralIndex = 0;
    }
