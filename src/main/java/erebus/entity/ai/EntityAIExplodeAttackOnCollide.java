...
public class EntityAIExplodeAttackOnCollide extends EntityAIBase {
    ...
    public EntityAIExplodeAttackOnCollide(EntityCreature entityCreature, Class<? extends Entity> par2Class, double par3,
        boolean par5) {
    ...
    public boolean shouldExecute() {
        ...
        if (entitylivingbase == null) return false;
            ...
            field_75445_i = 4 + attacker.getRNG()
                .nextInt(7);
    ...
    public void updateTask() {
        ...
        if ((longMemory || attacker.getEntitySenses()
            .canSee(entitylivingbase)) && --field_75445_i <= 0) {
            field_75445_i = failedPathFindingPenalty + 4
                + attacker.getRNG()
                    .nextInt(7);
            attacker.getNavigator()
        ...
        if (attacker.getDistanceSq(entitylivingbase.posX, entitylivingbase.boundingBox.minY, entitylivingbase.posZ)
            <= d0) if (attackTick <= 0) {
                attackTick = 20;
                if (attacker.getHeldItem() != null) attacker.swingItem();
                attacker.attackEntityAsMob(entitylivingbase);
                boolean rule = worldObj.getGameRules()
                    .getGameRuleBooleanValue("mobGriefing");
                if (rule) worldObj.createExplosion(
                    attacker,
