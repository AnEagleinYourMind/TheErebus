...
public class EntityBombardierBeetleLarva extends EntityBeetleLarva {
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            if (getInflateSize() <= 0) setInflateSize(0);
            if (getInflateSize() >= 100) explode();
            if (getAttackTarget() == null) setInflateSize(getInflateSize() - 2);
            if (getAttackTarget() != null) {
                float distance = (float) getDistance(
                    getAttackTarget().posX,
                    getAttackTarget().boundingBox.minY,
                    getAttackTarget().posZ);
                if (getInflateSize() < 100 && distance <= 4) setInflateSize(getInflateSize() + 2);
                if (getInflateSize() < 100 && distance > 4) setInflateSize(getInflateSize() - 2);
            }
        }
    }
    ...
    public boolean attackEntityAsMob(Entity entity) {
        if (entity != null && getDistanceToEntity(entity) <= 1.5F
