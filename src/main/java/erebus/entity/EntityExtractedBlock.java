...
public class EntityExtractedBlock extends EntityFlying implements IEntityAdditionalSpawnData {
    ...
    public void onUpdate() {
        if (!worldObj.isRemote) {
            ...
            if (posX == targetX && posZ == targetZ) if (posY >= targetY) motionY += 0.75D;

            if (posY <= targetY) if (onGround) {
                setDead();
        ...
        if (entity instanceof EntityPlayer) {
            if (!worldObj.isRemote)
                if (((EntityPlayer) entity).inventory.addItemStackToInventory(new ItemStack(blockID, 1, blockMeta)));
                    ...
                    .dropStack(worldObj, (int) posX, (int) posY, (int) posZ, new ItemStack(blockID, 1, blockMeta));
        } else if (!worldObj.isRemote) worldObj.setBlock((int) posX, (int) posY, (int) posZ, blockID, blockMeta, 3);
