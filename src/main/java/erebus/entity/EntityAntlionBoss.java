...
public class EntityAntlionBoss extends EntityMob implements IBossDisplayData {
    ...
    protected void entityInit() {
        super.entityInit();
        dataWatcher.addObject(21, Byte.valueOf((byte) 3));
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 400D : 400D * ConfigHandler.INSTANCE.mobHealthMultipier);
        getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(
            ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 6D
    ...
    public void onUpdate() {
        ...
        if (blamCount == 5) setBlam(5, (byte) 3);

        if (blamCount < 75 && blamCount > 10)
            if (dataWatcher.getWatchableObjectByte(21) >= 1 && dataWatcher.getWatchableObjectByte(21) <= 2)
                if (blamCount % 5 == 0 && blamCount % 10 != 0) setBlam(blamCount, (byte) 2);
                else if (blamCount % 10 == 0) {
                    setBlam(blamCount, (byte) 1);
                    areaOfEffect2();
                }

        if (blamCount == 75) setBlam(75, (byte) 3);

        if (dataWatcher.getWatchableObjectByte(21) == 1) spawnRumbleParticles();
    ...
    public boolean attackEntityFrom(DamageSource source, float damage) {
        if (source.equals(DamageSource.inWall) || source.equals(DamageSource.drown)
            || source instanceof EntityDamageSourceIndirect) return false;
        return super.attackEntityFrom(source, damage);
    }

    public void spawnBlamParticles() {
        if (onGround) PacketPipeline.sendToAllAround(this, 64D, new PacketParticle(this, ParticleType.ANTLION_BLAM));
    }
    ...
    protected Entity areaOfEffect2() {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (entity != null) if (entity instanceof EntityLivingBase && !(entity instanceof EntityAntlionBoss)) {
                float Knockback = (-3.0F + worldObj.rand.nextInt(4)) * 0.1F;
                entity.attackEntityFrom(
                    DamageSource.causeMobDamage(this),
                    (float) (ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 2D
                        : 2D * ConfigHandler.INSTANCE.mobAttackDamageMultiplier));
                entity.addVelocity(
                    -MathHelper.sin(rotationYaw * -3.141593F + worldObj.rand.nextInt(3) + 0.141593F / 180.0F)
                        * Knockback,
                    0.01D,
                    MathHelper.cos(rotationYaw * -3.141593F + worldObj.rand.nextInt(3) + 0.141593F / 180.0F)
                        * Knockback);
    ...
    protected void onDeathUpdate() {
        ...
        if (deathTicks % 25 == 1) {
            ...
            worldObj.playSoundEffect(
                ...
                "erebus:antliongrowl",
                1.0F,
                worldObj.rand.nextFloat() * 0.1F + 0.3F);
            worldObj.playSoundEffect(
                posX,
                posY,
                posZ,
                "erebus:antliongrowl",
                1.0F,
                worldObj.rand.nextFloat() * 0.1F + 0.1F);
        }
        ...
        if (!worldObj.isRemote) if (deathTicks > 150 && deathTicks % 5 == 0) {
            i = 1000;
        ...
        if (deathTicks == 200 && !worldObj.isRemote) {
            ...
            if (getInPyramid() == 1) {
                AntlionMazeDungeon.setTeleporter(
                    worldObj,
                    getSpawnPointX(),
                    getSpawnPointY(),
                    getSpawnPointZ(),
            ...
            worldObj.setBlock(
                ...
                ModBlocks.antlionEgg);
            Utils.dropStackNoRandom(
                worldObj,
                MathHelper.floor_double(posX),
                MathHelper.floor_double(posY + 1.5),
                MathHelper.floor_double(posZ),
            ...
            Utils.dropStackNoRandom(
                ...
                MathHelper.floor_double(posY + 1.5),
                MathHelper.floor_double(posZ),
                new ItemStack(ModItems.warHammer));
            setDead();
        }
    }
    ...
    public void destroyBlocksInAABB(AxisAlignedBB AABB) {
        ...
        for (int k1 = i; k1 <= l; ++k1) for (int l1 = j; l1 <= i1; ++l1) for (int i2 = k; i2 <= j1; ++i2) {
