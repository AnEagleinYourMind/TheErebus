...
public class EntitySolifuge extends EntityMob {
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        int amount;
        for (amount = 0; amount < chance; ++amount) entityDropItem(ItemMaterials.DATA.BIO_VELOCITY.makeStack(), 0.0F);
    }
    ...
    public void setDead() {
        ...
        if (!worldObj.isRemote && getHealth() <= 0.0F) for (int i = 0; i < 4; i++) {
            EntitySolifugeSmall entitySolifugeSmall = new EntitySolifugeSmall(worldObj);
            entitySolifugeSmall.setPosition(
                posX + (rand.nextFloat() * 0.03D - rand.nextFloat() * 0.03D),
                posY + 1,
                posZ + (rand.nextFloat() * 0.03D - rand.nextFloat() * 0.03D));
