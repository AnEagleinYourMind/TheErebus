...
public class EntityZombieAntSoldier extends EntityMob {
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 40D : 40D * ConfigHandler.INSTANCE.mobHealthMultipier);
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        for (int a = 0; a < amount; ++a) entityDropItem(ItemMaterials.DATA.PLATE_ZOMBIE_ANT.makeStack(), 0.0F);
        if (rand.nextInt(5) == 0)
            if (rand.nextBoolean()) entityDropItem(ItemMaterials.DATA.ANT_PHEROMONES.makeStack(), 0.0F);
            else entityDropItem(ItemMaterials.DATA.TERPSISHROOM.makeStack(), 0.0F);
    }

    @Override
    public void onUpdate() {
        super.onUpdate();
        if (worldObj.isRemote) worldObj.spawnParticle(
            "reddust",
