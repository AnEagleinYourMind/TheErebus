...
public class EntityLavaWebSpider extends EntityMob {
    ...
    public void onUpdate() {
        super.onUpdate();
        if (!worldObj.isRemote) setBesideClimbableBlock(isCollidedHorizontally);
    ...
    public void onLivingUpdate() {
        if (rand.nextInt(50) == 0) {
            ...
            if (worldObj.isAirBlock(x, y, z) && Blocks.fire.canPlaceBlockAt(
                worldObj,
                x + MathHelper.floor_double(offSetX),
                y,
                z + MathHelper.floor_double(offSetZ)))
                worldObj.setBlock(
                    x + MathHelper.floor_double(offSetX),
                    y,
                    z + MathHelper.floor_double(offSetZ),
    ...
    protected void attackEntity(Entity entity, float distance) {
        ...
        if (distance >= 5 & distance <= 16.0F) if (attackTime == 0) {
            ++shouldDo;
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        if (recentlyHit && (rand.nextInt(3) == 0 || rand.nextInt(1 + looting) > 0)) dropItem(Items.spider_eye, 1);
    }
    ...
    public boolean getCanSpawnHere() {
        return worldObj.checkNoEntityCollision(boundingBox) && worldObj.getCollidingBoundingBoxes(this, boundingBox)
            .isEmpty() && worldObj.isMaterialInBB(boundingBox, Material.lava);
    }
