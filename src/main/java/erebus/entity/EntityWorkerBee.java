...
public class EntityWorkerBee extends EntityTameable {
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 30D : 30D * ConfigHandler.INSTANCE.mobHealthMultipier);
    }
    ...
    public boolean getCanSpawnHere() {
        ...
        if (light >= 0F)
            return worldObj.checkNoEntityCollision(boundingBox) && worldObj.getCollidingBoundingBoxes(this, boundingBox)
    ...
    public boolean isOnLadder() {
        ...
        if (isCollidedHorizontally) if (block != ModBlocks.stiga
            || !block.hasTileEntity(worldObj.getBlockMetadata((int) posX, (int) posY - 1, (int) posZ)))
    ...
    public void onUpdate() {
        if (!isFlying()) wingFloat = 0.0F;
        else wingFloat = mathWings.swing(4.0F, 0.1F);

        if (motionY < 0.0D) motionY *= 0.6D;

        if (!worldObj.isRemote) {
            if (getEntityToAttack() == null && !beePollinating && !beeCollecting) {
                if (rand.nextInt(200) == 0) if (!beeFlying) setBeeFlying(true);
                else setBeeFlying(false);

                if (beeFlying) flyAbout();
                else land();
            }
            ...
            if (getEntityToAttack() != null) {
                currentFlightTarget = new ChunkCoordinates(
                    ...
                    (int) ((int) getEntityToAttack().posY + getEntityToAttack().getEyeHeight()),
            ...
            if (MathHelper.floor_double(posX) == getDropPointX() && MathHelper.floor_double(posY) == getDropPointY() + 1
                && MathHelper.floor_double(posZ) == getDropPointZ()
                && getNectarPoints() > 0) {
    ...
    private void addHoneyToInventory(int x, int y, int z) {
        if (Utils.addItemStackToInventory(
            Utils.getTileEntity(worldObj, x, y, z, IInventory.class),
            ItemMaterials.DATA.NECTAR.makeStack(2))) setNectarPoints(getNectarPoints() - 2);
    }
    ...
    public void flyAbout() {
        if (currentFlightTarget != null && currentFlightTarget.posX != getDropPointX()
            && currentFlightTarget.posY != getDropPointY()
            && currentFlightTarget.posZ != getDropPointZ())
            if (!worldObj.isAirBlock(currentFlightTarget.posX, currentFlightTarget.posY, currentFlightTarget.posZ)
                || currentFlightTarget.posY < 1) currentFlightTarget = null;

        if (getTameState() == 0) if (currentFlightTarget == null || rand.nextInt(30) == 0
            || currentFlightTarget.getDistanceSquared((int) posX, (int) posY, (int) posZ) < 10F)
            currentFlightTarget = new ChunkCoordinates(
                (int) posX + rand.nextInt(7) - rand.nextInt(7),
                (int) posY + rand.nextInt(6) - 2,
                (int) posZ + rand.nextInt(7) - rand.nextInt(7));

        if (getTameState() == 1) if (currentFlightTarget == null || rand.nextInt(30) == 0
            || currentFlightTarget.getDistanceSquared((int) posX, (int) posY, (int) posZ) < 10F)
            currentFlightTarget = new ChunkCoordinates(
                getDropPointX() + rand.nextInt(32) - rand.nextInt(32),
                getDropPointY() + rand.nextInt(8) - 2,
                getDropPointZ() + rand.nextInt(32) - rand.nextInt(32));

        flyToTarget();
    }

    public void flyToTarget() {
        if (currentFlightTarget != null && getEntityToAttack() == null
            && worldObj.getBlock(
                MathHelper.floor_double(posX),
                MathHelper.floor_double(posY) + 1,
                MathHelper.floor_double(posZ)) == ModBlocks.erebusFlower
            && worldObj.isAirBlock(currentFlightTarget.posX, currentFlightTarget.posY + 1, currentFlightTarget.posZ)
            || currentFlightTarget != null && getEntityToAttack() == null
                && worldObj.getBlock(
                    MathHelper.floor_double(posX),
                    MathHelper.floor_double(posY) + 1,
                    MathHelper.floor_double(posZ)) == ModBlocks.erebusFlower
                && worldObj
                    .isAirBlock(currentFlightTarget.posX, currentFlightTarget.posY + 2, currentFlightTarget.posZ)) {
            if (worldObj.getBlock(currentFlightTarget.posX, currentFlightTarget.posY, currentFlightTarget.posZ)
                == ModBlocks.stiga
                || Utils.getTileEntity(
                    worldObj,
                    currentFlightTarget.posX,
                    currentFlightTarget.posY,
                    currentFlightTarget.posZ,
                    IInventory.class) != null) {
                if (worldObj
                    .getEntitiesWithinAABBExcludingEntity(
                        ...
                        AxisAlignedBB.getBoundingBox(
                            ...
                            currentFlightTarget.posZ,
                            currentFlightTarget.posX + 1,
                            currentFlightTarget.posY + 2,
                            currentFlightTarget.posZ + 1))
                    .isEmpty())
                    setPosition(currentFlightTarget.posX, currentFlightTarget.posY + 1, currentFlightTarget.posZ);
            } else currentFlightTarget = null;
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (!worldObj.isRemote && is != null && is.getItem() == ModItems.nectarCollector) if (getNectarPoints() > 0) {
        ...
        if (is != null && is.getItem() == ModItems.beeTamingAmulet
            && is.hasTagCompound()
            && is.stackTagCompound.hasKey("homeX")) {
            setDropPoint(
                is.getTagCompound()
                    .getInteger("homeX"),
                is.getTagCompound()
    ...
    public boolean attackEntityAsMob(Entity entity) {
        return entity.attackEntityFrom(
            ...
            (float) (ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 4D
                : 4D * ConfigHandler.INSTANCE.mobAttackDamageMultiplier));
    }
