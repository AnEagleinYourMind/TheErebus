...
public class EntityPunchroom extends EntityMob {
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        if (rand.nextInt(5) == 0) entityDropItem(ItemMaterials.DATA.ELASTIC_FIBRE.makeStack(1 + looting), 0.0F);
    }
    ...
    public void onUpdate() {
        ...
        } else if (!onGround && flag) squishAmount = 1.0F;
    ...
    protected void updateEntityActionState() {
        ...
        if (onGround && shroomJumpDelay-- <= 0) {
            ...
            if (entityplayer != null) shroomJumpDelay /= 3;
            isJumping = true;
            moveStrafing = 1.0F - rand.nextFloat() * 2.0F;
            moveForward = 1;
        } else {
            isJumping = false;
            if (onGround) moveStrafing = moveForward = 0.0F;
        }
    }
        ...
        if (!player.capabilities.isCreativeMode && !worldObj.isRemote) {
            ...
            if (player.boundingBox.maxY >= boundingBox.minY && player.boundingBox.minY <= boundingBox.maxY)
                if (worldObj.difficultySetting.ordinal() > 1)
                    if (worldObj.difficultySetting == EnumDifficulty.NORMAL) knockback = 0.4F;
                    else if (worldObj.difficultySetting == EnumDifficulty.HARD) knockback = 0.6F;
