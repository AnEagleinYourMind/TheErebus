...
public class EntityGooBall extends EntityThrowable {
    ...
    public void onUpdate() {
        super.onUpdate();
        if (worldObj.isRemote) trailParticles(worldObj, posX - 0.5D, posY, posZ - 0.5D, rand);
    }
            ...
            if (mop.entityHit instanceof EntityPlayer) {
                if (!worldObj.isRemote) {
                    ((EntityLivingBase) mop.entityHit)
                        .addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 5 * 20, 3));
                    setDead();
                }
                if (worldObj.isRemote) PacketPipeline
                    .sendToAllAround(mop.entityHit, 64D, new PacketParticle(this, ParticleType.BEETLE_LARVA_SQUISH));
            }
            if (mop.typeOfHit != null && mop.typeOfHit == MovingObjectType.BLOCK) setDead();
        }
