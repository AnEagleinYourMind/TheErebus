...
public class EntityMosquito extends EntityMob {
    ...
    public void onUpdate() {
        ...
        if (consumptionTimer > 0) if (--consumptionTimer == 0) setBloodConsumed(0);
        if (ridingEntity != null) {
            suckFloat = 1.0F + mathSucking.swing(1.0F, 0.15F);
            if (rand.nextInt(10) == 0) for (int i = 0; i < 8; i++) worldObj.spawnParticle(
                "reddust",
                posX + (rand.nextFloat() - rand.nextFloat()),
                posY + rand.nextFloat() + 1D,
                posZ + (rand.nextFloat() - rand.nextFloat()),
                0,
                0,
                0);
            wingFloat = 0.0F;
        ...
        if (!worldObj.isRemote) {
            ...
            if (entityToAttack != null && ridingEntity == null
                && getDistanceToEntity(entityToAttack) <= 1.2D
                && !worldObj.isRemote
                && entityToAttack.riddenByEntity == null
                && getBloodConsumed() < maxBloodLevel) mountEntity(entityToAttack);
            if (ridingEntity != null && ridingEntity instanceof EntityLivingBase
                && getBloodConsumed() == maxBloodLevel) {
                entityToAttack = null;
                mountEntity(null);
            }
        }
    ...
    protected void updateEntityActionState() {
        ...
        if (m == Blocks.water && rand.nextInt(10) == 0
            && (motionX > 0.05D || motionZ > 0.05D || motionX < -0.05D || motionZ < -0.05D)) motionY = 0.25D;
    }
    ...
    public void onLivingUpdate() {
        if (!worldObj.isRemote) {
            ...
            if (entityToAttack != null
                && entityToAttack.posY + entityToAttack.getEyeHeight() > posY + getEyeHeight() + heightOffset) {
            ...
            if (entityToAttack == null) flyAbout();
        }
    ...
    public boolean attackEntityFrom(DamageSource source, float par2) {
        if (isEntityInvulnerable()) return false;
            ...
            if (riddenByEntity != var3 && ridingEntity != var3) {
                if (var3 != this) entityToAttack = (EntityLivingBase) var3;
                return true;
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 10 + maxBloodLevel
                : (10 + maxBloodLevel) * ConfigHandler.INSTANCE.mobHealthMultipier);
    ...
    public int getDamage() {
        ...
        if (this.isPotionActive(Potion.weakness)) var2 -= 2 << getActivePotionEffect(Potion.weakness).getAmplifier();
        return var2;
    }

    @Override
    protected String getLivingSound() {
        if (ridingEntity != null) return "erebus:mosquitosucking";
        else return "erebus:mosquitoflying";
    }
    ...
    public boolean getCanSpawnHere() {
        ...
        for (int p1 = n; p1 < o; p1++) for (int q1 = p; q1 < q; q1++) for (int n2 = n1; n2 < o1; n2++) {
            if (!worldObj.blockExists(p1, q1, n2)) continue;
            Block o2 = worldObj.getBlock(p1, q1, n2);
            if (o2.isAir(worldObj, p1, q1, n2)) continue;
            if (o2 == Blocks.water) return true;
        }
        return false;
    }
