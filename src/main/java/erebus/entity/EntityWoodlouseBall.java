...
public class EntityWoodlouseBall extends EntityThrowable {
    ...
    protected void onImpact(MovingObjectPosition target) {
        if (target.entityHit != null) {
            ...
            if (isBurning() && !(target.entityHit instanceof EntityEnderman)) target.entityHit.setFire(5);
            for (int i = 0; i < 8; i++) worldObj.spawnParticle("reddust", posX, posY, posZ, 0.0D, 0.0D, 0.0D);
            if (!worldObj.isRemote) {
                target.entityHit.addVelocity(
                    -MathHelper.sin(getThrower().rotationYaw * 3.141593F / 180.0F) * 0.5F,
                    0.1D,
                    MathHelper.cos(getThrower().rotationYaw * 3.141593F / 180.0F) * 0.5F);
                spawnWoodlouse();
            }
        } else if (!worldObj.isRemote) spawnWoodlouse();
    }
