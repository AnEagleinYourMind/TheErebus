...
public class EntityAnimatedBambooCrate extends EntityAnimatedBlock implements IInventory {
    ...
    public TileEntityBambooCrate getTile() {
        ...
        for (int i = 0; i < inventory.length; i++) crate.setInventorySlotContents(i, inventory[i]);
        return crate;
    }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (is != null && is.getItem() == ModItems.wandOfAnimation) {
            ...
            worldObj.setBlock(
                ...
                blockMeta,
                3);
            TileEntityBambooCrate chest = Utils.getTileEntity(
                worldObj,
                MathHelper.floor_double(posX),
                MathHelper.floor_double(posY),
                MathHelper.floor_double(posZ),
                TileEntityBambooCrate.class);
            for (int i = 0; i < chest.getSizeInventory(); i++) chest.setInventorySlotContents(i, inventory[i]);
            ...
            player.openGui(
                Erebus.instance,
                CommonProxy.GuiID.ANIMATED_BAMBOO_CRATE.ordinal(),
                player.worldObj,
                getEntityId(),
                0,
                0);
