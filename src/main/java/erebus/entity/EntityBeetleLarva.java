...
public class EntityBeetleLarva extends EntityAnimal {
    ...
    protected void entityInit() {
        super.entityInit();
        dataWatcher.addObject(28, Float.valueOf(1.0F));
        dataWatcher.addObject(29, Byte.valueOf((byte) 0));
    }

    public void setLarvaSize(float byteSize) {
        dataWatcher.updateObject(28, Float.valueOf(byteSize));
    ...
    protected boolean canDespawn() {
        if (getTame() != 0 && getTame() != 4) return false;
        else return true;
    }
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 8D : 8D * ConfigHandler.INSTANCE.mobHealthMultipier);
    ...
    public void onCollideWithPlayer(EntityPlayer player) {
        ...
        byte duration = 0;
        if (!worldObj.isRemote && player.boundingBox.maxY >= boundingBox.minY
            && player.boundingBox.minY <= boundingBox.maxY
            && player.boundingBox.maxX >= boundingBox.minX
            && player.boundingBox.minX <= boundingBox.maxX
            && player.boundingBox.maxZ >= boundingBox.minZ
            && player.boundingBox.minZ <= boundingBox.maxZ
            ...
            if (duration > 0) player.addPotionEffect(new PotionEffect(Potion.confusion.id, duration * 20, 0));
            setisSquashed(true);
            setDead();
            onDeathUpdate();
    ...
    protected String getLivingSound() {
        ...
        if (isEating) actionSound = "erebus:beetlelarvamunch";
        return actionSound;
    }
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) if (getLarvaSize() > 1.8F) {
            setDead();
            spawnBeetle();
        }
    }
    ...
    public void onDeathUpdate() {
        ...
        if (isSquashed) {
            ...
            if (!worldObj.isRemote) {
                if (rand.nextInt(200) == 0) entityDropItem(new ItemStack(Items.diamond), 0.0F);
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        if (isBurning()) entityDropItem(new ItemStack(ModItems.food, 1, 1), 0.0F);
        else entityDropItem(new ItemStack(ModItems.food, 1, 0), 0.0F);
    }

    private boolean isStick(ItemStack stack) {
        int stick = OreDictionary.getOreID("stickWood");
        for (int id : OreDictionary.getOreIDs(stack)) if (id == stick) return true;
        return false;
    }
    ...
    public void setMoveTasks(boolean task) {
        if (!task) tasks.removeTask(aiWander);
        else tasks.addTask(2, aiWander);
    }
    ...
    public void onDeath(DamageSource dmgSrc) {
        ...
        if (dmgSrc instanceof EntityDamageSource) {
            ...
            if (killer instanceof EntityPlayer) {
                ...
                for (EntityItem entityDrop : capturedDrops) if (entityDrop != null) {
