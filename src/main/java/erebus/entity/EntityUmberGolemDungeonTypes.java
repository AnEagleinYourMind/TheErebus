...
public class EntityUmberGolemDungeonTypes extends EntityMob implements IEntityAdditionalSpawnData, IBossDisplayData {
    ...
    /*
     * protected String getLivingSound() { return "erebus:umbergolemsound"; } protected String getHurtSound() { return
     * "erebus:umbergolemhurt"; }
     */
    ...
    public void onUpdate() {
        ...
        if (getAttackTarget() != null) {
            ...
            if (!hasBlock) {
                if (getRangeAttackTimer() < 100 && distance > 3) setRangeAttackTimer(getRangeAttackTimer() + 2);
                if (getRangeAttackTimer() == 100 && distance > 3) shootMissile(getAttackTarget(), distance);
            }
            ...
            if (isCollidedHorizontally) {
                ...
                removeBlock(
                    (int) (posX + -Math.sin(direction) * 1.5D),
                    (int) posY,
                    (int) (posZ + Math.cos(direction) * 1.5D));
                removeBlock(
                    (int) (posX + -Math.sin(direction) * 1.5D),
                    (int) posY + 1,
                    (int) (posZ + Math.cos(direction) * 1.5D));
    ...
    protected void removeBlock(int x, int y, int z) {
        ...
        if (hardness <= 0.0F || block == null || !canBreakBlock(block, blockMeta) || !isInSamePos()) hasBlock = false;
        ...
        if (hasBlock) {
            ...
            if (worldObj.rand.nextInt(30) == 0) worldObj.playAuxSFXAtEntity(
                null,
                2001,
                blockX,
                blockY,
                blockZ,
                Block.getIdFromBlock(worldObj.getBlock(blockX, blockY, blockZ)));
    ...
    protected boolean canBreakBlock(Block block, int blockMeta) {
        if (block == ModBlocks.gneiss) return false;
        if (block.hasTileEntity(blockMeta)) return false;
        return true;
    }
    ...
    protected boolean Attack(Entity entity) {
        if (!worldObj.isRemote && canEntityBeSeen(entity)) {
            ...
            entity.attackEntityFrom(
                ...
                (float) (ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? getAttackStrength()
                    : getAttackStrength() * ConfigHandler.INSTANCE.mobAttackDamageMultiplier));
            entity.addVelocity(
                -MathHelper.sin(rotationYaw * 3.141593F / 180.0F) * knockback * 0.5F,
                0.4D,
                MathHelper.cos(rotationYaw * 3.141593F / 180.0F) * knockback * 0.5F);
    ...
    public void shootMissile(EntityLivingBase entity, float distance) {
        ...
        if (canEntityBeSeen(entity)) {
            ...
            if (getType() == 1) ((EntityWebSling) missile).setType((byte) 0);
            if (getType() == 2) ((EntityWebSling) missile).setType((byte) 2);
    ...
    public void setType(byte isType) {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? getGolemHealth()
                : getGolemHealth() * ConfigHandler.INSTANCE.mobHealthMultipier);
        getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(
            ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? getAttackStrength()
                : getAttackStrength() * ConfigHandler.INSTANCE.mobAttackDamageMultiplier);
    }
