...
public class EntitySporeBall extends EntityThrowable {
    ...
    public void onUpdate() {
        super.onUpdate();
        if (ridingEntity == null) if (worldObj.isRemote) trailParticles(worldObj, posX, posY + 0.35D, posZ, rand);

        if (ridingEntity != null) {
            yOffset = -1.5F;
            if (worldObj.isRemote) confusionParticles(worldObj, posX, posY, posZ, rand);

            if (!worldObj.isRemote) if (ridingEntity instanceof EntityPlayer) {
                EntityPlayer player = (EntityPlayer) ridingEntity;
                ItemStack is = player.inventory.getCurrentItem();
                if (is != null) player.dropOneItem(true);
            }

            if (ticksExisted > 140) setDead();
        }
    }
    ...
    protected void onImpact(MovingObjectPosition mop) {
        ...
        if (mop.entityHit != null) {
            ...
            if (mop.entityHit instanceof EntityPlayer) {
                ...
                if (helm != null && helm.getItem() == ModItems.mushroomHelm) setDead();

                else if (!worldObj.isRemote) if (player.riddenByEntity == null) {
                    mountEntity(player);
                    ticksExisted = 0;
                }
            }
            ...
            if (mop.entityHit instanceof EntityLivingBase && !(mop.entityHit instanceof EntityPlayer)) {
                if (!worldObj.isRemote) {
                    ...
                    ((EntityLivingBase) mop.entityHit).attackEntityFrom(
                        DamageSource.causeMobDamage(getThrower()),
                        (float) (ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 1D
