...
public class EntityWasp extends EntityMob implements IEntityAdditionalSpawnData {
    ...
    public final EntityAINearestAttackableTarget aiNearestAttackableTarget = new EntityAINearestAttackableTarget(
        this,
        EntityPlayer.class,
        0,
        true);
    public final EntityErebusAIAttackOnCollide aiAttackOnCollide = new EntityErebusAIAttackOnCollide(
        this,
    ...
    protected void updateBossAttributes() {
        if (worldObj != null && !worldObj.isRemote) if (getIsBoss() == 1) {
            setSize(2.5F, 2F);
            experienceValue = 25;
            ...
            getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(
                ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 8D
                    : 8D * ConfigHandler.INSTANCE.mobAttackDamageMultiplier);
            ...
            getEntityAttribute(SharedMonsterAttributes.attackDamage).setBaseValue(
                ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 4D
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        int amount;
        for (amount = 0; amount < chance; ++amount) entityDropItem(ItemMaterials.DATA.WASP_STING.makeStack(), 0.0F);
        if (getIsBoss() == 1) entityDropItem(new ItemStack(ModItems.bottleAntiVenom), 0.0F);
    }
    ...
    public void onUpdate() {
        ...
        if (worldObj.isRemote) {
            ...
            if (i == 1) {
                setSize(2.5F, 2F);
                if (!hasCustomNameTag()) if (rand.nextBoolean()) setCustomNameTag("Livid's Bane");
                else setCustomNameTag("Hornet of Despair");
            } else setSize(1.5F, 1.0F);
        }
        ...
        if (!worldObj.isRemote) {
            if (getAttackTarget() == null) {
                if (rand.nextInt(200) == 0) if (!waspFlying) setWaspFlying(true);
                else setWaspFlying(false);

                if (waspFlying) flyAbout();
                else land();
            }

            if (getAttackTarget() != null) {
                currentFlightTarget = new ChunkCoordinates(
                    (int) getAttackTarget().posX,
                    (int) ((int) getAttackTarget().posY + getAttackTarget().getEyeHeight()),
        ...
        if (canEntityBeSeen(entity)) {
            if (super.attackEntityAsMob(entity)) {
                if (entity instanceof EntityLivingBase) {
                    ...
                    if (duration > 0) ((EntityLivingBase) entity)
                        .addPotionEffect(new PotionEffect(Potion.poison.id, duration * 20, 0));
                }
            }
            return true;
        } else return false;
