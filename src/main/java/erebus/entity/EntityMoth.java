...
public class EntityMoth extends EntityAmbientCreature {
    ...
    protected void updateAITasks() {
        ...
        if (getIsMothHanging()) {
            if (!worldObj.getBlock(MathHelper.floor_double(posX), (int) posY + 1, MathHelper.floor_double(posZ))
                .isNormalCube()) setIsMothHanging(false);
            else {
    ...
    public boolean attackEntityFrom(DamageSource source, float par2) {
        if (isEntityInvulnerable()) return false;
            ...
            if (!worldObj.isRemote && getIsMothHanging()) setIsMothHanging(false);
    ...
    protected void dropFewItems(boolean par1, int par2) {
        if (rand.nextInt(5) == 0) entityDropItem(new ItemStack(Items.glowstone_dust, 1, 0), 0.0F);
    }
}
