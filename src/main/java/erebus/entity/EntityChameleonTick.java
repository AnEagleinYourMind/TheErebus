...
public class EntityChameleonTick extends EntityMob {
    ...
    private final EntityAINearestAttackableTarget aiAttackTarget = new EntityAINearestAttackableTarget(
        this,
        EntityPlayer.class,
        0,
        true);
    private final EntityErebusAIAttackOnCollide aiAttackOnCollide = new EntityErebusAIAttackOnCollide(
        this,
        EntityPlayer.class,
        0.65D,
        false);
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        int amount;
        for (amount = 0; amount < chance; ++amount) entityDropItem(ItemMaterials.DATA.CAMO_POWDER.makeStack(), 0.0F);
    }

    @Override
    public void onUpdate() {
        super.onUpdate();
        Block newblockID = worldObj
            .getBlock(MathHelper.floor_double(posX), MathHelper.floor_double(posY) - 1, MathHelper.floor_double(posZ));
        int newBlockMeta = worldObj.getBlockMetadata(
            MathHelper.floor_double(posX),
            MathHelper.floor_double(posY) - 1,
            MathHelper.floor_double(posZ));

        if (onGround && newblockID != null
            && newblockID != blockID
        ...
        if (!worldObj.isRemote && animation == 9 && active) setAIs(true);

        if (!worldObj.isRemote && !active) {
            stationaryEntity();
            if (animation == 1) setAIs(false);
        }
    }
    ...
    public void stationaryEntity() {
        ...
        if (worldObj.getBlock(x, y, z) == null) posY -= 1;
    }
