...
public class EntityErebusLightningBolt extends EntityWeatherEffect {
    ...
    public void onUpdate() {
        ...
        if (lightningState < 0) if (boltLivingTime == 0) setDead();
        ...
        if (lightningState >= 0) if (worldObj.isRemote) worldObj.lastLightningBolt = 2;
        ...
        if (lightningState >= 0) if (worldObj.isRemote) worldObj.lastLightningBolt = 2;
            ...
            List<Entity> list = worldObj.getEntitiesWithinAABBExcludingEntity(
                this,
                AxisAlignedBB.getBoundingBox(posX - d0, posY - d0, posZ - d0, posX + d0, posY + 6.0D + d0, posZ + d0));
