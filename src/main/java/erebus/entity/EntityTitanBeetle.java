...
public class EntityTitanBeetle extends EntityTameable {
    ...
    public void onUpdate() {
        super.onUpdate();
        if (!worldObj.isRemote && isDead) for (ItemStack is : inventory)
            if (is != null) Utils.dropStack(worldObj, (int) posX, (int) posY, (int) posZ, is);
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        for (int a = 0; a < var3; ++a) entityDropItem(ItemMaterials.DATA.PLATE_EXO.makeStack(), 0.0F);

        entityDropItem(
            new ItemStack(
                ModItems.food,
                1 + rand.nextInt(1),
                isBurning() ? ItemErebusFood.FoodType.TITAN_CHOP_COOKED.ordinal()
                    : ItemErebusFood.FoodType.TITAN_CHOP_RAW.ordinal()),
            0.0F);
        dropChests();
    }
    ...
    public void dropChests() {
        if (!worldObj.isRemote) {
            if (getTameState() == 3) dropItem(Item.getItemFromBlock(Blocks.chest), 1);
            if (getTameState() == 4) dropItem(Item.getItemFromBlock(Blocks.ender_chest), 1);
        }
    }

    public void openGUI(EntityPlayer player) {
        if (!worldObj.isRemote && (riddenByEntity == null || riddenByEntity == player) && getTameState() != 0) {
            if (getTameState() == 3) player.displayGUIChest(new TileEntityTitanChest(this));
            if (getTameState() == 4) {
    ...
    public boolean attackEntityAsMob(Entity entity) {
        ...
        if (entity != null && getDistanceToEntity(entity) <= 2.5F
    ...
    public void readEntityFromNBT(NBTTagCompound data) {
        ...
        if (getTameState() == 3) {
            ...
            for (int i = 0; i < nbttaglist.tagCount(); i++) {
                ...
                if (b0 >= 0 && b0 < inventory.length) inventory[b0] = ItemStack.loadItemStackFromNBT(nbttagcompound1);
            }
        }
    }
}
