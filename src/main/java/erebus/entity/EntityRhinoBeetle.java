...
public class EntityRhinoBeetle extends EntityTameable {
    ...
    private final EntityAINearestAttackableTarget aiNearestAttackableTarget = new EntityAINearestAttackableTarget(
        this,
        EntityPlayer.class,
        0,
        true);
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 60D : 60D * ConfigHandler.INSTANCE.mobHealthMultipier);
    ...
    public boolean getCanSpawnHere() {
        ...
        if (light >= 0F) return worldObj.checkNoEntityCollision(boundingBox)
    ...
    protected boolean canDespawn() {
        if (getTameState() != 0) return false;
        else return true;
    }
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        if (getTameState() == 2) entityDropItem(ItemMaterials.DATA.RHINO_RIDING_KIT.makeStack(), 0.0F);
        int dropRate = 1 + rand.nextInt(2 + looting);
        for (int a = 0; a < dropRate; ++a) entityDropItem(ItemMaterials.DATA.PLATE_EXO_RHINO.makeStack(), 0.0F);
        if (rand.nextInt(20) == 0) entityDropItem(ItemMaterials.DATA.RHINO_BEETLE_HORN.makeStack(), 0.0F);
    }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (is != null && is.getItem() == ModItems.materials
            && is.getItemDamage() == ItemMaterials.DATA.BEETLE_TAMING_AMULET.ordinal()
            && getTameState() == 0) {
            healingBuff = (float) (ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 20D
                : 20D * ConfigHandler.INSTANCE.mobHealthMultipier);
            is.stackSize--;
            setTameState((byte) 1);
            playTameEffect(true);
            player.swingItem();
            ...
            getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
                ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 80D : 80D * ConfigHandler.INSTANCE.mobHealthMultipier);
            heal(healingBuff);
            return true;
        }
        if (is != null && is.getItem() == ModItems.materials
            && is.getItemDamage() == ItemMaterials.DATA.RHINO_RIDING_KIT.ordinal()
            && getTameState() == 1) {
            is.stackSize--;
            player.swingItem();
            setTameState((byte) 2);
        ...
        if (is != null && is.getItem() == ModItems.materials
            && is.getItemDamage() == ItemMaterials.DATA.BAMBOO_SHOOT.ordinal()
            && getTameState() != 0) {
            healingBuff = (float) (ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 5D
                : 5D * ConfigHandler.INSTANCE.mobHealthMultipier);
    ...
    protected void collideWithEntity(Entity entity) {
        if (riddenByEntity != null && entity instanceof EntityLivingBase
            && !(entity instanceof EntityPlayer)
            && !(entity instanceof EntityBotFlyLarva)
            && ramming) {
    ...
    public void setAttackTarget(EntityLivingBase entity) {
        if (getTameState() != 0) {
            if (entity instanceof EntityPlayer) super.setAttackTarget((EntityLivingBase) null);
        } else super.setAttackTarget(entity);
    }

    @Override
    public boolean attackEntityAsMob(Entity entity) {
        if (getTameState() != 0) if (entity instanceof EntityPlayer) {
            setAttackTarget((EntityLivingBase) null);
            return false;
        }
        return ram(
            entity,
            1F,
            (float) (ConfigHandler.INSTANCE.mobAttackDamageMultiplier < 2 ? 6D
                : 6D * ConfigHandler.INSTANCE.mobAttackDamageMultiplier));
    }

    private boolean ram(Entity entity, float knockback, float damage) {
        if (getTameState() == 0 || riddenByEntity == null) setRammingCharge((byte) 32);
        if (!worldObj.isRemote && entity.boundingBox.maxY >= boundingBox.minY
            && entity.boundingBox.minY <= boundingBox.maxY
            && entity.boundingBox.maxX >= boundingBox.minX - 0.25D
            && entity.boundingBox.minX <= boundingBox.maxX + 0.25D
            && entity.boundingBox.maxZ >= boundingBox.minZ - 0.25D
            && entity.boundingBox.minZ <= boundingBox.maxZ + 0.25D) {
            entity.attackEntityFrom(DamageSource.causeMobDamage(this), (int) damage);
            entity.addVelocity(
                -MathHelper.sin(rotationYaw * 3.141593F / 180.0F) * knockback,
                0.4D,
                MathHelper.cos(rotationYaw * 3.141593F / 180.0F) * knockback);
            worldObj.playSoundAtEntity(entity, "game.player.hurt.fall.big", 1.0F, 1.0F);
            ((EntityLivingBase) entity).addPotionEffect(
                new PotionEffect(Potion.moveSlowdown.id, worldObj.difficultySetting.ordinal() * 50, 0));
            setRamAttack(false);
        }
        return true;
    }
    ...
    public void onUpdate() {
        super.onUpdate();
        if (!ramming) if (getTameState() == 0 || riddenByEntity == null)
            if (worldObj.getWorldTime() % 10 == 0) setRammingCharge((byte) 0);
        if (shagCount > 0) shagCount--;

        if (worldObj.isRemote) if (ramming && !KeyBindingHandler.beetleRam.getIsKeyPressed())
    ...
    private void travelSpeed(float velocity) {
        if (!worldObj.isRemote) {
            if (velocity >= 4F) setRammingCharge((byte) (getRammingCharge() + 1));
            else if (velocity <= 4F) setRammingCharge((byte) (getRammingCharge() - 1));
            if (getRammingCharge() <= 0) setRammingCharge((byte) 0);
            if (getRammingCharge() >= 25) setRammingCharge((byte) 25);
        }
    }
    ...
    public boolean isBreedingItem(ItemStack is) {
        if (getTameState() != 0) return is != null && is.getItem() == ModItems.turnip;
        else return false;
    }
