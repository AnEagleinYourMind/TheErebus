...
public class EntityTarantulaMiniboss extends EntityMob implements IBossDisplayData {
    ...
    protected void applyEntityAttributes() {
        ...
        getEntityAttribute(SharedMonsterAttributes.maxHealth).setBaseValue(
            ConfigHandler.INSTANCE.mobHealthMultipier < 2 ? 300D : 300D * ConfigHandler.INSTANCE.mobHealthMultipier);
    ...
    protected void onDeathUpdate() {
        ...
        if (deathTicks == 200 && !worldObj.isRemote) {
            ...
            worldObj.setBlock(
                ...
                ModBlocks.tarantulaEgg);
            Utils.dropStackNoRandom(
                worldObj,
                MathHelper.floor_double(posX),
                MathHelper.floor_double(posY + 1.5),
                MathHelper.floor_double(posZ),
                new ItemStack(ModItems.spiderTShirt));
            setDead();
        }
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float damage) {
        if (getHealth() > getMaxHealth() / 2 && !(source instanceof EntityDamageSourceIndirect)) return false;
        if (getHealth() <= getMaxHealth() / 2 && source instanceof EntityDamageSourceIndirect) return false;
    ...
    public IEntityLivingData onSpawnWithEgg(IEntityLivingData entityLivingData) {
        ...
        if (entityLivingData1 == null) {
            ...
            if (entityLivingData1 instanceof EntitySpider.GroupData) {
                ...
                if (i > 0 && Potion.potionTypes[i] != null) addPotionEffect(new PotionEffect(i, Integer.MAX_VALUE));
            }
        }
    ...
    public void forceCollideWithPlayer(EntityLivingBase entity, float distance) {
        if (distance > 1.0F && distance < 8.0F) if (onGround) {
