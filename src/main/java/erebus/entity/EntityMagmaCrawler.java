...
public class EntityMagmaCrawler extends EntityMob {
    ...
    public boolean isOnCeiling() {
        return upAbove && worldObj
            .getBlock(MathHelper.floor_double(posX), MathHelper.floor_double(posY + 1.5), MathHelper.floor_double(posZ))
            == ModBlocks.gneiss
            && worldObj.isAirBlock(
    ...
    public boolean getCanSpawnHere() {
        return worldObj.checkNoEntityCollision(boundingBox) && worldObj.getCollidingBoundingBoxes(this, boundingBox)
            ...
            && worldObj
                .isAirBlock(MathHelper.floor_double(posX), MathHelper.floor_double(posY), MathHelper.floor_double(posZ))
            && worldObj.getBlock(
                MathHelper.floor_double(posX),
                MathHelper.floor_double(posY) + 1,
                MathHelper.floor_double(posZ)) == ModBlocks.gneiss;
    }
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            if (upAbove
                ...
                && worldObj.getBlock(
                    MathHelper.floor_double(posX),
                    MathHelper.floor_double(posY) + 2,
                    MathHelper.floor_double(posZ)) == ModBlocks.gneiss) {
                motionY += 0.1D;
                if (getEntityToAttack() != null && upAbove) {
                    double distance = getDistance(
                        getEntityToAttack().posX,
                        getEntityToAttack().boundingBox.maxY,
                        getEntityToAttack().posZ);
                    if (distance < 2.0D) {
                        motionY += 0D;
                        upAbove = false;
                    }
    ...
    public void onLivingUpdate() {
        if (!worldObj.isRemote && upAbove) if (getEntityToAttack() != null) {
            ...
            if (attackTime == 0) {
                ...
                if (shouldDo > 1) {
                    ...
                    EntitySmallFireball fireball = new EntitySmallFireball(
                        worldObj,
                        this,
                        distanceX + rand.nextGaussian() * targetAreaOffset,
                        distanceY,
                        distanceZ + rand.nextGaussian() * targetAreaOffset);
