...
public class EntityBlackAnt extends EntityTameable implements IInventory {
    ...
    private static final String[] names = { "Antwan", "George", "Geoff", "Alberto", "Jose", "Linda", "Chantelle",
        "Dave", "Basil", "Gertrude", "Herbert", "Russel", "Adam", "Gwen", "Billy Bob Joe Bob Joe Harrison Jr.", "Sid",
        "Dylan", "Jade" };
    ...
    public void setTamed(boolean tamed) {
        if (tamed) {
            ...
            if (!hasCustomNameTag()) setCustomNameTag(names[worldObj.rand.nextInt(names.length)]);
        } else dataWatcher.updateObject(16, Byte.valueOf((byte) 0));
    }

    public void openGUI(EntityPlayer player) {
        player
            .openGui(Erebus.instance, CommonProxy.GuiID.ANT_INVENTORY.ordinal(), player.worldObj, getEntityId(), 0, 0);
    }
    ...
    public boolean interact(EntityPlayer player) {
        ...
        if (is != null && is.getItem() == ModItems.antTamingAmulet
            && is.hasTagCompound()
    ...
    public void setBlockHarvested(Block block, int meta) {
        ...
        if (block != null) {
            Utils.dropStack(
                ...
                (int) posY,
                (int) posZ,
                new ItemStack(block.getItemDropped(meta, rand, 0), rand.nextInt(2) + 1));
            Utils.dropStack(
                worldObj,
                (int) posX,
                (int) posY,
                (int) posZ,
                new ItemStack(block.getItemDropped(0, rand, 0), rand.nextInt(2) + 1));
        }
    }
    ...
    protected void dropEquipment(boolean rencentHit, int fortune) {
        for (ItemStack stack : inventory) if (stack != null) entityDropItem(stack, 0.0F);
    }
    ...
    public void onLivingUpdate() {
        ...
        // not full
        if (isTamed()) {
            if (canPickupItems && !isFilterSlotEmpty()
                && (getAntInvSlotStack() == null
                    || getAntInvSlotStack().stackSize < getAntInvSlotStack().getMaxStackSize())) {
            ...
            if (!isTaskSlotEmpty() && getTaskSlotStack().getItem() instanceof ItemHoe
                || !isTaskSlotEmpty() && getTaskSlotStack().getItem() == Items.bone)
                if (isAntInvSlotEmpty() && !isFilterSlotEmpty()) canCollectFromSilo = true; // this stops the planting
                                                                                            // or
    ...
    private void getStackFromSilo() {
        ...
        for (int i = 0; i < siloInventory.length; i++)
            if (siloTile.getStackInSlot(i) != null) if (siloTile.getStackInSlot(i)
                .getItem() == getStackInSlot(CROP_ID_SLOT).getItem()
                && siloTile.getStackInSlot(i)
                    .getItemDamage() == getStackInSlot(CROP_ID_SLOT).getItemDamage())
    ...
    private void addDropToInventory(int x, int y, int z) {
        ...
        if (!isAntInvSlotEmpty()) Utils.addItemStackToInventory(
        ...
        Collections.sort(list, new Comparator<EntityItem>() {
            ...
            public int compare(EntityItem e1, EntityItem e2) {
                return Double.compare(
                    e1.getDistanceSq(entity.posX, entity.posY, entity.posZ),
                    e2.getDistanceSq(entity.posX, entity.posY, entity.posZ));
            }
    ...
    public void moveToSilo() {
        PathEntity pathentity = worldObj.getEntityPathToXYZ(
            this,
            getDropPointX(),
            getDropPointY() - 1,
            getDropPointZ(),
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        if (slot == TOOL_SLOT) return stack.getItem() == Items.shears || stack.getItem() == Items.bucket
    ...
    public void readEntityFromNBT(NBTTagCompound nbt) {
        ...
        if (nbt.getByte("tamed") == 1) setTamed(true);
        else setTamed(false);
        ...
        for (int i = 0; i < tags.tagCount(); i++) {
            ...
            if (j >= 0 && j < inventory.length) inventory[j] = ItemStack.loadItemStackFromNBT(data);
        }
    }
    ...
    public void writeEntityToNBT(NBTTagCompound nbt) {
        ...
        if (isTamed()) nbt.setByte("tamed", Byte.valueOf((byte) 1));
        else nbt.setByte("tamed", Byte.valueOf((byte) 0));
