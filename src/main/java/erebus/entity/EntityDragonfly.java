...
public class EntityDragonfly extends EntityMob {
    ...
    public void onUpdate() {
        ...
        if (riddenByEntity != null) if (!worldObj.isRemote && captured()
            && (posY > pickupHeight + 10D || countDown <= 0
                || !worldObj.isRemote && captured()
                    && worldObj.isSideSolid(
                        MathHelper.floor_double(posX),
                        MathHelper.floor_double(posY + 1D),
        ...
        if (worldObj.isRemote) if (getSkin() == 0) {
            spawnParticles(worldObj, posX - 0.5D, posY, posZ - 0.5D, rand);
            if (!hasCustomNameTag()) setCustomNameTag("Ender Dragonfly");
        }
    }
    ...
    public void flyAbout() {
        ...
        if (currentFlightTarget != null
            ...
            && (worldObj.isAirBlock(currentFlightTarget.posX, currentFlightTarget.posY - 3, currentFlightTarget.posZ)
        ...
        if (currentFlightTarget == null || rand.nextInt(30) == 0
            ...
            currentFlightTarget = new ChunkCoordinates(
                (int) posX + rand.nextInt(10) - rand.nextInt(10),
                (int) posY + rand.nextInt(4) - rand.nextInt(4),
                (int) posZ + rand.nextInt(10) - rand.nextInt(10));
        if (currentFlightTarget != null && getEntityToAttack() != null
            && !worldObj.isAirBlock(currentFlightTarget.posX, currentFlightTarget.posY - 3, currentFlightTarget.posZ)
            || currentFlightTarget != null && !getDropped() && currentFlightTarget.posY < pickupHeight + 10D)
    ...
    public boolean attackEntityFrom(DamageSource source, float amp) {
        if (isEntityInvulnerable()) return false;
            ...
            if (riddenByEntity != entity && ridingEntity != entity) {
                if (entity != this) entityToAttack = (EntityLivingBase) entity;
                return true;
            } else return true;
        } else return false;
    }
    ...
    protected void dropFewItems(boolean recentlyHit, int looting) {
        ...
        if (getSkin() == 0) entityDropItem(new ItemStack(Items.ender_pearl, rand.nextInt(1) + 1 + looting), 0.0F);
    }
