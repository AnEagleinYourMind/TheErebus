...
public class EntityThrownSand extends EntityThrowable {
    ...
    protected void onImpact(MovingObjectPosition movingObjectPosition) {
        ...
        if (movingObjectPosition.entityHit != null) {
            if (!worldObj.isRemote) if (movingObjectPosition.entityHit instanceof EntityLivingBase) {
                ((EntityLivingBase) movingObjectPosition.entityHit)
                    .addPotionEffect(new PotionEffect(Potion.blindness.id, 2 * 20, 0));
    ...
    public void onUpdate() {
        ...
        if (rotationticks < 360F) {
            ...
            if (rotationticks >= 360F) rotationticks = 0;
        }
    }
}
