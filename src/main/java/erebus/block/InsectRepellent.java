...
public class InsectRepellent extends Block {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        for (int l = 0; l < 6; ++l) {
            ...
            if (l == 0 && !world.getBlock(x, y + 1, z)
                .isOpaqueCube()) particleY = y + 1 + d0;

            if (l == 1 && !world.getBlock(x, y - 1, z)
                .isOpaqueCube()) particleY = y - d0;

            if (l == 2 && !world.getBlock(x, y, z + 1)
                .isOpaqueCube()) particleZ = z + 1 + d0;

            if (l == 3 && !world.getBlock(x, y, z - 1)
                .isOpaqueCube()) particleZ = z - d0;

            if (l == 4 && !world.getBlock(x + 1, y, z)
                .isOpaqueCube()) particleX = x + 1 + d0;

            if (l == 5 && !world.getBlock(x - 1, y, z)
                .isOpaqueCube()) particleX = x - d0;
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (!world.isRemote && entity instanceof EntityLiving)
            if (entity.worldObj.getBlock(x, y, z) == ModBlocks.insectRepellent
                && ((EntityLiving) entity).getCreatureAttribute()
                    .equals(EnumCreatureAttribute.ARTHROPOD)) {
                        int Knockback = 1;
                        entity.addVelocity(
