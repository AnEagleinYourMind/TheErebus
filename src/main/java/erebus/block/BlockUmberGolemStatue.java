...
public class BlockUmberGolemStatue extends BlockContainer {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (player.getCurrentEquippedItem() != null) if (player.getCurrentEquippedItem()
            .getItem() == ModItems.wandOfAnimation) {
                ...
                if (!world.isRemote) {
                    ...
                    entityUmberGolem.setPositionAndRotation(
                        x + 0.5D,
                        y,
                        z + 0.5D,
                        MathHelper.wrapAngleTo180_float(player.rotationYaw * 360.0F),
                        0.0F);
