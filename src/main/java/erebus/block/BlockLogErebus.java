...
public class BlockLogErebus extends BlockLog {
    ...
    public String getLocalizedName() {
        return StatCollector
            .translateToLocal("tile." + Reference.MOD_ID + ".log_" + wood.getUnlocalisedName() + ".name");
    }
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        iconSide = iconRegister.registerIcon(
            "erebus:log_" + wood.name()
                .toLowerCase());
        iconTop = iconRegister.registerIcon(
            "erebus:log_" + wood.name()
                .toLowerCase() + "_top");
    }
    ...
    public void onBlockDestroyedByPlayer(World world, int x, int y, int z, int meta) {
        if (!world.isRemote && wood.name() == "Rotten") if (world.rand.nextInt(30) == 0) {
