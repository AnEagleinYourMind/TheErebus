...
public class BlockBambooShoot extends BlockFlower implements IGrowable {
    ...
    public static byte calculateBambooHappiness(World world, int x, int y, int z, Block block) {
        ...
        int perfectWaterAmount = (int) Math
            .floor(35D * (Math.pow(biome.temperature + 1D, 2) / 4D) * (biome.canSpawnLightningBolt() ? 1D : 1.75D));
        double waterFound = 0D;

        for (int a = 0, xx, zz; a < 150; a++) if (world
            .getBlock(
                xx = x + world.rand.nextInt(10) - 5,
                bottomY + world.rand.nextInt(4) - 2,
                zz = z + world.rand.nextInt(10) - 5)
            .getMaterial() == Material.water) waterFound += 8D - Math.sqrt(Math.pow(xx - x, 2) + Math.pow(zz - z, 2));
        ...
        if (!world.isRemote) {
            ...
            if (rand.nextInt(5) <= 1 && happiness > rand.nextInt(35)) {
                 ...
                 */world.setBlock(
                    ...
                    y,
                    z,
                    ModBlocks.bambooCrop,
                    (meta >= 8 ? 8 : 0) + (int) Math.min(
                        7D,
                        2D + happiness / 7D
                            + (meta >= 8 ? 0.5D : 0D)
                            + Math.abs((happiness / 10D + (meta >= 8 ? 0.6D : 0D)) * rand.nextGaussian())),
                    3);
            }
        }
    }
    ...
    public void func_149853_b(World world, Random rand, int x, int y, int z) {
        int meta = world.getBlockMetadata(x, y, z);
        if (meta < 7) world.setBlockMetadataWithNotify(x, y, z, meta + 9, 4);
    }
}
