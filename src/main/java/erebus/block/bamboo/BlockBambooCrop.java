...
public class BlockBambooCrop extends Block {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if ((meta & 7) != 0)
            if (BlockBambooShoot.calculateBambooHappiness(world, x, y, z, this) > rand.nextInt(110 - meta * 2)
                && world.isAirBlock(x, y + 1, z)) world.setBlock(x, y + 1, z, this, meta - 1, 3);
    }
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        return world.getBlock(x, y - 1, z) == this
            || block != null && block.isSideSolid(world, x, y, z, ForgeDirection.UP);
    }
    ...
    public boolean canPlaceBlockOnSide(World world, int x, int y, int z, int side) {
        ...
        if (!(world.getBlock(x, y - 1, z) == this && world.getBlockMetadata(x, y - 1, z) == 0
            || block != null && block.isSideSolid(world, x, y, z, ForgeDirection.UP))) return false;
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLivingBase, ItemStack is) {
        Block block = world.getBlock(x, y - 1, z);
        if (!(world.getBlock(x, y - 1, z) == this && world.getBlockMetadata(x, y - 1, z) == 0
            || block != null && block.isSideSolid(world, x, y, z, ForgeDirection.UP))) world.setBlockToAir(x, y, z);
    }
}
