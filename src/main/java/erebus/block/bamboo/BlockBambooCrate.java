...
public class BlockBambooCrate extends BlockContainer {
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        if (isCrate(world, x, y, z)) {
            // BOTTOM
            if (isCrate(world, x, y + 1, z) && isCrate(world, x + 1, y + 1, z)
                && isCrate(world, x + 1, y + 1, z + 1)
                && isCrate(world, x, y + 1, z + 1))
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block neighbour) {
        if (neighbour == this) for (int i = -1; i <= 1; i++) for (int j = -1; j <= 1; j++) for (int k = -1; k <= 1; k++)
            if (world.getBlock(x + i, y + k, z + j) == this) onBlockAdded(world, x + i, y + k, z + j);
    }
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS)
            if (world.getBlock(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ) == this) {
                ...
                if (world.getBlock(
                    ...
                    z + dir.getOpposite().offsetZ) == this) return false;
            }
        return true;
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (tileCrate != null) if (world.getBlockMetadata(x, y, z) == 0) {
            ItemStack current = player.inventory.getCurrentItem();
            if (current != null && current.getItem() == Item.getItemFromBlock(this)) return false;
            player.openGui(Erebus.instance, CommonProxy.GuiID.BAMBOO_CRATE.ordinal(), world, x, y, z);
        } else for (int i = -1; i <= 1; i++) for (int j = -1; j <= 1; j++) for (int k = -1; k <= 1; k++)
            if (world.getBlock(x + i, y + k, z + j) == this) if (world.getBlockMetadata(x + i, y + k, z + j) == 1) {
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
        ...
        if (tile != null) for (int i = 0; i < tile.getSizeInventory(); i++) {
