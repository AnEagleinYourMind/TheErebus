...
public class BlockBambooBridge extends BlockContainer {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityLivingBase, ItemStack is) {
        ...
        if (rotation == 0) meta = 2;
        if (rotation == 1) meta = 5;
        if (rotation == 2) meta = 3;
        if (rotation == 3) meta = 4;
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        ...
        switch (world.getBlockMetadata(x, y, z)) {
            case 2:
                if (!right) te.setRenderSide1(true);
                if (!left) te.setRenderSide2(true);
                if (right) te.setRenderSide1(false);
                if (left) te.setRenderSide2(false);
                break;
            case 3:
                if (!right) te.setRenderSide2(true);
                if (!left) te.setRenderSide1(true);
                if (right) te.setRenderSide2(false);
                if (left) te.setRenderSide1(false);
                break;
            case 4:
                if (!back) te.setRenderSide1(true);
                if (!front) te.setRenderSide2(true);
                if (back) te.setRenderSide1(false);
                if (front) te.setRenderSide2(false);
                break;
            case 5:
                if (!back) te.setRenderSide2(true);
                if (!front) te.setRenderSide1(true);
                if (back) te.setRenderSide2(false);
                if (front) te.setRenderSide1(false);
                break;
        }
    }
    ...
    public boolean canConnectBridgeTo(IBlockAccess world, int x, int y, int z) {
        ...
        if (block != this)
            return !block.isAir(world, x, y, z) && block.renderAsNormalBlock() ? block.getMaterial() != Material.gourd
                : false;
        else return true;
    }
}
