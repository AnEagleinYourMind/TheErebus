...
public class JarOHoney extends BlockGlowingJar {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase player, ItemStack stack) {
        ...
        if (tile != null)
            if (player instanceof EntityPlayer) tile.setOwner(((EntityPlayer) player).getCommandSenderName());
            else tile.setOwner("Boo Boo");
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (FluidContainerRegistry.isFilledContainer(stack)) {
            ...
            if (fluidStack.isFluidEqual(tile.getHoney()))
                if (fluidStack.amount + tile.getHoney().amount > TileEntityJarOHoney.HONEY_MAX_AMOUNT) return true;
                else {
