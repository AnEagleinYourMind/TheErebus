...
public class BlockPlanksErebus extends Block implements IHasCustomItemBlock {
    ...
    public void getSubBlocks(Item item, CreativeTabs tab, List list) {
        for (int i = 0; i < EnumWood.values().length; i++)
            if (EnumWood.values()[i].hasPlanks()) list.add(new ItemStack(item, 1, i));
    }
    ...
    public void registerBlockIcons(IIconRegister reg) {
        ...
        for (int i = 0; i < icons.length; i++) {
            ...
            if (wood.hasPlanks()) {
                String name = "erebus:planks_" + EnumWood.values()[i].name()
                    .toLowerCase();
                if (ConfigHandler.INSTANCE.alternativePlanks && wood != EnumWood.Bamboo) name += "_alt";
