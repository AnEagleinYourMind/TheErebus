    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        ...
        if (entity instanceof EntityItem && entity.boundingBox.minY >= y + offsetY && te.active) {
            ...
            if (is.isItemStackDamageable() && repairDamage > 0) {
                if (te.notUsed) te.setSpawnTicks(160);
                if (te.getSpawnTicks() == 60 && te.getCollisions() == 101) {
                    ...
                    is.getItem()
                        .setDamage(is, -repairDamage);
                }
                if (te.getSpawnTicks() % 2 == 0 && te.getCollisions() < 101)
                    if (world.isRemote) te.sparky(world, x, y, z);
            }
            if (te.getCollisions() > 101) te.setSpawnTicks(0);
        }
    }
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (player.getCurrentEquippedItem() != null) {
            if (player.getCurrentEquippedItem()
                .getItem() == ModItems.wandOfAnimation && !te.active) {
