...
public class BlockPlantedGiantFlower extends BlockSapling implements IHasCustomItemBlock {
    ...
    public void func_149878_d(World world, int x, int y, int z, Random rand) {
        ...
        if (meta >= 0 && meta <= 13) worldGen.setFlowerColor(meta);
        world.setBlockToAir(x, y, z);
        if (!worldGen.generate(world, rand, x, y, z)) world.setBlock(x, y, z, ModBlocks.flowerPlanted, meta, 3);
    }
    ...
    public void getSubBlocks(Item id, CreativeTabs creativeTabs, List list) {
        for (int i = 0; i < FLOWER_TYPE.values().length; i++) list.add(new ItemStack(id, 1, i));
    }
