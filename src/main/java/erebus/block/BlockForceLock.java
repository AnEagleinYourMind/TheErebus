...
public class BlockForceLock extends Block {
    ...
    public IIcon getIcon(int side, int meta) {
        return meta == 0 && side == 4 ? front
            : meta == 1 && side == 2 ? front
                : meta == 2 && side == 5 ? front : meta == 3 && side == 3 ? front : blockIcon;
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess world, int x, int y, int z, int side) {
        return world.getBlock(x, y, z) == ModBlocks.forceField ? false
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (stack != null && stack.getItem() == ModItems.materials
            && stack.getItemDamage() == ItemMaterials.DATA.FORCE_KEY.ordinal()) {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (rand.nextInt(5) == 0) for (int l = 0; l < 6; ++l) {
    ...
    public AxisAlignedBB getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        return AxisAlignedBB
            .getBoundingBox(x + twoPixels, y, z + twoPixels, x + 1 - twoPixels, y + 1, z + 1 - twoPixels);
    }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (entity instanceof EntityLivingBase) {
            ...
            entity.addVelocity(
                MathHelper.sin(entity.rotationYaw * 3.141593F / 180.0F) * Knockback * 0.1F,
                0.08D,
                -MathHelper.cos(entity.rotationYaw * 3.141593F / 180.0F) * Knockback * 0.1F);
