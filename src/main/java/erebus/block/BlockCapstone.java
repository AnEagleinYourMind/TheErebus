...
public class BlockCapstone extends Block {
    ...
    private IIcon capstoneMud, capstoneMud1, capstoneIron, capstoneIron1, capstoneGold, capstoneGold1, capstoneJade,
        capstoneJade1;
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block neighbour) {
        int meta = world.getBlockMetadata(x, y, z);
        if (meta == 5) if (world.getBlockMetadata(x + 1, y, z) == 6) if (world.getBlockMetadata(x, y, z + 1) == 7)
            if (world.getBlockMetadata(x + 1, y, z + 1) == 8) openPyramid(world, x, y, z, 1, 1);
        if (meta == 6) if (world.getBlockMetadata(x - 1, y, z) == 5) if (world.getBlockMetadata(x, y, z + 1) == 8)
            if (world.getBlockMetadata(x - 1, y, z + 1) == 7) openPyramid(world, x, y, z, -1, 1);

        if (meta == 7) if (world.getBlockMetadata(x + 1, y, z) == 8) if (world.getBlockMetadata(x, y, z - 1) == 5)
            if (world.getBlockMetadata(x + 1, y, z - 1) == 6) openPyramid(world, x, y, z, 1, -1);

        if (meta == 8) if (world.getBlockMetadata(x - 1, y, z) == 7) if (world.getBlockMetadata(x, y, z - 1) == 6)
            if (world.getBlockMetadata(x - 1, y, z - 1) == 5) openPyramid(world, x, y, z, -1, -1);
    }
    ...
    private void openPyramid(World world, int x, int y, int z, int offsetX, int offsetZ) {
        ...
        world
            .playAuxSFXAtEntity(null, 2001, x + offsetX, y, z, Block.getIdFromBlock(world.getBlock(x + offsetX, y, z)));
        world.setBlockToAir(x + offsetX, y, z);
        world
            .playAuxSFXAtEntity(null, 2001, x, y, z + offsetZ, Block.getIdFromBlock(world.getBlock(x, y, z + offsetZ)));
        ...
        world.playAuxSFXAtEntity(
            ...
            y,
            z + offsetZ,
            Block.getIdFromBlock(world.getBlock(x + offsetX, y, z + offsetZ)));
