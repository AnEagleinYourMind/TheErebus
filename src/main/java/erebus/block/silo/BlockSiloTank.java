...
public class BlockSiloTank extends BlockContainer {
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        if (isSiloComplete(world, x, y, z)) {
            ...
            if (tile instanceof TileEntitySiloTank) {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (tile instanceof TileEntitySiloTank) {
            ...
            if (current != null && current.getItem() == Item.getItemFromBlock(ModBlocks.siloRoof)) return false;
            if (isSiloComplete(world, x, y, z)) {
    ...
    public boolean isSiloComplete(World world, int x, int y, int z) {
        return world.getBlock(x, y - 1, z) == ModBlocks.siloSupports
            && world.getBlock(x, y + 1, z) == ModBlocks.siloRoof;
    }
