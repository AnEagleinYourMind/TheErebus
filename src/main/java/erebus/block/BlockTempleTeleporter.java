...
public class BlockTempleTeleporter extends BlockContainer {
    ...
    public static final String[] iconPaths = new String[] { "templeTeleport1", "templeTeleport2", "templeTeleport3",
        "templeTeleport4", "templeTeleport5", "templeTeleport6", "templeTeleportNE", "templeTeleportNW",
        "templeTeleportSE", "templeTeleportSW", "templeBrick" };
    ...
    public void onEntityWalking(World world, int x, int y, int z, Entity entity) {
        ...
        if (!world.isRemote) if (meta >= 4 && meta <= 9) if (entity instanceof EntityLivingBase && tile != null) {
            ((EntityLivingBase) entity)
                .setPositionAndUpdate(tile.getTargetX() + 0.5D, tile.getTargetY() + 1D, tile.getTargetZ() + 0.5D);
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        for (int l = 0; l < 6; ++l) {
            ...
            if (l == 0 && !world.getBlock(x, y + 1, z)
                .isOpaqueCube()) particleY = y + 1 + pixel;

            if (l == 1 && !world.getBlock(x, y - 1, z)
                .isOpaqueCube()) particleY = y - pixel;

            if (l == 2 && !world.getBlock(x, y, z + 1)
                .isOpaqueCube()) particleZ = z + 1 + pixel;

            if (l == 3 && !world.getBlock(x, y, z - 1)
                .isOpaqueCube()) particleZ = z - pixel;

            if (l == 4 && !world.getBlock(x + 1, y, z)
                .isOpaqueCube()) particleX = x + 1 + pixel;

            if (l == 5 && !world.getBlock(x - 1, y, z)
                .isOpaqueCube()) particleX = x - pixel;

            if (particleX < x || particleX > x + 1
                || particleY < 0.0D
                || particleY > y + 1
                || particleZ < z
                || particleZ > z + 1) {
