...
public class ErebusPortal extends Block {
    ...
    public static boolean obeysPortalRule(World world, int x, int y, int z, boolean actualPortal) {
        ...
        if (axisFlag == 0x7) return false; // 0b111, meaning there's a nieghbor on 3 sides, therefore not defining a
                                           // plane
        return true;
    }
    ...
    public void onNeighborBlockChange(World w, int x, int y, int z, Block neighbor) {
        if (!obeysPortalRule(w, x, y, z, true)) w.setBlockToAir(x, y, z);
    }
    ...
    public void onEntityCollidedWithBlock(World world, int i, int j, int k, Entity entity) {
        if (entity.ridingEntity == null && entity.riddenByEntity == null && entity.timeUntilPortal <= 0) {
            if (entity.dimension == 0 || entity.dimension == 1) TeleporterHandler.transferToErebus(entity);
            else TeleporterHandler.transferToOverworld(entity);
            if (entity != null) entity.timeUntilPortal = ConfigHandler.INSTANCE.portalCooldown * 20;
        }
    }
