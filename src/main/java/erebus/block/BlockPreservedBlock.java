...
public class BlockPreservedBlock extends BlockContainer {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase placer, ItemStack stack) {
        if (!world.isRemote && stack.hasTagCompound()) {
            ...
            if (tile != null) tile.setEntityNBT(
                stack.getTagCompound()
                    .getCompoundTag("EntityNBT"));
        }

        int rotation = MathHelper.floor_double(placer.rotationYaw * 4.0F / 360.0F + 0.5D) & 3;
        if (rotation == 0) world.setBlockMetadataWithNotify(x, y, z, 2 + 4 * stack.getItemDamage(), 2);
        else if (rotation == 1) world.setBlockMetadataWithNotify(x, y, z, 5 + 4 * stack.getItemDamage(), 2);
        else if (rotation == 2) world.setBlockMetadataWithNotify(x, y, z, 3 + 4 * stack.getItemDamage(), 2);
        else if (rotation == 3) world.setBlockMetadataWithNotify(x, y, z, 4 + 4 * stack.getItemDamage(), 2);
    }
    ...
    public void onBreakEvent(BlockEvent.BreakEvent event) {
        ...
        TileEntityPreservedBlock tile = Utils.getTileEntity(world, x, y, z, TileEntityPreservedBlock.class);
        if (tile != null) if (EnchantmentHelper.getSilkTouchModifier(player)) {
            ItemStack stack = new ItemStack(this, 1, world.getBlockMetadata(x, y, z) > 5 ? 1 : 0);
            NBTTagCompound nbt = new NBTTagCompound();
            nbt.setTag("EntityNBT", tile.getEntityNBT());
            stack.setTagCompound(nbt);
            this.dropBlockAsItem(world, x, y, z, stack);
        } else tile.spawnTrappedEntity();
    }
}
