...
public class BlockPricklyPair extends Block implements IPlantable {
            ...
            if (world.isAirBlock(x, y + 1, z)) {
                ...
                if (height < 3) {
                    ...
                    if (meta == 10) {
                        ...
                        else world.setBlockMetadataWithNotify(x, y, z, 0, 4);
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
        if (entity instanceof EntityLivingBase) entity.attackEntityFrom(DamageSource.cactus, 1);
    }

    @Override
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ArrayList<ItemStack> drops = new ArrayList<>();
        if (meta <= 10) drops.add(new ItemStack(Item.getItemFromBlock(this)));
        else drops.add(new ItemStack(ModItems.food, 1, ItemErebusFood.FoodType.PRICKLY_PAIR_RAW.ordinal()));
        return drops;
    }
