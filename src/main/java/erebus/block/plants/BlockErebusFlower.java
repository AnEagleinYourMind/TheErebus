...
public class BlockErebusFlower extends Block implements IHasCustomItemBlock {
    ...
    public void onBlockHarvested(World world, int x, int y, int z, int meta, EntityPlayer player) {
        if (meta == FLOWER_TYPE.EXPLODING_STIGMA.ordinal()) world.createExplosion(
            player,
            x,
            y,
            z,
    ...
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        if (meta == FLOWER_TYPE.EXPLODING_STIGMA.ordinal()) ret.add(new ItemStack(Items.gunpowder));
        else ret.add(new ItemStack(this, 1, meta));
    ...
    public void registerBlockIcons(IIconRegister reg) {
        icons = new IIcon[3];
        for (int i = 0; i < icons.length; i++) icons[i] = reg.registerIcon("erebus:erebusFlower" + i);
    }
