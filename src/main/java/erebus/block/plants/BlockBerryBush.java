...
public class BlockBerryBush extends Block {
    ...
    public String getLocalizedName() {
        return String
            .format(StatCollector.translateToLocal("tile." + Reference.MOD_ID + "." + type + "BerryBush.name"));
    }

    @Override
    @SideOnly(Side.CLIENT)
    public IIcon getIcon(int side, int meta) {
        if (meta > 2) return Minecraft.getMinecraft().gameSettings.fancyGraphics ? fruitIcon : fastFruitIcon;
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if (rand.nextInt(25) == 0) switch (meta) {
            case 0:
        ...
        if (meta == 2 && rand.nextInt(50) == 0) world.setBlock(x, y, z, this, 3, 2);
    }
    ...
    public void onBlockHarvested(World world, int x, int y, int z, int id, EntityPlayer player) {
        int meta = world.getBlockMetadata(x, y, z);
        if (meta == 3) Utils.dropStack(world, (int) (x + 0.5D), (int) (y + 0.5D), (int) (z + 0.5D), getBerry());
    }
    ...
    private boolean isValidBlock(Block block) {
        return block == ModBlocks.mud || block == Blocks.farmland;
    }
    ...
    public ItemStack getBerry() {
        ItemStack item = null;
        if (type == "jade") item = new ItemStack(ModItems.materials, 1, DATA.JADE_BERRIES.ordinal());
        if (type == "heart") item = new ItemStack(ModItems.heartBerries, 1);
        if (type == "swamp") item = new ItemStack(ModItems.food, 1, FoodType.SWAMPBERRIES.ordinal());
        return item;
    }
}
