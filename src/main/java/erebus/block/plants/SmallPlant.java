...
public class SmallPlant extends BlockTallGrass implements IHasCustomItemBlock {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if (rand.nextInt(25) == 0) {
            ...
            if (world.isAirBlock(xx, yy, zz) && canBlockStay(world, xx, yy, zz)) {
                if ("nettle".equals(name) && rand.nextInt(3) == 0) world.setBlock(x, y, z, ModBlocks.nettleFlowered);
                if ("nettleFlowered".equals(name)) world.setBlock(xx, yy, zz, ModBlocks.nettle);
            }
        }
    }
    ...
    public int colorMultiplier(IBlockAccess world, int x, int y, int z) {
        return colour ? world.getBiomeGenForCoords(x, z)
            .getBiomeGrassColor(x, y, z) : 0xFFFFFF;
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        if ("fireBloom".equals(name)) world.spawnParticle("flame", x + 0.5F, y + 1F, z + 0.5F, 0.0D, 0.0D, 0.0D);
    }
