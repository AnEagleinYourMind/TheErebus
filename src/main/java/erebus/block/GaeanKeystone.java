...
public class GaeanKeystone extends BlockContainer {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (held == null || held.getItem() != ModItems.portalActivator) return false;
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
        ...
        if (meta > 0) {
            ...
            world.spawnEntityInWorld(
                new EntityItem(world, x + 0.5, y + 0.5, z + 0.5, new ItemStack(ModItems.portalActivator)));
        }
    }
    ...
    static class PCoord {
        ...
        static void iterateCube(PCoord min, PCoord max, ICoordFunc func) {
            for (int x = min.x; x <= max.x; x++) for (int z = min.z; z <= max.z; z++)
                for (int y = min.y; y <= max.y; y++) if (func.visit(new PCoord(min.w, x, y, z))) return;
        }
        ...
        public void ensureFloored() {
            ...
            if (b.isReplaceable(w, x, y, z)) setBlock(ModBlocks.umberstone);
        }
    ...
    void breakPortal(World w, int x, int y, int z) {
        ...
        PCoord.iterateCube(min, max, new PCoord.ICoordFunc() {
            ...
            public boolean visit(PCoord at) {
                if (!at.is(ModBlocks.portal)) return false;
                ...
                while (!frontier.isEmpty()) {
                    ...
                    for (PCoord pc : f.neighbors()) {
                        ...
                        if (pc.is(ModBlocks.portal)) frontier.add(pc);
                    }
                }
                for (PCoord pc : found) pc.setBlockNoNotify(Blocks.air);
                return true;
            }
        });
    }
    ...
    boolean makePortal(World w, int x, int y, int z) {
        ...
        PCoord.iterateCube(min, max, new PCoord.ICoordFunc() {
            ...
            public boolean visit(PCoord at) {
                ...
                if (visitLeaves(at, contig, badLeaves) && contig.size() < MAX_PORTAL_SIZE) return true;
        ...
        for (PCoord at : contig) at.setBlockNoNotify(ModBlocks.portal);
        return true;
    }
    ...
    static final byte[] portalFrame = new byte[] { 0, F, F, F, 0, END, F, L, L, L, F, END, F, L, L, L, F, END, F, L, L,
        L, F, END, 0, F, F, F, 0, END, };

    public void buildDestinationPortal(World w, int x, int y, int z) {
        PCoord keystone = new PCoord(w, x, y, z);
        while (keystone.isAir() && keystone.y > 0) keystone = keystone.add(0, -1, 0);
        ...
        PCoord.iterateCube(floorMin, floorMax, new PCoord.ICoordFunc() {
            ...
            public boolean visit(PCoord at) {
                ...
                int yMin = at.y + 1;
                int yMax = at.y + C;
                for (int y = yMin; y < yMax; y++) at.w.setBlockToAir(at.x, y, at.z);
                return false;
            }
        });
        ...
        for (byte b : portalFrame) {
            if (b == END) {
                ...
                Block block;
                int md = 0;
                if (b == L) if (w.difficultySetting != EnumDifficulty.HARD) block = Blocks.leaves;
