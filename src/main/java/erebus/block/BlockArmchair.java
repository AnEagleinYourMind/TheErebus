...
public class BlockArmchair extends BlockContainer {
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (player.isSneaking()) {
            ...
            if (player.dimension == ConfigHandler.INSTANCE.erebusDimensionID
                && ConfigHandler.INSTANCE.allowRespawning) {
                if (!world.isAirBlock(x, y + 1, z) || !world.isAirBlock(x, y + 2, z))
                    ...
                    player.getEntityData()
                        .setInteger("armchairX", x);
                    player.getEntityData()
                        .setInteger("armchairY", y);
                    player.getEntityData()
                        .setInteger("armchairZ", z);
                    player.getEntityData()
    ...
    public void breakBlock(World world, int x, int y, int z, Block block, int meta) {
        if (!world.isRemote && entityMountableBlock != null) entityMountableBlock.setDead();
    }
