...
public class BlockRedGem extends Block implements IHasCustomItemBlock {
    ...
    public int quantityDropped(int meta, int fortune, Random random) {
        if (meta == 0) return 1 + random.nextInt(2 + fortune);
        return 1;
    }
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        ...
        if (!world.isRemote && (meta == 1 || meta == 2))
            ...
            else if (meta != 2 && world.isBlockIndirectlyGettingPowered(x, y, z)) world.setBlock(x, y, z, this, 2, 2);
    }
    ...
    public boolean isSideSolid(IBlockAccess world, int x, int y, int z, ForgeDirection side) {
        int meta = world.getBlockMetadata(x, y, z);
        if (meta == 0 && side == ForgeDirection.UP) return true;
