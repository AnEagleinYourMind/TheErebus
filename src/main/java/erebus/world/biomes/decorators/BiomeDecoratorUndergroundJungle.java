...
public class BiomeDecoratorUndergroundJungle extends BiomeDecoratorBaseErebus {
    ...
    private final WorldGenerator genTreeMahogany = new WorldGenErebusTrees(
        true,
        5,
        false,
        EnumWood.Mahogany,
    ...
    protected void decorate() {
        if (rand.nextInt(3) == 0) for (attempt = 0; attempt < 5; attempt++)
            if (genAmberUmberstone.generate(world, rand, x + offsetXZ(), rand.nextInt(120), z + offsetXZ())) break;

        if (rand.nextInt(6) == 0) for (attempt = 0; attempt < 4; attempt++)
        ...
        for (attempt = 0; attempt < 10; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genQuickSand.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 12; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genBigMushroomRed.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 20; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genBigMushroomBrown.generate(world, rand, xx, yy, zz);
        }
        ...
        if (rand.nextInt(11) == 0) {
            ...
            for (yy = 90; yy > 20; yy--)
                if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) if (genBamboo.generate(world, rand, xx, yy, zz)) break;
        }
        ...
        for (attempt = 0; attempt < 40; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genFerns.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 16; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genFiddleheads.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 850; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genGrass.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 800; attempt++) {
            ...
            if (world.isAirBlock(xx, yy, zz)) {
                ...
                if (!world.getBlock(xx + Direction.offsetX[offset], yy, zz + Direction.offsetZ[offset])
                ...
                for (int vineY = rand.nextInt(30); vineY > 0; vineY--)
                    if (world.isAirBlock(xx + Direction.offsetX[offset], yy - vineY, zz + Direction.offsetZ[offset]))
                        world.setBlock(
                            xx + Direction.offsetX[offset],
                            yy - vineY,
                            zz + Direction.offsetZ[offset],
                            Blocks.vine,
                            offset == 3 ? 1 : offset == 2 ? 4 : offset == 1 ? 0 : 2,
                            3);
            }
        }
        ...
        for (attempt = 0; attempt < 10; attempt++) {
            ...
            for (int hangerY = rand.nextInt(20); hangerY > 0; hangerY--) if (world.isAirBlock(xx, yy - hangerY, zz))
                world.setBlock(xx, yy - hangerY, zz, ModBlocks.hanger, 4, 2);
        }

        if (rand.nextInt(3) == 0) for (attempt = 0; attempt < 6; ++attempt) {
            xx = x + offsetXZ();
            yy = 15 + rand.nextInt(90);
            zz = z + offsetXZ();

            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genTurnips.generate(world, rand, xx, yy, zz);
        }
        else if (rand.nextBoolean() || rand.nextBoolean()) for (attempt = 0; attempt < 3; ++attempt) {
            xx = x + offsetXZ();
            yy = 15 + rand.nextInt(90);
            zz = z + offsetXZ();

            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genMelons.generate(world, rand, xx, yy, zz);
        }
    }
    ...
    protected void modifyOreGen(OreSettings oreGen, OreType oreType, boolean extraOres) {
        switch (oreType) {
            case COAL:
                oreGen.setIterations(extraOres ? 3 : 4)
                    .setY(27, 48)
                    .generate(world, rand, x, z); // generate first half above caves
                oreGen.setIterations(extraOres ? 4 : 6)
                    .setOreAmount(12, 14)
                    .setY(6, 24); // setup more & bigger clusters below caves, let the base code generate
                break;
            case DIAMOND:
                oreGen.setChance(0.1F)
                    .setY(6, 16);
