...
public class BiomeDecoratorFungalForest extends BiomeDecoratorBaseErebus {
    ...
    public static final Block[] mushrooms = { ModBlocks.dutchCap, ModBlocks.kaizersFinger, ModBlocks.sarcasticCzech,
        ModBlocks.grandmasShoes, ModBlocks.darkCapped };
    ...
    public void decorate() {
        ...
        for (attempt = 0; attempt < 200; attempt++) {
            ...
            if (r < 16) genGiantMushrooms.setMushroomType(MushroomType.DUTCH_CAP);
            else if (r < 25) genGiantMushrooms.setMushroomType(MushroomType.SARCASTIC_CZECH);
            else if (r < 80) genGiantMushrooms.setMushroomType(MushroomType.KAIZERS_FINGERS);
            else if (r < 96) genGiantMushrooms.setMushroomType(MushroomType.GRANDMAS_SHOES);
            ...
            for (int yAttempt = 0; yAttempt < 10; yAttempt++) if (checkSurface(SurfaceType.GRASS, xx, --yy, zz)) break;

            if (checkSurface(SurfaceType.GRASS, xx, yy, zz) && genGiantMushrooms.generate(world, rand, xx, yy, zz)
                && rand.nextInt(5) == 0) break;
        }
        ...
        for (attempt = 0; attempt < 100; attempt++) {
            ...
            for (yy = 20; yy < 100; yy += rand.nextBoolean() ? 2 : 1) if (checkSurface(SurfaceType.MIXED, xx, yy, zz)) {
        ...
        for (attempt = 0; attempt < 10; attempt++) {
            ...
            if (world.isAirBlock(xx, yy, zz)) genLichenPatch.generate(world, rand, xx, yy, zz);
        }
        ...
        switch (oreType) {
            ...
            case IRON:
                oreGen.setChance(0.75F)
                    .setIterations(extraOres ? 2 : 3, extraOres ? 4 : 5)
                    .setY(5, 42);
