...
public class BiomeDecoratorSubterraneanSavannah extends BiomeDecoratorBaseErebus {
    ...
    public void populate() {
        ...
        if (rand.nextBoolean() && rand.nextBoolean()) {
            ...
            if (rand.nextInt(3) != 0) for (yy = 100; yy > 20; yy--) {
                xx = x + offsetXZ();
                zz = z + offsetXZ();

                if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genBamboo.generate(world, rand, xx, yy, zz);
            }
        }
    }
    ...
    public void decorate() {
        ...
        for (int yUp = yy; 64 + yy >= yUp; yUp++)
            if (checkSurface(SurfaceType.GRASS, xx, yUp, zz) && checkSurface(SurfaceType.GRASS, xx - 6, yUp, zz - 6)
                && checkSurface(SurfaceType.GRASS, xx + 6, yUp, zz + 6)
                || checkSurface(SurfaceType.GRASS, xx, yUp, zz) && checkSurface(SurfaceType.GRASS, xx + 6, yUp, zz - 6)
                    && checkSurface(SurfaceType.GRASS, xx - 6, yUp, zz + 6)) {
                        genGiantBaobab.generate(world, rand, xx, yUp, zz);
                        break;
                    }

        if (rand.nextInt(12) == 0) for (attempt = 0; attempt < 5; attempt++)
            if (genAmberUmberstone.generate(world, rand, x + offsetXZ(), rand.nextInt(120), z + offsetXZ())) break;

        if (rand.nextInt(24) == 0) for (attempt = 0; attempt < 4; attempt++)
        ...
        for (attempt = 0; attempt < 65; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genTreeAcacia.generate(world, rand, xx, yy, zz);
        }
        ...
        if (rand.nextBoolean() && rand.nextBoolean()) {
            ...
            for (yy = 100; yy > 20; yy--)
                if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genRocks.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 10; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genTreeAsper.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 20; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx - 2, yy, zz - 2)
                && checkSurface(SurfaceType.GRASS, xx + 2, yy, zz + 2)
                && checkSurface(SurfaceType.GRASS, xx + 2, yy, zz - 2)
                && checkSurface(SurfaceType.GRASS, xx - 2, yy, zz + 2)) genTreeBaobab.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 28; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genRottenAcacia.generate(world, rand, xx, yy, zz);
        }
    ...
    protected void modifyOreGen(OreSettings oreGen, OreType oreType, boolean extraOres) {
        switch (oreType) {
            case GOLD:
                oreGen.setChance(0.75F)
                    .setIterations(extraOres ? 1 : 2)
                    .setY(50, 126)
                    .generate(world, rand, x, z); // split into 3 parts, higher areas have less gold
                oreGen.setChance(0.9F)
                    .setIterations(extraOres ? 1 : 2, extraOres ? 2 : 3)
                    .setY(25, 50)
                    .generate(world, rand, x, z); // balanced so there are more veins per chunk
                oreGen.setIterations(extraOres ? 2 : 3, extraOres ? 3 : 4)
                    .setY(5, 25);
                break;
            case EMERALD:
                ...
                break; // less common
            case DIAMOND:
                oreGen.setChance(0.4F)
                    .setIterations(1, 2)
                    .setOreAmount(2)
                    .setY(5, 16);
                break; // 2 diamonds per cluster, ~7-8 times smaller area thus lowered chance and iterations
            case JADE:
                oreGen.setChance(0.3F)
                    .setIterations(0, 3);
                break; // less common
            case PETRIFIED_WOOD:
                oreGen.setChance(0.5F)
                    .setIterations(0, extraOres ? 2 : 3)
                    .setY(5, 64);
