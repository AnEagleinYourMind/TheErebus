...
public class BiomeDecoratorElysianFields extends BiomeDecoratorBaseErebus {
    ...
    protected void populate() {
        if (rand.nextInt(3) == 0) for (attempt = 0; attempt < 25; attempt++) {
            xx = x + 16;
    ...
    public void decorate() {
        for (attempt = 0; attempt < 105; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genTreeCypress.generate(world, rand, xx, yy, zz);
        }

        if (generateFlowers && rand.nextBoolean()) for (int attempt = 0, xx, yy, zz; attempt < 65; attempt++) {
            xx = x + offsetXZ();
            yy = 15 + rand.nextInt(90);
            zz = z + offsetXZ();

            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genGiantFlowers.generate(world, rand, xx, yy, zz);
        }
    ...
    public void generateFeature(FeatureType featureType) {
        if (featureType == FeatureType.REDGEM) for (attempt = 0; attempt < 2 + rand.nextInt(2); attempt++)
    ...
    public static class BiomeDecoratorElysianForest extends BiomeDecoratorElysianFields {
        ...
        protected void populate() {
            /*
             * if (rand.nextInt(4) == 0){ for(attempt = 0; attempt < 45; attempt++){ xx = x + 16; yy =
             * rand.nextInt(120); zz = z + 16; if (checkSurface(SurfaceType.GRASS,xx,yy,zz)){
             * genPonds.prepare((rand.nextDouble() + 0.7D) * 1.5D); genPonds.generate(world,rand,xx,yy,zz); if
             * (rand.nextBoolean()){ break; } } } }
             */
