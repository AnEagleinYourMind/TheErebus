...
public class BiomeDecoratorSubmergedSwamp extends BiomeDecoratorBaseErebus {
    ...
    public void decorate() {
        ...
        if (rand.nextInt(34) == 0) for (int attempt = 0; attempt < 15; attempt++) if (genGiantLilyPad.generate(
            world,
            rand,
            x + 5 + rand.nextInt(6) + 8,
            ChunkProviderErebus.swampWaterHeight,
            z + 5 + rand.nextInt(6) + 8)) break;
        // Water
        ...
        for (attempt = 0; attempt < 10; attempt++) {
            ...
            if (world.isAirBlock(xx, yy, zz)) genMossPatch.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 600; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz) && checkSurface(SurfaceType.GRASS, xx - 2, yy, zz - 2)
        ...
        if (ConfigHandler.INSTANCE.generateVents)
            if (rand.nextInt(6) == 0) for (attempt = 0; attempt < rand.nextInt(4); attempt++) {
                xx = x + offsetXZ();
                yy = 25 + rand.nextInt(75);
                zz = z + offsetXZ();

                for (; yy > 20; yy--) if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) {
                    genGasVent.generate(world, rand, xx, yy, zz);
                    break;
                }
            }

        if (rand.nextInt(10) == 0) for (attempt = 0; attempt < rand.nextInt(4); attempt++) {
            xx = x + offsetXZ();
            yy = 25 + rand.nextInt(75);
            zz = z + offsetXZ();
        ...
        for (attempt = 0; attempt < rand.nextInt(3); attempt++) {
            ...
            if (checkSurface(SurfaceType.MIXED, xx, yy, zz)) genRottenAcacia.generate(world, rand, xx, yy, zz);
        }
        ...
        for (attempt = 0; attempt < 200; attempt++) {
            ...
            if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) genSwampPlant.generate(world, rand, xx, yy, zz);
        }
    ...
    protected void modifyOreGen(OreSettings oreGen, OreType oreType, boolean extraOres) {
        switch (oreType) {
            case COAL:
                oreGen.setChance(0.85F)
                    .setIterations(extraOres ? 2 : 3)
                    .setOreAmount(7, 10)
                    .setY(5, 56);
    ...
    public void generateFeature(FeatureType featureType) {
        if (featureType == FeatureType.REDGEM) for (attempt = 0; attempt < 8; attempt++)
