...
public class BiomeDecoratorUlteriorOutback extends BiomeDecoratorBaseErebus {
    ...
    public void decorate() {
        ...
        if (rand.nextBoolean()) for (attempt = 0; attempt < 20; attempt++) {
            xx = x + offsetXZ();
        ...
        for (attempt = 0; attempt < 194; attempt++) {
            ...
            for (yy = 20; yy < 100; yy += rand.nextBoolean() ? 2 : 1) if (checkSurface(SurfaceType.GRASS, xx, yy, zz)) {
                ...
                } else if (rand.nextInt(80) == 0) world.setBlock(xx, yy, zz, ModBlocks.fireBloom, 0, 2);
        ...
        if (rand.nextBoolean()) for (attempt = 0; attempt < 180; attempt++) {
            xx = x + offsetXZ();
            yy = 20 + rand.nextInt(80);
            zz = z + offsetXZ();
