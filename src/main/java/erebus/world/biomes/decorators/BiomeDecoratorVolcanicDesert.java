...
public class BiomeDecoratorVolcanicDesert extends BiomeDecoratorBaseErebus {
    ...
    public void populate() {
        for (attempt = 0; attempt < 35; attempt++) {
            ...
            if (checkSurface(SurfaceType.SAND, xx, yy, zz)) genLavaLakes.generate(world, world.rand, xx, yy, zz);
        }
    }
    ...
    public void decorate() {
        ...
        if (rand.nextInt(20) == 0) for (attempt = 0; attempt < rand.nextInt(4); attempt++) {
            xx = x + offsetXZ();
            yy = 25 + rand.nextInt(75);
            zz = z + offsetXZ();

            for (; yy > 20; yy--) if (checkSurface(SurfaceType.SAND, xx, yy, zz)) {
                genPricklyPair.generate(world, rand, xx, yy, zz);
                break;
            }
        }

        if (rand.nextInt(34) == 0) for (int attempt = 0; attempt < 15; attempt++) if (genAntlionLair
            .generate(world, rand, x + 5 + rand.nextInt(6) + 8, 15 + rand.nextInt(35), z + 5 + rand.nextInt(6) + 8))
            break;
    }

    @Override
    public void generateFeature(FeatureType featureType) {
        if (featureType == FeatureType.REDGEM) for (attempt = 0; attempt < 10; attempt++)
    ...
    protected void modifyOreGen(OreSettings oreGen, OreType oreType, boolean extraOres) {
        switch (oreType) {
            ...
            case GOLD:
                ...
                break; // less common
            case DIAMOND:
                oreGen.setType(OreType.DIAMOND_ENCRUSTED)
                    .setChance(0.4F)
                    .setIterations(1, 2)
                    .setOreAmount(2)
                    .setY(5, 16);
