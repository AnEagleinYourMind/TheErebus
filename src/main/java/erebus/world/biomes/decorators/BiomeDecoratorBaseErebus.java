...
public abstract class BiomeDecoratorBaseErebus {
    ...
    public final void decorate(World world, Random rand, int x, int z) {
        ...
        boolean extraOres = OreType.LEAD.isEnabled() || OreType.SILVER.isEnabled()
            || OreType.COPPER.isEnabled()
            || OreType.TIN.isEnabled()
            || OreType.ALUMINIUM.isEnabled();

        for (OreType oreType : OreType.values()) if (oreType.isEnabled()) {
