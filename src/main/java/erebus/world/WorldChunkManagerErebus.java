...
public class WorldChunkManagerErebus extends WorldChunkManager {
    ...
    private static final ArrayList<BiomeGenBase> allowedBiomes = new ArrayList<BiomeGenBase>(
        Arrays.asList(ModBiomes.undergroundJungle, ModBiomes.subterraneanSavannah));
    ...
    public WorldChunkManagerErebus(World world) {
        ...
        biomeGenLayer = GenLayerErebus.initializeAllBiomeGenerators(
            world.getSeed(),
            world.getWorldInfo()
                .getTerrainType())[1];
    }
    ...
    @Override
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase biomesForGeneration[], int x, int z, int sizeX,
        int sizeZ) {
    ...
    /*
     * @Override public float[] getTemperatures(float temperatureArray[], int x, int z, int sizeX, int sizeZ) { if
     * (temperatureArray == null || temperatureArray.length < sizeX * sizeZ) temperatureArray = new float[sizeX *
     * sizeZ]; Arrays.fill(temperatureArray, 0, sizeX * sizeZ, temperature); return temperatureArray; }
     */
    ...
    @Override
    public BiomeGenBase[] loadBlockGeneratorData(BiomeGenBase biomesForGeneration[], int x, int z, int sizeX,
        int sizeZ) {
        return getBiomeGenAt(biomesForGeneration, x, z, sizeX, sizeZ, true);
    }

    @Override
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] biomesForGeneration, int x, int z, int sizeX, int sizeZ,
        boolean useCache) {
        IntCache.resetIntCache();
    ...
    public ChunkPosition findBiomePosition(int x, int z, int checkRadius, List viableBiomes, Random rand) {
        ...
        for (int index = 0; index < sizeX * sizeZ; ++index) {
            ...
            if (viableBiomes.contains(BiomeGenBase.getBiomeGenArray()[biomeArray[index]])
                && (pos == null || rand.nextInt(attempts + 1) == 0)) {
    ...
    public boolean areBiomesViable(int x, int z, int checkRadius, List viableBiomes) {
        ...
        for (int index = 0; index < sizeX * sizeZ; ++index)
            if (!viableBiomes.contains(BiomeGenBase.getBiomeGenArray()[biomeArray[index]])) return false;
