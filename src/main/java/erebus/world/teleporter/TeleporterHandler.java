...
public final class TeleporterHandler {
    ...
    public void onServerTick(ServerTickEvent e) {
        ...
        for (UUID uuid : ids) if (waitingPlayers.adjustOrPutValue(uuid, (byte) -1, (byte) 0) <= 0) {
    ...
    private void transferEntity(Entity entity, int dimensionId) {
        ...
        if (!world.isRemote && !entity.isDead) if (entity instanceof EntityPlayerMP) {
            if (entity instanceof FakePlayer) return;

            EntityPlayerMP player = (EntityPlayerMP) entity;

            if (waitingPlayers.containsKey(
            ...
            waitingPlayers.put(
                player.getGameProfile()
                    .getId(),
                (byte) 40); // if there are any issues, we can either increase the number or rewrite the "is player in
                            // portal?" checking part
            ...
            player.mcServer.getConfigurationManager()
                .transferPlayerToDimension(
                    player,
                    dimensionId,
                    dimensionId == 0 ? teleportToOverworld : teleportToErebus);
            player.mcServer.getConfigurationManager()
                .transferPlayerToDimension(
                    player,
                    dimensionId,
             ...
             */
        } else if (!(entity instanceof EntityMinecartContainer)) { // TODO we cannot handle this, would result in
                                                                   // container breaking in both worlds and duplicate
                                                                   // items;
