...
final class TeleporterErebus extends Teleporter {
    ...
    private boolean isClear(Entity ent) {
        ...
        for (int x = minX; x < maxX; ++x) for (int z = minZ; z < maxZ; ++z) {
            for (int y = minY; y < maxY; ++y) {
                ...
                if (mat.isLiquid() || mat.isSolid() || mat.getMaterialMobility() == 2) return false;
            }
        }
    ...
    public boolean placeInExistingPortal(Entity entity, double x, double y, double z, float rotationYaw) {
        if (entity instanceof EntityPlayer) ((EntityPlayer) entity).triggerAchievement(ModAchievements.welcome);
            ...
            for (int chunkX = roundX - checkRadius; chunkX <= roundX + checkRadius; chunkX += 16)
                for (int chunkZ = roundZ - checkRadius; chunkZ <= roundZ + checkRadius; chunkZ += 16) {
                    ...
                    for (TileEntity te : (Iterable<TileEntity>) chunk.chunkTileEntityMap.values()) {
                        if (!(te instanceof TileEntityGaeanKeystone)) continue;
        ...
        if (distToPortal < checkRadius) {
            if (portalNotSaved) {
                destinationCoordinateCache
                    .add(coordPair, new PortalPosition(posX, posY, posZ, worldServerInstance.getTotalWorldTime()));
            ...
            if (entity instanceof EntityPlayer && ConfigHandler.INSTANCE.allowRespawning) {
                ...
                if (player.dimension == ConfigHandler.INSTANCE.erebusDimensionID) {
                    ...
                    player.getEntityData()
                        .setInteger("erebusPortalX", (int) posX);
                    player.getEntityData()
                        .setInteger("erebusPortalY", (int) posY);
                    player.getEntityData()
                        .setInteger("erebusPortalZ", (int) posZ);
                }
            }
    ...
    public void removeStalePortalLocations(long timer) {
        if (timer % 100L == 0L) {
            ...
            while (iterator.hasNext()) {
                ...
                PortalPosition position = (PortalPosition) destinationCoordinateCache
                    .getValueByKey(hashedPortalPos.longValue());
