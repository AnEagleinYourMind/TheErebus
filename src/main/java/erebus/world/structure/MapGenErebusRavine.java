...
public class MapGenErebusRavine extends MapGenBase {
    ...
    protected void generateRavine(long seed, int x, int z, Block[] blocks, double par6, double par8, double seed0,
        float seed2, float seed3, float seed4, int seed5, int seed6, double seed7) {
        ...
        for (int k1 = 0; k1 < 128; ++k1) {
            if (k1 == 0 || random.nextInt(3) == 0) f5 = 1.0F + random.nextFloat() * random.nextFloat() * 1.0F;
        ...
        for (; seed5 < seed6; ++seed5) {
            ...
            if (flag || random.nextInt(4) != 0) {
                ...
                if (par6 >= blockCoordX - 16.0D - d6 * 2.0D && seed0 >= blockCoordZ - 16.0D - d6 * 2.0D
                    && par6 <= blockCoordX + 16.0D + d6 * 2.0D
                    && seed0 <= blockCoordZ + 16.0D + d6 * 2.0D) {
                    ...
                    for (int posX = minX; !flag1 && posX < maxX; ++posX)
                        for (int posZ = minZ; !flag1 && posZ < maxZ; ++posZ)
                            for (int posY = maxY + 1; !flag1 && posY >= minY - 1; --posY) {
                                ...
                                if (posY >= 0 && posY < 128) {
                                    if (isOceanBlock(blocks, arrayIndex, posX, posY, posZ, x, z)) flag1 = true;

                                    if (posY != minY - 1 && posX != minX
                                        && posX != maxX - 1
                                        && posZ != minZ
                                        && posZ != maxZ - 1) posY = minY;
                                }
                            }
                    ...
                    if (!flag1) {
                        for (int posX = minX; posX < maxX; ++posX) {
                            ...
                            for (int posZ = minZ; posZ < maxZ; ++posZ) {
                                ...
                                if (d12 * d12 + d13 * d13 < 1.0D) for (int posY = maxY - 1; posY >= minY; --posY) {
                                    double d14 = (posY + 0.5D - par8) / d7;

                                    if ((d12 * d12 + d13 * d13) * field_75046_d[posY] + d14 * d14 / 6.0D < 1.0D) {
                                        if (isTopBlock(blocks, arrayIndex, posX, posY, posZ, x, z)) flag2 = true;
    ...
    protected void digBlock(Block[] blocks, int index, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
        ...
        if (y < 3) blocks[index] = Blocks.bedrock;
        else if (y < 4) blocks[index] = ModBlocks.umberstone;
        else if (y < 10 && biome.biomeID == ModBiomes.volcanicDesertID) blocks[index] = Blocks.flowing_lava;
            ...
            if (foundTop && blocks[index - 1] == biome.fillerBlock) blocks[index - 1] = biome.topBlock;
        }
    }
}
