...
public class WorldGenSwampHut implements IWorldGenerator {
    ...
    private void generate(World world, Random random, int x, int z) {
        ...
        if (biomeBase == ModBiomes.submergedSwamp)
            for (int newX = x; newX <= x + length; ++newX) for (int newZ = z; newZ <= z + width; ++newZ) {
    ...
    public boolean generateStructure(World world, Random rand, int x, int y, int z) {
        ...
        for (int direction = 0; direction < 4; direction++) {
            ...
            rotatedBeam(
                ...
                4,
                stairs,
                direction == 0 ? 6 : direction == 1 ? 4 : direction == 2 ? 7 : 5,
                8,
                direction);
            ...
            rotatedBeam(
                ...
                7,
                roof,
                direction == 0 ? 2 : direction == 1 ? 0 : direction == 2 ? 3 : 1,
                4,
                direction);
            rotatedBeam(
                world,
            ...
            rotatedBeam(
                ...
                7,
                roof,
                direction == 0 ? 7 : direction == 1 ? 5 : direction == 2 ? 6 : 4,
                1,
                direction);
            rotatedBeam(
                world,
            ...
            rotatedBeam(
                ...
                8,
                roof,
                direction == 0 ? 3 : direction == 1 ? 1 : direction == 2 ? 2 : 0,
                4,
                direction);
            rotatedBeam(
                world,
        ...
        if (!leftExtension && !rightExtension && !backExtension) { // && !frontExtension
            ...
            rotatedBeam(
                ...
                5,
                door,
                direction == 1 ? 0 : direction == 3 ? 2 : direction == 2 ? 3 : 1,
                1,
                direction);
    ...
    private void addExtention(World world, Random rand, int x, int y, int z, int direction) {
        ...
        for (int beamLength = 0; beamLength < 4; beamLength++) rotatedBeam(
            world,
            rand,
            x,
            y + 2,
        ...
        // air gap
        for (int beamLength = 0; beamLength < 2; beamLength++) for (int beamHeight = 0; beamHeight < 3; beamHeight++)
    ...
    public void rotatedBeam(World world, Random rand, int x, int y, int z, int a, int b, Block blockType, int blockMeta,
        int size, int direction) {
        switch (direction) {
            case 0:
                for (int xx = x + a; xx < x + a + size; xx++) world.setBlock(xx, y, z + b, blockType, blockMeta, 2);
                break;
            case 1:
                for (int zz = z + a; zz < z + a + size; zz++) world.setBlock(x + b, y, zz, blockType, blockMeta, 2);
                break;
            case 2:
    ...
    public void verticalBeam(World world, Random rand, int x, int y, int z, Block blockType, int blockMeta, int size,
        int direction) {
        for (int yy = y; yy < y + size; yy++) world.setBlock(x, yy, z, blockType, blockMeta, 2);
    }

    public void horizontalBeam(World world, Random rand, int x, int y, int z, Block blockType, int blockMeta, int size,
        int direction) {
        switch (direction) {
            case 0:
                for (int xx = x; xx < x + size; xx++) world.setBlock(xx, y, z, blockType, blockMeta, 2);
                break;
            case 1:
                for (int zz = z; zz < z + size; zz++) world.setBlock(x, y, zz, blockType, blockMeta, 2);
                break;
        }
    }
}
