...
public class WorldGenUmbergolemHut extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int xx = x - 6; xx <= x + 6; xx++) for (int zz = z - 6; zz <= z + 6; zz++)
            for (int yy = y + 1; yy < y + 10; yy++) if (!world.isAirBlock(xx, yy, zz)) return false;
        /*
         ...
         */
        for (int xx = x - 6; xx <= x + 6; xx++)
            for (int zz = z - 6; zz <= z + 6; zz++) for (int yy = y; yy < y + 6; yy++) {
                ...
                if (Math.round(Math.sqrt(dSqDome)) < 6) if (dSqDome >= Math.pow(6 - 2, 2.0D))
                    world.setBlock(xx, yy + 4, zz, ModBlocks.scorchedPlanks, 0, 3);
                else world.setBlock(xx, yy, zz, Blocks.air, 0, 2);
