...
public class WorldGenMarshwoodTree extends WorldGenTreeBase {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int xx = x - maxRadius; xx <= x + maxRadius; xx++) for (int zz = z - maxRadius; zz <= z + maxRadius; zz++)
            for (int yy = y + 2; yy < y + height; yy++) if (!world.isAirBlock(xx, yy, zz)) return false;
    ...
    private void createBranch(World world, Random rand, int x, int y, int z, int dir, boolean root) {
        ...
        for (int i = 0; i <= branchLength; ++i) {
            ...
            if (dir == 1) if (!root) {
                ...
                if (i < branchLength) addHangers(world, rand, x + i, y - 1, z);
                if (i == branchLength) createLeaves(world, rand, x + i, y - 1, z, 1);
            } else {
            ...
            if (dir == 3) if (!root) {
                ...
                if (i < branchLength) addHangers(world, rand, x, y - 1, z + i);
                if (i == branchLength) createLeaves(world, rand, x, y - 1, z + i, 1);
            } else {
            ...
            if (dir == 5) if (!root) {
                ...
                if (i < branchLength) addHangers(world, rand, x + i - 1, y - 1, z + i - 1);
                if (i == branchLength) createLeaves(world, rand, x + i, y - 1, z + i, 1);
            } else {
    ...
    public void createLeaves(World world, Random rand, int x, int y, int z, int radius) {
        ...
        for (int xx = x - radius; xx <= x + radius; xx++)
            for (int zz = z - radius; zz <= z + radius; zz++) for (int yy = y; yy > y - height; yy--) {
                double dSq = Math.pow(xx - x, 2.0D) + Math.pow(zz - z, 2.0D) + Math.pow(yy - y, 2.0D);
                if (Math.round(Math.sqrt(dSq)) <= radius)
                    if (Math.round(Math.sqrt(dSq)) == 0) world.setBlock(xx, yy, zz, log, 0, 2);
    ...
    public void addHangers(World world, Random rand, int x, int y, int z) {
        if (rand.nextInt(4) != 0) {
            ...
            for (int yy = y; yy > y - length; --yy)
                if (world.getBlock(x, yy, z) == Blocks.air) world.setBlock(x, yy, z, ModBlocks.hanger, 4, 2);
                else break;
        }
    }
}
