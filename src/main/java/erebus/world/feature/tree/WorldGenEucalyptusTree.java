...
public class WorldGenEucalyptusTree extends WorldGenTreeBase {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int yy = 0; yy < span; yy++) for (int xx = -span; xx <= span; xx++)
            for (int zz = -span; zz <= span; zz++) if (!world.isAirBlock(x + xx, y + height + yy, z + zz)) return false;

        for (int p = -2; p < 3; p++) for (int r = -1; r < 2; r++) {
            world.setBlock(x + p, y + height + span + 1, z + r, leaves, 0, 3);
            world.setBlock(x + r, y + height + span + 1, z + p, leaves, 0, 3);
        }

        for (int p = -1; p < 2; p++)
            for (int r = -1; r < 2; r++) world.setBlock(x + r, y + height + span + 2, z + p, leaves, 0, 3);
        ...
        for (int a = 0; a < branches; a++) {
            ...
            for (int p = -1; p < 2; p++)
                for (int r = -1; r < 2; r++) world.setBlock(posX + r, posY + 1, posZ + p, leaves, 0, 3);
        ...
        for (int yy = 0; yy < height + span + 2; yy++) world.setBlock(x, y + yy, z, log, 0, 3);
