...
public class WorldGenTallJungleTree extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (testY = y + 1; testY < y + baseHeight; ++testY) if (!world.isAirBlock(x, testY, z)) return false;

        for (testY = y + baseHeight; testY <= y + treeHeight; ++testY) for (int testX = x - 2; testX <= x + 2; ++testX)
