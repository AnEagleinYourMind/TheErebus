...
public class WorldGenSapTree extends WorldGenTreeBase {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int yy = y; yy < y + height; ++yy) {
            ...
            if (yy < y + height - 1) world.setBlock(x, yy, z, log, 0, 2);
    ...
    private void createLeaves(World world, int x, int y, int z) {
        for (int d = -1; d < 2; d++) for (int d2 = -1; d2 < 2; d2++) placeLeaves(world, x + d, y, z + d2);
    }

    private void placeLeaves(World world, int x, int y, int z) {
        if (world.getBlock(x, y, z) == Blocks.air) world.setBlock(x, y, z, leaves);
    }
