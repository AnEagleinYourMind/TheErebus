...
public class WorldGenGiantBaobab extends WorldGenerator {
    ...
    public static final WeightedLootList chestLoot = new WeightedLootList(
        new LootItemStack[] { new LootItemStack(Items.bow).setWeight(1), new LootItemStack(Items.arrow).setAmount(3, 10)
            .setWeight(18),
            new LootItemStack(Items.book).setAmount(1, 4)
                .setWeight(18),
            ...
            new LootItemStack(ModItems.jadePickaxe).setWeight(1), new LootItemStack(Items.stone_pickaxe).setWeight(1),
            new LootItemStack(Items.golden_shovel).setWeight(3), new LootItemStack(Items.iron_shovel).setWeight(2),
            new LootItemStack(ModItems.jadeShovel).setWeight(1), new LootItemStack(Items.stone_shovel).setWeight(1),
            new LootItemStack(Items.golden_axe).setWeight(3), new LootItemStack(Items.iron_axe).setWeight(2),
            new LootItemStack(ModItems.jadeAxe).setWeight(1), new LootItemStack(Items.stone_axe).setWeight(1),
            new LootItemStack(Items.golden_sword).setWeight(3), new LootItemStack(Items.iron_sword).setWeight(2),
            new LootItemStack(ModItems.jadeSword).setWeight(1), new LootItemStack(Items.stone_sword).setWeight(1),
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int xx = x - baseRadius; xx <= x + baseRadius; xx++)
            for (int zz = z - baseRadius; zz <= z + baseRadius; zz++)
                for (int yy = y + 1; yy < y + height; yy++) if (!world.isAirBlock(xx, yy, zz)) return false;
        ...
        for (int yy = y; height + y >= yy; yy++)
            for (int i = radius * -1; i <= radius; ++i) for (int j = radius * -1; j <= radius; ++j) {
                double dSq = i * i + j * j;
                ...
                if (yy <= y + layer2 && yy > y + layer1) {
                    if (Math.round(Math.sqrt(dSq)) == radius - 1) world.setBlock(x + i, yy, z + j, log);
                    world.setBlock(x, yy, z, log);
                }

                if (yy <= y + layer3 && yy > y + layer2) {
                    if (Math.round(Math.sqrt(dSq)) == radius - 2) world.setBlock(x + i, yy, z + j, log);
                    world.setBlock(x, yy, z, log);
                }
                ...
                if (yy <= y + layer4 && yy > y + layer3) {
                    ...
                    if (Math.round(Math.sqrt(dSq)) == radius - 3
                        || Math.round(Math.sqrt(dSq)) <= radius - 3 && Math.round(Math.sqrt(dSq)) > radius - 12
                            && yy >= y + 9
                            && yy <= y + 12)
                        world.setBlock(x + i, yy, z + j, log);
                }
                ...
                if (yy <= y + layer5 && yy > y + layer4) {
                    ...
                    if (Math.round(Math.sqrt(dSq)) <= radius - 3 && yy == y + 20) world.setBlock(x + i, yy, z + j, log);
                ...
                if (Math.round(Math.sqrt(dSq)) < radius - 3 && Math.round(Math.sqrt(dSq)) % 2 == 0 && yy == y + 21)
                    if (x + i != x && z + j != z) world.setBlock(x + i, yy, z + j, Blocks.web);
            }
    ...
    public void createLeaves(World world, Random rand, int x, int y, int z) {
        ...
        for (int xx = x - radius; xx <= x + radius; xx++)
            for (int zz = z - radius; zz <= z + radius; zz++) for (int yy = y - height; yy < y + height; yy++) {
                double dSq = Math.pow(xx - x, 2.0D) + Math.pow(zz - z, 2.0D) + Math.pow(yy - y, 2.0D);
                if (Math.round(Math.sqrt(dSq)) < radius)
                    if (dSq >= Math.pow(radius - 2, 2.0D)) if (rand.nextInt(10) == 0) world.setBlock(xx, yy, zz, log);
                else world.setBlock(xx, yy, zz, leaves);
            }
    }
    ...
    public void createRoots(World world, Random rand, int x, int y, int z) {
        ...
        for (int yy = y - 1; yy > y - height; --yy)
            for (int i = (int) (radius * -1); i <= radius; ++i) for (int j = (int) (radius * -1); j <= radius; ++j) {
                double dSq = i * i + j * j;
                if (Math.round(Math.sqrt(dSq)) <= radius)
                    if (rand.nextInt(5) != 0) world.setBlock(x + i, yy, z + j, log, 15, 2);
                if (yy % 4 == 0) radius -= 0.02;
            }
    }
