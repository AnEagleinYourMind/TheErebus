...
public class WorldGenAsperTree extends WorldGenTreeBase {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (testY = y + 1; testY <= y + height + 1; ++testY) for (int testX = x - 2; testX <= x + 2; ++testX)
            for (int testZ = z - 2; testZ <= z + 2; ++testZ) if (!world.isAirBlock(testX, testY, testZ)) return false;
        ...
        for (int yy = 1; yy < height; yy++) for (int xx = -1; xx <= 1; xx++) for (int zz = -1; zz <= 1; zz++) {
        ...
        for (int a = 0; a < 4; a++) world.setBlock(x + offsetX[a], y + height, z + offsetZ[a], leaves, 0, 3);
