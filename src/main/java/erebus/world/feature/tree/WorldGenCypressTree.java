...
public class WorldGenCypressTree extends WorldGenTreeBase {
    ...
    private boolean checkEmptyArea(World world, int x, int y, int z, int totalHeight, int rad) {
        ...
        for (int xx = x - rad; xx <= x + rad; xx++) for (int zz = z - rad; zz <= z + rad; zz++)
            for (int yy = y + 1; yy <= y + totalHeight; yy++) if (world.getBlock(xx, yy, zz)
                .getMaterial() != Material.air) return false;
    ...
    private boolean generateSmallTree(World world, Random rand, int x, int y, int z) {
        ...
        for (int yy = y; yy <= y + trunkH; yy++) world.setBlock(x, yy, z, log, 0, 2);
        for (int yy = y + trunkH + 1; yy <= y + trunkH + 2; yy++) world.setBlock(x, yy, z, leaves, 0, 2);

        for (int a = 0; a < 4; a++) {
            for (int yy = y + nakedTrunkH; yy <= y + trunkH; yy++)
                world.setBlock(x + Direction.offsetX[a], yy, z + Direction.offsetZ[a], leaves, 0, 2);

            if (leafH - 4 > 1) for (int yy = y + nakedTrunkH + 2; yy <= y + trunkH - 2; yy++) {
        ...
        for (int a = 0; a < 2; a++)
            for (int b = 0; b < 2; b++) for (int yy = y + nakedTrunkH + 1; yy <= y + trunkH - 1; yy++) {
    ...
    private boolean generateMediumTree(World world, Random rand, int x, int y, int z) {
        ...
        for (int yy = y; yy <= y + trunkH; yy++) world.setBlock(x, yy, z, log, 0, 2);
        for (int yy = y + trunkH + 1; yy <= y + trunkH + 3; yy++) world.setBlock(x, yy, z, leaves, 0, 2);
        ...
        for (int a = 0; a < 4; a++) {
            ...
            if (leafH - 7 > 1) for (int yy = y + nakedTrunkH + 3; yy <= y + trunkH - 4; yy++) {
        ...
        for (int a = 0; a < 2; a++) for (int b = 0; b < 2; b++) {
            ...
            for (int yy = y + nakedTrunkH + 2; yy <= y + trunkH - 3; yy++) {
                ...
                if (!canSkip || rand.nextInt(10) != 0) world.setBlock(x - 2 + 4 * a, yy, z + -1 + 2 * b, leaves, 0, 2);
                if (!canSkip || rand.nextInt(10) != 0) world.setBlock(x - 1 + 2 * a, yy, z + -2 + 4 * b, leaves, 0, 2);
            }
        }
    ...
    private boolean generateLargeTree(World world, Random rand, int x, int y, int z) {
        ...
        for (int yy = y; yy <= y + trunkH; yy++) {
            ...
            if (yy <= y + trunkH - 2) for (int a = 0; a < 4; a++) {
        ...
        for (int yy = y + trunkH + 1; yy <= y + trunkH + 4; yy++) world.setBlock(x, yy, z, leaves, 0, 2);
        ...
        for (int a = 0; a < 2; a++) for (int b = 0; b < 2; b++) {
            ...
            if (leafH - 9 > 1) for (int yy = y + nakedTrunkH + 4; yy <= y + trunkH - 5; yy++) {
                boolean canSkip = (yy == y + nakedTrunkH + 4 || yy == y + trunkH - 5) && leafH - 9 > 2;
                if (!canSkip || rand.nextInt(12) != 0) world.setBlock(x - 3 + 6 * a, yy, z - 1 + 2 * b, leaves, 0, 2);
                if (!canSkip || rand.nextInt(12) != 0) world.setBlock(x - 1 + 2 * a, yy, z - 3 + 6 * b, leaves, 0, 2);
            }
        }
