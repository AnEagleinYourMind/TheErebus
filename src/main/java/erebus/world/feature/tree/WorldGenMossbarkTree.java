...
public class WorldGenMossbarkTree extends WorldGenTreeBase {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (testY = y + 1; testY < y + 3; ++testY) if (!world.isAirBlock(x, testY, z)) return false;

        for (testY = y + 3; testY <= y + 1 + maxHeight; ++testY) {
            for (int testX = x - 4; testX <= x + 4; ++testX) for (int testZ = z - 1; testZ <= z + 1; ++testZ)
                if (!world.isAirBlock(testX, testY, testZ)) return false;

            for (int testX = x - 1; testX <= x + 1; ++testX) for (int testZ = z - 4; testZ <= z + 4; ++testZ)
                if (!world.isAirBlock(testX, testY, testZ)) return false;
        }
        ...
        for (int a = 0; a < 4; a++) generateOutsideBranch(
            world,
            rand,
            x + offsetX[a] * 3,
            y + middleBranchHeights[a],
            z + offsetZ[a] * 3,
            outsideBranchHeights[a]);
    ...
    public void generateMiddleBranch(World world, Random rand, int x, int y, int z, int height) {
        for (int yy = 0; yy < 2; ++yy) world.setBlock(x, y - 1 - yy, z, leaves, 0, 3);
        for (int yy = 0; yy < height; ++yy) world.setBlock(x, y + yy, z, log, 0, 3);
    }
    ...
    public void generateOutsideBranch(World world, Random rand, int x, int y, int z, int height) {
        ...
        for (int yy = 0; yy < height; ++yy) {
            if (yy < height - 1) world.setBlock(x, y + yy, z, log, 0, 3);

            for (int a = 0; a < 4; a++) world.setBlock(x + offsetX[a], y + yy, z + offsetZ[a], leaves, 0, 3);
        }
    }
}
