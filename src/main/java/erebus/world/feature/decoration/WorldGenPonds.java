...
public class WorldGenPonds extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (y <= 29 || world.isAirBlock(x, y, z)) return false;
        y -= 4;
        ...
        for (int xx = 0; xx < 16; ++xx) for (int zz = 0; zz < 16; ++zz) for (int yy = 0; yy < 8; ++yy) {
            boolean flag = !placeWater[(xx * 16 + zz) * 8 + yy] && (xx < 15 && placeWater[((xx + 1) * 16 + zz) * 8 + yy]
                || xx > 0 && placeWater[((xx - 1) * 16 + zz) * 8 + yy]
                || zz < 15 && placeWater[(xx * 16 + zz + 1) * 8 + yy]
                || zz > 0 && placeWater[(xx * 16 + zz - 1) * 8 + yy]
                || yy < 7 && placeWater[(xx * 16 + zz) * 8 + yy + 1]
                || yy > 0 && placeWater[(xx * 16 + zz) * 8 + yy - 1]);
            if (!flag) continue;

            Material mat = world.getBlock(x + xx, y + yy, z + zz)
                .getMaterial();
            if (yy >= 4 && mat.isLiquid()
                || yy < 4 && !mat.isSolid() && world.getBlock(x + xx, y + yy, z + zz) != Blocks.flowing_water)
                return false;
        }

        for (int xx = 0; xx < 16; ++xx)
            for (int zz = 0; zz < 16; ++zz) for (int yy = 0; yy < 8; ++yy) if (placeWater[(xx * 16 + zz) * 8 + yy])
                world.setBlock(x + xx, y + yy, z + zz, yy >= 4 ? Blocks.air : Blocks.flowing_water, 0, 3);

        for (int xx = 0; xx < 16; ++xx) for (int zz = 0; zz < 16; ++zz) for (int yy = 4; yy < 8; ++yy)
            if (placeWater[(xx * 16 + zz) * 8 + yy] && world.getBlock(x + xx, y + yy - 1, z + zz) == Blocks.dirt
                && world.getSavedLightValue(EnumSkyBlock.Sky, x + xx, y + yy, z + zz) > 0) {
        ...
        if (rand.nextBoolean()) for (int attempt = 0, xx, zz; attempt < 2 + rand.nextInt(6); attempt++) {
            xx = x + rand.nextInt(8) + 4;
            zz = z + rand.nextInt(8) + 4;

            for (int yy = 0; yy < 8; yy++) if (world.getBlock(xx, y + yy, zz) == Blocks.sand) {
                double rad = rand.nextDouble() * 1.3D + 1.8D;
                int irad = (int) Math.ceil(rad);

                for (int px = xx - irad; px <= xx + irad; px++) for (int pz = zz - irad; pz <= zz + irad; pz++)
                    for (int py = y + yy - irad; py <= y + yy + irad; py++)
                        if (world.getBlock(px, py, pz) == Blocks.sand
                            && Math.sqrt(Math.pow(px - xx, 2) + Math.pow(pz - zz, 2) + Math.pow(py - y + yy, 2))
                                <= rad + rand.nextFloat() * 0.3F)
        ...
        for (int attempt = 0; attempt < 5; attempt++) genLily.generate(
            world,
            rand,
            x + rand.nextInt(8) - rand.nextInt(8) + 8,
            y + 2 + rand.nextInt(6),
            z + rand.nextInt(8) - rand.nextInt(8) + 8);
        ...
        for (int sugarCaneAttempt = 0, xx, yy, zz; sugarCaneAttempt < 30; sugarCaneAttempt++) {
            ...
            if ((block == Blocks.grass || block == Blocks.sand || block == ModBlocks.mud)
                && Blocks.reeds.canPlaceBlockAt(world, xx, yy, zz))
        ...
        for (int bullRushAttempt = 0, xx, yy, zz; bullRushAttempt < 50; bullRushAttempt++) {
            ...
            if (block == Blocks.sand || block == ModBlocks.mud)
                for (int height = 0; height < 1; height++) if (world.isAirBlock(xx, yy + height, zz)) {
