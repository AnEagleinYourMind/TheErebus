    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (y <= 4) return false;
            ...
            for (i1 = 0; i1 < l; ++i1) {
                ...
                for (int j1 = 1; j1 < 15; ++j1) for (int k1 = 1; k1 < 15; ++k1) for (int l1 = 1; l1 < 7; ++l1) {
                    double d6 = (j1 - d3) / (d0 / 2.0D);
                    double d7 = (l1 - d4) / (d1 / 2.0D);
                    double d8 = (k1 - d5) / (d2 / 2.0D);
                    double d9 = d6 * d6 + d7 * d7 + d8 * d8;

                    if (d9 < 1.0D) aboolean[(j1 * 16 + k1) * 8 + l1] = true;
                }
            }
            ...
            for (i1 = 0; i1 < 16; ++i1) for (j2 = 0; j2 < 16; ++j2) for (i2 = 0; i2 < 8; ++i2) {
            ...
            for (i1 = 0; i1 < 16; ++i1)
                for (j2 = 0; j2 < 16; ++j2) for (i2 = 0; i2 < 8; ++i2) if (aboolean[(i1 * 16 + j2) * 8 + i2])
                    world.setBlock(x + i1, y + i2, z + j2, i2 >= 4 ? Blocks.air : fillerFluid, 0, 2);

            for (i1 = 0; i1 < 16; ++i1) for (j2 = 0; j2 < 16; ++j2) for (i2 = 4; i2 < 8; ++i2)
                if (aboolean[(i1 * 16 + j2) * 8 + i2] && world.getBlock(x + i1, y + i2 - 1, z + j2) == Blocks.dirt
                    && world.getSavedLightValue(EnumSkyBlock.Sky, x + i1, y + i2, z + j2) > 0) {
                        ...
                        else world.setBlock(x + i1, y + i2 - 1, z + j2, Blocks.grass, 0, 2);
                    }

            if (fillerFluid.getMaterial() == Material.water) for (i1 = 0; i1 < 16; ++i1) for (j2 = 0; j2 < 16; ++j2) {
                for (i2 = 0; i2 < 8; ++i2) {
                    flag = !aboolean[(i1 * 16 + j2) * 8 + i2] && (i1 < 15 && aboolean[((i1 + 1) * 16 + j2) * 8 + i2]
                        || i1 > 0 && aboolean[((i1 - 1) * 16 + j2) * 8 + i2]
                        || j2 < 15 && aboolean[(i1 * 16 + j2 + 1) * 8 + i2]
                    ...
                    if (flag && (i2 < 4 || rand.nextInt(2) != 0)
                        && world.getBlock(x + i1, y + i2, z + j2)
                            .getMaterial()
                            .isSolid()) {
                        world.setBlock(x + i1, y + i2, z + j2, ModBlocks.quickSand, 0, 2);
                        if (rand.nextInt(10) == 0) world.setBlock(x + i1, y + 2, z + j2, ModBlocks.mireCoral, 0, 2);
                    }
                }
