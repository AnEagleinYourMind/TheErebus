...
public class WorldGenAmberGround extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int xx = -ceilRad; xx <= ceilRad; xx++)
            for (int yy = -ceilRad; yy <= ceilRad; yy++) for (int zz = -ceilRad; zz <= ceilRad; zz++)
    ...
    protected void setBlock(World world, int x, int y, int z, Random rand) {
        if (rand.nextFloat() > BUGGED_AMBER_CHANCE) world.setBlock(x, y, z, ModBlocks.amber);
        else {
            ...
            if (tile != null) try {
                ...
                if (rand.nextFloat() > WAND_CHANCE) {
                    ...
                    entity = cls.getConstructor(new Class[] { World.class })
                        .newInstance(world);
                } else {
