...
public class WorldGenScorchedWood extends WorldGenerator {
    ...
    public boolean generate(World world, Random random, int x, int y, int z) {
        ...
        for (int testY = y + 1; testY <= y + treeHeight; ++testY) for (int testX = x - 1; testX <= x + 1; ++testX)
            for (int testZ = z - 1; testZ <= z + 1; ++testZ) if (!world.isAirBlock(testX, testY, testZ)) return false;

        // generate tree

        for (int part = 0; part < partAmount; part++) {
            for (int a = 0; a < 2; a++) world.setBlock(x, y + part * 2 + a, z, ModBlocks.scorchedWood, 0, 3);
