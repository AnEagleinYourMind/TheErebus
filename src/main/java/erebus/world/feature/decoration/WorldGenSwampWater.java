...
public class WorldGenSwampWater extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int x1 = x - radius; x1 <= x + radius; x1++)
            for (int z1 = z - radius; z1 <= z + radius; z1++) for (int y1 = y; y1 > y - depth; y1--) {
                ...
                if (Math.round(Math.sqrt(dSq)) < radius) {
                    if (dSq >= Math.pow(radius - 2, 2.0D)) {
                        if (world.getBlock(x1, y1, z1) != Blocks.water) world.setBlock(x1, y1, z1, ModBlocks.mud, 0, 2);
                        else world.setBlock(x1, y1, z1, Blocks.water, 0, 3);
                    } else if (dSq < Math.pow(radius - 3, 2.0D)) world.setBlock(x1, y1, z1, Blocks.water, 0, 3);
                    if (dSq <= Math.pow(radius, 2.0D) && dSq >= Math.pow(radius - 3, 2.0D))
                        if (world.getBlock(x1, y1, z1) != Blocks.water)
                            if (rand.nextInt(3) != 0 ? world.setBlock(x1, y, z1, Blocks.water, 0, 3)
                                : world.setBlock(x1, y, z1, Blocks.grass, 0, 2));

                    if (dSq <= Math.pow(radius - 2, 2.0D)) {
                        world.setBlock(x1, y - depth, z1, ModBlocks.mud, 0, 2);
                        int length = rand.nextInt(13) + 20;
                        for (int yy = y - depth - 1; yy > y - depth - 1 - length; --yy)
                            if (world.getBlock(x, yy, z) == Blocks.air) world.setBlock(x1, yy, z1, Blocks.dirt, 0, 2);
                        else break;
                    }
        ...
        for (int attempt = 0; attempt < 5; attempt++) genAlgae.generate(
            world,
            rand,
        ...
        for (int attempt = 0; attempt < 20; attempt++) genLily.generate(
            world,
            rand,
            x + rand.nextInt(8) - rand.nextInt(8) + 8,
            y + 2 + rand.nextInt(6),
