...
public class WorldGenMossPatch extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (blockPlaced) createPatch(world, rand, x, y, z);
        return true;
    }
    ...
    private void createPatch(World world, Random rand, int x, int y, int z) {
        ...
        for (int xx = x - radius; xx <= x + radius; ++xx) for (int zz = z - radius; zz <= z + radius; ++zz)
            for (int yy = y - radius; yy <= y + radius; ++yy) if (world.isAirBlock(xx, yy, zz))
                for (int attempt = 0; attempt < 3; attempt++) placeBlockAt(world, rand, xx, yy, zz);
    }
