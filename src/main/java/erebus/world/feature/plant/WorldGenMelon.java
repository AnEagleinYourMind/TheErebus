...
public class WorldGenMelon extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (!canSpawn) for (int waterAttempt = 0; waterAttempt < 30; waterAttempt++) if (world
            .getBlock(
                x + rand.nextInt(8) - rand.nextInt(8),
                y + rand.nextInt(4) - rand.nextInt(4),
        ...
        for (int attempt = 0, xx, yy, zz; attempt < 64; ++attempt) {
            ...
            if (world.isAirBlock(xx, yy, zz) && world.getBlock(xx, yy - 1, zz) == Blocks.grass
                && Blocks.melon_block.canPlaceBlockAt(world, xx, yy, zz))
