...
public class WorldGenGiantMushrooms extends WorldGenErebus {
    ...
    public enum MushroomType {
        ...
        public static MushroomType getFromShroom(Block block) {
            for (MushroomType type : values()) if (type.mushroom == block) return type;
            return null;
        }
    }
    ...
    private boolean genBulbCapped(int x, int y, int z, Block mushroom) {
        ...
        if (!checkAirCube(x, y, z, x, y + stalkHeight - sideHeight, z)
            || !checkAirCube(x - 2, y + stalkHeight - sideHeight + 1, z - 2, x + 2, y + stalkHeight + 1, z + 2))
            return false;
        ...
        for (int py = 1; py <= sideHeight; py++) for (int off = -1; off <= 1; off++) {
    ...
    private boolean genKaizersFingers(int x, int y, int z, Block mushroom) {
        ...
        if (!checkAirCube(x - 1, y + 3, z - 1, x + 1, y + mainShroomHeight + 1, z + 1)
            || !checkAirCube(x - 4, y, z - 4, x + 4, y + 2, z + 4)
            || !checkSolidCube(x - 4, y - 1, z - 4, x + 4, y - 1, z + 4)) return false;
        ...
        for (int smallShroomAttempt = 0, xx, zz; smallShroomAttempt < 4 + rand.nextInt(7); smallShroomAttempt++) {
            ...
            if (!isAir(xx, y, zz) || !isAir(xx - 1, y, zz)
                || !isAir(xx + 1, y, zz)
                || !isAir(xx, y, zz - 1)
                || !isAir(xx, y, zz + 1)) continue;
        ...
        for (int connectionAttempt = 0, dir, xx, zz; connectionAttempt < 48; connectionAttempt++) {
            ...
            ChunkCoordinates coords2 = rand.nextInt(3) != 0 ? connectList.get(0)
                : connectList.get(rand.nextInt(coordAmt));
    ...
    private boolean genDutchCap(int x, int y, int z, Block mushroom) {
        ...
        if (!checkAirCube(x - 2, y, z - 2, x + 3, y + 4, z + 3)
            || !checkAirCube(x - 3, y + 5, z - 3, x + 5, y + height + 1, z + 5)
            || !checkAirCube(x - 4, y + height + 2, z - 4, x + 7, y + height + 4, z + 7)
            || !checkSolidCube(x - 2, y - 1, z - 2, x + 3, y - 1, z + 3)) return false;
        ...
        for (int py = 4; py <= height; py++) {
            ...
            if (rand.nextInt(3) == 0 || isTop) for (int attempt = 0; attempt < (isTop ? 2 : 1); attempt++) {
                ...
                for (int branch = 1; branch <= branchSize; branch++) {
                    setBlockRect(
                        x + branchAddX * branch,
                        z + branchAddZ * branch,
                        x + branchAddX * branch + 1,
                        z + branchAddZ * branch + 1,
                        y + py - 1 + branch,
                        mushroom,
                        stalkMeta);
                    if (isTop) setBlockRect(
                        x + branchAddX * branch,
                        z + branchAddZ * branch,
                        x + branchAddX * branch + 1,
                        z + branchAddZ * branch + 1,
                ...
                int branchTopX = x + branchAddX * branchSize, branchTopY = y + py + branchSize,
                    branchTopZ = z + branchAddZ * branchSize;
                ...
                if (isTop) {
                    ...
                    for (int a = 0; a < 2; a++) for (int b = 0; b < 2; b++) {
                        ...
                        if (isAir(branchTopX - 1 + 3 * a, branchTopY - 2, branchTopZ - 1 + 3 * b)) setBlock(
                            branchTopX - 1 + 3 * a,
                            branchTopY - 2,
                            branchTopZ - 1 + 3 * b,
                            mushroom,
                ...
                } else for (int a = 0; a < 2; a++) for (int b = 0; b < 2; b++) {
                    if (isAir(branchTopX + a, branchTopY, branchTopZ + b))
                        setBlock(branchTopX + a, branchTopY, branchTopZ + b, mushroom, bulbFullMeta);
                    if (isAir(branchTopX - 1 + 3 * a, branchTopY - 1, branchTopZ + b))
                        setBlock(branchTopX - 1 + 3 * a, branchTopY - 1, branchTopZ + b, mushroom, bulbFullMeta);
    ...
    private boolean genGrandmasShoes(int x, int y, int z, Block mushroom) {
        int height = 5 + rand.nextInt(8), splits = rand.nextInt(height > 8 ? 3 : 2),
            splitSize = splits == 0 ? height : (int) Math.ceil(height / (1F + splits)),
            splitDir = splits != 0 ? rand.nextInt(4) : -1;

        int splitOffX = splitDir == -1 ? 0 : Direction.offsetX[splitDir],
            splitOffZ = splitDir == -1 ? 0 : Direction.offsetZ[splitDir];
        if (!checkAirCube(x - 1, y, z - 1, x + 2, y + height - 2, z + 2)
            || !checkAirCube(
                x - 3 + splitOffX,
                z - 3 + splitOffZ,
                y + height - 1,
                ...
                y + height + 1,
                z + 3 + splitOffZ)
            || !checkSolidCube(x - 1, y - 1, z - 1, x + 3, y - 1, z + 3)) return false;
    ...
    private boolean genSarcasticCzech(int x, int y, int z, Block mushroom) {
        ...
        if (!checkAirCube(x, y, z, x, y + height, z)
            || !checkAirCube(x - armLength, y + height, z - armLength, x + armLength, y + height + 1, z + armLength))
            return false;
        ...
        for (ForgeDirection[] dirs : new ForgeDirection[][] {
            new ForgeDirection[] { ForgeDirection.EAST, ForgeDirection.SOUTH },
            new ForgeDirection[] { ForgeDirection.EAST, ForgeDirection.NORTH },
            new ForgeDirection[] { ForgeDirection.WEST, ForgeDirection.SOUTH },
            new ForgeDirection[] { ForgeDirection.WEST, ForgeDirection.NORTH } }) {
            ...
            for (int i = -1; i <= 1; i++)
                for (int j = -1; j <= 1; j++) setBlock(xx + i, yy, zz + j, mushroom, bulbFullMeta);
        }
    ...
    private int getBulbMetadata(int centerX, int centerZ, ChunkCoordinates bulb) {
        ...
        if (posX && negX) if (bulb.posZ > centerZ) return 8; // use top and south
        else return 2; // use top and north

        if (posZ && negZ) if (bulb.posX > centerX) return 6; // use top and east
        ...
        if (posXposZ && negXposZ || negXnegZ && posXnegZ || posXposZ && posXnegZ || negXposZ && negXnegZ) return 14; // use
                                                                                                                     // full
                                                                                                                     // cap
                                                                                                                     // for
                                                                                                                     // +
        ...
        if (sides == 0 && corners == 2) for (int dir = 0, meta; dir < 4; dir++) {
        ...
        if (sides == 1 && corners == 1 || sides == 0 && corners == 2) if (bulb.posX > centerX) {
