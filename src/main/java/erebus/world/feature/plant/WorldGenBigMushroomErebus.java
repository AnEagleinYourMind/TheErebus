        ...
        if (y >= 1 && y + height + 1 < 256) {
            ...
            for (int yy = y; yy <= y + 1 + height; ++yy) {
                ...
                for (int xx = x - rad; xx <= x + rad && canGenerate; ++xx)
                    for (int zz = z - rad; zz <= z + rad && canGenerate; ++zz) if (yy >= 0 && yy < 256) {
                        if (!world.getBlock(xx, yy, zz)
                            .isLeaves(world, xx, yy, zz)) canGenerate = false;
            ...
            for (int yy = y + height - (mushroomType == 1 ? 3 : 0); yy <= y + height; ++yy) {
                ...
                for (int xx = x - capRad; xx <= x + capRad; ++xx) for (int zz = z - capRad; zz <= z + capRad; ++zz) {
                    int meta = 5;
                    ...
                    if ((meta != 0 || y >= y + height - 1) && world.getBlock(xx, yy, zz)
                        .canBeReplacedByLeaves(world, xx, yy, zz))
                        world.setBlock(
                            xx,
                            yy,
                            zz,
                            mushroomType == 0 ? Blocks.brown_mushroom_block : Blocks.red_mushroom_block,
                            meta,
                            3);
                }
            }

            for (int yy = 0; yy < height; ++yy) if (world.getBlock(x, y + yy, z)
                .canBeReplacedByLeaves(world, x, y + yy, z))
                world.setBlock(
                    x,
                    y + yy,
                    z,
