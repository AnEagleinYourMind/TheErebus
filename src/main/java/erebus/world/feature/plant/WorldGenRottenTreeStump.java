            ...
            new LootItemStack(ModItems.jadeBoots).setWeight(1), new LootItemStack(Items.golden_boots).setWeight(1) })
                .setPostProcessor(new IPostProcess() {
                    ...
                    public ItemStack postProcessItem(ItemStack is, Random rand) {
                        if (rand.nextBoolean()
                            && (is.getItem() == Items.enchanted_book || is.getItem() instanceof ItemTool
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int x1 = x - baseRadius; x1 <= x + baseRadius; x1++)
            for (int z1 = z - baseRadius; z1 <= z + baseRadius; z1++)
                for (int y1 = y + 1; y1 < y + height; y1++) if (!world.isAirBlock(x1, y1, z1)) return false;
        ...
        for (int yy = y; height + y >= yy; yy++)
            for (int i = radius * -1; i <= radius; ++i) for (int j = radius * -1; j <= radius; ++j) {
                ...
                if (Math.round(Math.sqrt(dSq)) == radius) {
                    ...
                    if (yy >= y + 5 && rand.nextInt(20) == 0) world.setBlock(x + i, yy, z + j, Blocks.air);
                    if (yy == y + height && rand.nextInt(2) == 0) world.setBlock(x + i, yy, z + j, Blocks.air);
                } else world.setBlock(x + i, yy, z + j, Blocks.air);
                // floor at 10 high
                if (Math.round(Math.sqrt(dSq)) <= radius - 1 && baseRadius >= maxRadius && height >= maxHeight)
                    if (yy == y + 10) world.setBlock(x + i, yy, z + j, log);
            }

        // Randomised root Base
        for (int i = baseRadius * -1; i <= baseRadius; ++i) for (int j = baseRadius * -1; j <= baseRadius; ++j) {
            double dSq = i * i + j * j;
            ...
            // Branches sort of
            if (Math.round(Math.sqrt(dSq)) == baseRadius) for (int yy = y; height + y >= yy; yy++)
        ...
        if (baseRadius >= maxRadius) {
            ...
            if (chest1 != null) LootUtil.generateLoot(chest1, rand, chestLoot, 3, 10);
            if (height >= maxHeight) {
                // Upper spawner
