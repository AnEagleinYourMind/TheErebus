...
public class ChunkProviderErebus implements IChunkProvider {
    ...
    public void generateTerrain(int x, int z, Block[] blocks) {
        ...
        for (int k = 0; k < byte0; k++) for (int l = 0; l < byte0; l++) for (int i1 = 0; i1 < 16; i1++) {
            double d = 0.125D;
            ...
            for (int j1 = 0; j1 < 8; j1++) {
                ...
                for (int k1 = 0; k1 < 4; k1++) {
                    ...
                    for (int i2 = 0; i2 < 4; i2++) {
                        ...
                        if (d15 > 0.0D) blocks[l1] = ModBlocks.umberstone;
    ...
    private double[] initializeNoiseField(double[] noise, int x, int y, int z, int sizeX, int sizeY, int sizeZ) {
        ...
        noiseData1 = noiseGen3
            .generateNoiseOctaves(noiseData1, x, y, z, sizeX, sizeY, sizeZ, d * 0.0125D, d1 / 60D, d * 0.0125D);
        ...
        for (int xx = 0; xx < sizeX; xx++) for (int zz = 0; zz < sizeZ; zz++) {
    ...
    public void replaceBlocksForBiome(int x, int z, Block[] blocks, byte[] metadata, BiomeGenBase[] biomes) {
        ...
        stoneNoise = noiseGen4
            .generateNoiseOctaves(stoneNoise, x * 16, z * 16, 0, 16, 16, 1, 0.0625D, 0.0625D, 0.0625D);
        double d0 = 0.03125D;
        double[] additionalNoise1 = new double[256];
        double[] additionalNoise2 = new double[256];
        additionalNoise1 = perlinAdditional1
            .func_151599_a(additionalNoise1, x * 16, z * 16, 16, 16, d0 * 2.0D, d0 * 2.0D, 1.0D);
        additionalNoise2 = perlinAdditional2
            .func_151599_a(additionalNoise2, x * 16, z * 16, 16, 16, d0 * 2.0D, d0 * 2.0D, 1.0D);

        for (int xInChunk = 0; xInChunk < 16; ++xInChunk) for (int zInChunk = 0; zInChunk < 16; ++zInChunk) {
                ...
                if (additionalNoise1[horIndex] > 0) {
                    ...
                    if (h > swampWaterHeight) {
                        for (h += 0; h > 23.08D - additionalNoise1[horIndex]; h--)
                            blocks[preHeightIndex + h] = h == swampWaterHeight
                                ? rand.nextInt(32) == 0 ? Blocks.waterlily : Blocks.air
                                : Blocks.air;
                        blocks[preHeightIndex + h] = additionalNoise1[horIndex] < 0.08D
                            ? ConfigHandler.INSTANCE.generateVents
                                ? rand.nextInt(12) == 0 ? ModBlocks.swampVent : ModBlocks.umberstone
                                : ModBlocks.umberstone
                            : additionalNoise1[horIndex] < 0.5D ? Blocks.sand
                                : additionalNoise1[horIndex] < 1 ? ModBlocks.quickSand
                                    : additionalNoise2[horIndex] > 1 ? ModBlocks.mud : Blocks.dirt;
                    }
                } else if (additionalNoise1[horIndex] > -0.15D) {
                    int h = getLowestAirBlock(blocks, preHeightIndex, 25, 35);
                    if (h > swampWaterHeight) {
                        for (h += 0; h >= (22 + h) / 2; h--) blocks[preHeightIndex + h] = Blocks.air;
                        h++;
                        if (h >= swampWaterHeight && rand.nextInt(8) == 0
                            && blocks[preHeightIndex + h] == Blocks.air
                            && blocks[preHeightIndex + h + 1] == Blocks.air) {
            ...
            if ((biome == ModBiomes.volcanicDesert || biome == ModBiomes.desertSubCharredForest)
            ...
            for (int yInChunk = 127; yInChunk >= 0; --yInChunk) {
                ...
                if (yInChunk <= 5 && yInChunk <= 0 + rand.nextInt(5)
                    || yInChunk >= 122 && yInChunk >= 127 - rand.nextInt(5)) blocks[index] = Blocks.bedrock;
                else {
                    Block block = blocks[index];

                    if (biome == ModBiomes.submergedSwamp && yInChunk < swampWaterHeight && block == Blocks.air)
                        blocks[index] = rand.nextInt(256) == 0 && blocks[index - 1].isOpaqueCube()
                            && yInChunk < swampWaterHeight - 1 ? ModBlocks.mireCoral : Blocks.water;

                    if (block.getMaterial() == Material.air) var13 = -1;
                        ...
                        if (var13 == -1) {
                            ...
                            if (yInChunk < var5 && topBlock.getMaterial() == Material.air)
                                if (temperature < 0.15F) topBlock = Blocks.ice;
                            else topBlock = Blocks.water;
    ...
    public void populate(IChunkProvider chunkProvider, int x, int z) {
        ...
        if (biomeBase instanceof BiomeBaseErebus) {
            ...
            rand.setSeed(
                x * (rand.nextLong() / 2L * 2L + 1L) + z * (rand.nextLong() / 2L * 2L + 1L) ^ worldObj.getSeed());
        ...
        for (int attempt = 0; attempt < 14; ++attempt) new WorldGenSpiderDungeons().generate(
            worldObj,
            rand,
            blockCoordX + rand.nextInt(16) + 8,
            rand.nextInt(128),
            blockCoordZ + rand.nextInt(16) + 8);
