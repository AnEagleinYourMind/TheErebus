...
public class Erebus {
    ...
    public void preInit(FMLPreInitializationEvent event) {
        ...
        if (event.getSide() == Side.CLIENT) {
            ...
            if (ConfigHandler.INSTANCE.playCustomSongs) FMLCommonHandler.instance()
                .bus()
                .register(new ErebusMusicHandler());
        ...
        DimensionManager
            .registerProviderType(ConfigHandler.INSTANCE.erebusDimensionID, WorldProviderErebus.class, true);
        DimensionManager
            .registerDimension(ConfigHandler.INSTANCE.erebusDimensionID, ConfigHandler.INSTANCE.erebusDimensionID);
    ...
    public void init(FMLInitializationEvent event) {
        ...
        for (IRecipe recipe : (List<IRecipe>) CraftingManager.getInstance()
            .getRecipeList()) if (recipe != null) {
                ...
                if (stack != null && stack.getItem() == Items.wooden_door) doorRecipes.add(recipe);
            }
        for (IRecipe recipe : doorRecipes) CraftingManager.getInstance()
            .getRecipeList()
            .remove(recipe);
        ...
        FMLCommonHandler.instance()
            .bus()
            .register(SpawnerErebus.INSTANCE);
        ...
        FMLCommonHandler.instance()
            .bus()
            .register(new AntiVenomDurationHandler());

        if (ConfigHandler.INSTANCE.allowRespawning) {
            FMLCommonHandler.instance()
                .bus()
                .register(new ErebusPlayerLoggedInEvent());
        ...
        if (ConfigHandler.INSTANCE.randomNames) MinecraftForge.EVENT_BUS.register(RandomMobNames.instance);

        if (!ConfigHandler.INSTANCE.disableThaumcraft) ModIntegrationHandler.addMod(ThaumcraftIntegration.class);

        if (!ConfigHandler.INSTANCE.disableFMP) ModIntegrationHandler.addMod(FMPIntegration.class);
