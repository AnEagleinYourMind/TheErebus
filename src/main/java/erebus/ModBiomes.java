...
public class ModBiomes {
    ...
    public static void init() {
        for (int id : new int[] { undergroundJungleID, volcanicDesertID, subterraneanSavannahID, elysianFieldsID,
            ...
            if (biome != null) throw new IllegalArgumentException(
                "Erebus can not use biome ID " + id
                    + " because it's being used by "
                    + biome
    ...
    public static void postInit() {
        for (int id : new int[] { undergroundJungleID, volcanicDesertID, subterraneanSavannahID, elysianFieldsID,
            ulteriorOutbackID, fungalForestID, submergedSwampID, fieldsSubForestID }) {
            BiomeGenBase biome = BiomeGenBase.getBiomeGenArray()[id];
            if (!(biome instanceof BiomeBaseErebus)) throw new IllegalArgumentException(
                "Erebus biome with id " + id
                    + " was replaced by "
                    + biome
                    + ". This is likely an ID conflict or a bug in another mod!");
        }
    }
}
