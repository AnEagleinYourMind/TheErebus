...
public class SmoothieMakerNEIHandler extends TemplateRecipeHandler {
    ...
    public final void drawForeground(int recipe) {
        ...
        for (int i = 0; i < fluids.length; i++) draw(GuiSmoothieMaker.tankPositions[i], fluids[i], 16000);
    }
    ...
    void draw(Rectangle rectangle, FluidStack fluid, int capacity) {
        ...
        for (int i = 0; i < rectangle.width; i += 16) for (int j = 0; j < amount; j += 16) {
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId.equals(getRecipeId())) for (SmoothieMakerRecipe recipe : SmoothieMakerRecipe.getRecipeList())
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        for (SmoothieMakerRecipe recipe : SmoothieMakerRecipe.getRecipeList())
            if (recipe.isPartOfInput(ingredient) || ingredient.getItem() == ModItems.materials
                && ingredient.getItemDamage() == ItemMaterials.DATA.SMOOTHIE_GLASS.ordinal())
                arecipes.add(new CachedSmoothieMakerRecipe(recipe));
    }
    ...
    public final List<String> handleTooltip(GuiRecipe guiRecipe, List<String> currenttip, int recipe) {
        ...
        if (GuiContainerManager.shouldShowTooltip(guiRecipe)) {
            ...
            Point relMouse = new Point(
                mouse.x - (guiRecipe.width - 176) / 2 - offset.x + 5,
                mouse.y - (guiRecipe.height - 166) / 2 - offset.y);

            FluidStack[] fluids = crecipe.getFluids();
            for (int i = 0; i < fluids.length; i++) if (GuiSmoothieMaker.tankPositions[i].contains(relMouse)) {
