...
public class OfferingAltarNEIHandler extends TemplateRecipeHandler {
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId.equals(getRecipeId())) for (OfferingAltarRecipe recipe : OfferingAltarRecipe.getRecipeList())
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        for (OfferingAltarRecipe recipe : OfferingAltarRecipe.getRecipeList())
            if (recipe.isPartOfInput(ingredient)) arecipes.add(new CachedOfferingAltarRecipe(recipe));
    }
    ...
    private class CachedOfferingAltarRecipe extends CachedRecipe {
        ...
        private CachedOfferingAltarRecipe(OfferingAltarRecipe recipe) {
            ...
            if (input.size() >= 1 && input.get(0) != null) inputs.add(new PositionedStack(input.get(0), x + 28, y));
