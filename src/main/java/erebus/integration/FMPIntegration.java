...
public class FMPIntegration implements IModIntegration {
    ...
    public void onInit() {
        ...
        for (EnumWood wood : EnumWood.values()) {
            if (wood.hasLog()) registerMaterial(wood.getLog());
            if (wood.hasLeaves()) registerMaterial(wood.getLeaves());
            if (wood.hasPlanks()) registerMaterial(ModBlocks.planks, wood.ordinal(), wood.ordinal() + 1);
        }
    }
