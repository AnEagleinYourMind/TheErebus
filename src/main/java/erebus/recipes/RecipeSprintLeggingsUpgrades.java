...
public class RecipeSprintLeggingsUpgrades implements IRecipe {
    ...
    public boolean matches(InventoryCrafting craftMatrix, World world) {
        ...
        for (int a = 0; a < size; a++) {
            ...
            if (is.getItem() == ModItems.materials && is.getItemDamage() == DATA.SUPERNATURAL_VELOCITY.ordinal())
                hasVelocity = true;
            else if (is.getItem() == ModItems.sprintLeggings) leggings = is;
        }

        if (cnt == 2 && hasVelocity && leggings != null) return leggings.stackTagCompound == null
            || leggings.stackTagCompound.getByte("upgradeTier") < ItemLeggingsSprint.maxTier;
        ...
        for (int a = 0; a < size; a++) {
            ...
            if (a == 4) {
                if (!(is.getItem() == ModItems.sprintLeggings && (is.stackTagCompound == null
                    || is.stackTagCompound.getByte("upgradeTier") < ItemLeggingsSprint.maxTier))) return false;
    ...
    public ItemStack getCraftingResult(InventoryCrafting craftMatrix) {
        ...
        for (int a = 0; a < craftMatrix.getSizeInventory(); a++) {
            ...
            if (is != null && is.getItem() == ModItems.sprintLeggings) break;
        }
