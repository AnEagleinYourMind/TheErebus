...
public class EnchantSensitiveRecipe extends ShapedOreRecipe {
    ...
    public boolean matches(InventoryCrafting grid, World world) {
        for (int i = 0; i <= 3 - width; i++) for (int j = 0; j <= 3 - height; j++) {
            if (checkMatch(grid, i, j, true)) return true;
            if (checkMatch(grid, i, j, false)) return true;
        }
        return false;
    }
    ...
    private boolean checkMatch(InventoryCrafting inv, int startX, int startY, boolean mirror) {
        for (int x = 0; x < 3; x++) for (int y = 0; y < 3; y++) {
            ...
            if (subX >= 0 && subY >= 0 && subX < width && subY < height)
                if (mirror) target = getInput()[width - subX - 1 + subY * width];
            else target = getInput()[subX + subY * width];

            ItemStack slot = inv.getStackInRowAndColumn(x, y);

            if (target instanceof ItemStack) {
                if (!OreDictionary.itemMatches((ItemStack) target, slot, false)
                    || !checkEnchants((ItemStack) target, slot)) return false;
    ...
    private boolean checkEnchants(ItemStack stack1, ItemStack stack2) {
        if (stack1.hasTagCompound() && stack2.hasTagCompound()) return Utils.getEnchantments(stack1)
            .equals(Utils.getEnchantments(stack2));
