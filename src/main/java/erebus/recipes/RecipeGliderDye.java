...
public class RecipeGliderDye extends RecipesArmorDyes {
    ...
    public boolean matches(InventoryCrafting inventory, World world) {
        ...
        for (int i = 0; i < inventory.getSizeInventory(); i++) {
            ...
            if (stack2 != null) if (stack2.getItem() == ModItems.armorGliderPowered) {
                if (stack != null) return false;
                stack = stack2;
            } else {
                if (!isDye(stack2)) return false;
                list.add(stack2);
            }
        }
    ...
    private boolean isDye(ItemStack stack) {
        for (String ore : Utils.getOreNames(stack))
            for (EnumColour colour : EnumColour.values()) if (ore.equals(colour.getOreName())) return true;
    ...
    private int getDyeIndex(ItemStack stack) {
        for (String ore : Utils.getOreNames(stack))
            for (EnumColour colour : EnumColour.values()) if (ore.equals(colour.getOreName())) return colour.ordinal();
    ...
    public ItemStack getCraftingResult(InventoryCrafting inventory) {
        ...
        for (k = 0; k < inventory.getSizeInventory(); k++) {
            ...
            if (craftStack != null) if (craftStack.getItem() == ModItems.armorGliderPowered) {
                ...
                if (!isDye(craftStack)) return null;
