...
public class ComposterRegistry {
    ...
    private static List<Material> compostableMaterials = Arrays.asList(
        Material.cactus,
        Material.cake,
        Material.coral,
        Material.gourd,
    ...
    public static ItemStack isCompostable(ItemStack stack) {
        ...
        for (ItemStack s : blacklist) if (Utils.areStacksTheSame(s, stack, false)) return null;
        ...
        for (ItemStack reg : registry)
            if (Utils.areStacksTheSame(stack, reg, false)) return new ItemStack(ModItems.compost);
