...
public class SmoothieMakerRecipe {
    ...
    public static ItemStack getOutput(ItemStack container, IFluidTank tank0, IFluidTank tank1, IFluidTank tank2,
        IFluidTank tank3, ItemStack... input) {
        SmoothieMakerRecipe recipe = getRecipe(container, tank0, tank1, tank2, tank3, input);
        return recipe != null ? recipe.getOutput() : null;
    }

    public static SmoothieMakerRecipe getRecipe(ItemStack container, IFluidTank tank0, IFluidTank tank1,
        IFluidTank tank2, IFluidTank tank3, ItemStack... input) {
        for (SmoothieMakerRecipe recipe : recipes)
            if (recipe.matches(container, tank0, tank1, tank2, tank3, input)) return recipe;
    ...
    private SmoothieMakerRecipe(ItemStack output, ItemStack container, FluidStack[] fluids, Object... input) {
        ...
        if (input.length > 4) throw new IllegalArgumentException("Input must be 1 to 4.");
        if (fluids.length > 4) throw new IllegalArgumentException("Fluids must be 1 to 4.");

        for (int c = 0; c < input.length; c++)
            if (input[c] instanceof ItemStack) this.input[c] = ItemStack.copyItemStack((ItemStack) input[c]);
            else if (input[c] instanceof String) this.input[c] = OreDictionary.getOres((String) input[c]);
            else throw new IllegalArgumentException("Input must be an ItemStack or an OreDictionary name");
    }
    ...
    public boolean matches(ItemStack container, IFluidTank tank0, IFluidTank tank1, IFluidTank tank2, IFluidTank tank3,
        ItemStack... stacks) {
        if (container != null && !areStacksTheSame(container, this.container)
            && container.stackSize == this.container.stackSize) return false;

        label: for (Object input : this.input) {
            for (int i = 0; i < stacks.length; i++) if (stacks[i] != null) if (areStacksTheSame(input, stacks[i])) {
                stacks[i] = null;
                continue label;
            }
        ...
        label: for (FluidStack fluid : fluids) {
            for (IFluidTank tank : new IFluidTank[] { tank0, tank1, tank2, tank3 })
                if (tank.getFluidAmount() >= fluid.amount && tank.getFluid()
                    .isFluidEqual(fluid)) continue label;
            return false;
        }
    ...
    public boolean isPartOfInput(ItemStack ingredient) {
        for (Object i : input) if (areStacksTheSame(i, ingredient)) return true;
    ...
    public FluidStack[] getFluids() {
        ...
        for (int i = 0; i < fluids.length; i++) fluids[i] = this.fluids[i].copy();
        return fluids;
    }

    @SuppressWarnings("unchecked")
    public boolean areStacksTheSame(Object obj, ItemStack target) {
        if (obj instanceof ItemStack) return Utils.areStacksTheSame((ItemStack) obj, target, false);
        else if (obj instanceof List) {
            List<ItemStack> list = (List<ItemStack>) obj;

            for (ItemStack stack : list) if (Utils.areStacksTheSame(stack, target, false)) return true;
        }
