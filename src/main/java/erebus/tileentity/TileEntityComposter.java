...
public class TileEntityComposter extends TileEntityBasicInventory {
    ...
    public void updateEntity() {
        ...
        if (composterBurnTime > 0) composterBurnTime--;
        ...
        if (!worldObj.isRemote) {
                ...
                if (composterBurnTime == 0 && canSmelt()) {
                    ...
                    if (composterBurnTime > 0) {
                        ...
                        if (inventory[1] != null) {
                            ...
                            if (inventory[1].stackSize == 0) inventory[1] = inventory[1].getItem()
            ...
            if (flag != composterBurnTime > 0) {
                ...
                if (blockActive && !tileActive) worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, 0, 3);
                if (!blockActive && tileActive) worldObj.setBlockMetadataWithNotify(xCoord, yCoord, zCoord, 1, 3);
            }
        }
    ...
    public void smeltItem() {
        if (canSmelt()) {
            ...
            if (inventory[2] == null) inventory[2] = itemstack.copy();
            else if (inventory[2].getItem() == itemstack.getItem()) inventory[2].stackSize += itemstack.stackSize; // Forge
                                                                                                                   // BugFix:
            // Results may
            // have multiple
            // items
    ...
    public static int getItemBurnTime(ItemStack is) {
        if (is == null) return 0;
            ...
            if (item instanceof ItemBlock && Block.getBlockFromItem(item) != Blocks.air) {
                ...
                if (block == ModBlocks.wallPlants && is.getItemDamage() == 1) return 800;

                if (block == ModBlocks.wallPlantsCultivated && is.getItemDamage() == 1) return 400;
            }
        }
