...
public class TileEntitySmoothieMaker extends TileEntityBasicInventory implements IFluidHandler {
    ...
    public TileEntitySmoothieMaker() {
        ...
        for (int i = 0; i < tanks.length; i++) tanks[i] = new FluidTank(FluidContainerRegistry.BUCKET_VOLUME * 16);
    }
    ...
    private void writeTanksToNBT(NBTTagCompound nbt) {
        for (int i = 0; i < tanks.length; i++) nbt.setTag("tank_" + i, tanks[i].writeToNBT(new NBTTagCompound()));
    }

    private void readTanksFromNBT(NBTTagCompound nbt) {
        for (int i = 0; i < tanks.length; i++) tanks[i].readFromNBT(nbt.getCompoundTag("tank_" + i));
    }
    ...
    public void updateEntity() {
        ...
        for (int i = 0; i < 4; i++) inputs[i] = inventory[i];
        SmoothieMakerRecipe recipe = SmoothieMakerRecipe
            .getRecipe(getStackInSlot(4), tanks[0], tanks[1], tanks[2], tanks[3], inputs);
        if (recipe != null) {
            progress++;

            if (progress >= MAX_TIME) {
                for (int i = 0; i < 5; i++)
                    if (inventory[i] != null) if (--inventory[i].stackSize <= 0) inventory[i] = null;
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        ...
        for (FluidTank tank : tanks) if (resource.isFluidEqual(tank.getFluid())) {
        ...
        if (!foundTankWithSameFluid && amountFilled < resource.amount)
            for (FluidTank tank : tanks) if (tank.getFluid() == null) {
                amountFilled += tank.fill(resource, doFill);
                break;
            }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        for (SmoothieMakerRecipe recipe : SmoothieMakerRecipe.getRecipeList())
            for (FluidStack recipeFluid : recipe.getFluids()) if (recipeFluid.getFluid() == fluid) return true;
        return false;
    }
