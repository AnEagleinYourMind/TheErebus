...
public class TileEntityErebusAltarLightning extends TileEntityErebusAltar {
    ...
    public void updateEntity() {
        ...
        if (active) {
            ...
            if (animationTicks <= 24) animationTicks++;
            if (animationTicks == 25) if (fuzz < 20) {
                fuzz++;
                if (fuzz >= 20) fuzz = 0;
            }
        }
        if (!active) {
            if (animationTicks == 25)
                worldObj.playSoundEffect(xCoord, yCoord, zCoord, "erebus:altarchangestate", 1.0F, 1.3F);
            if (animationTicks >= 1) animationTicks--;
            if (animationTicks == 1) worldObj.setBlock(xCoord, yCoord, zCoord, ModBlocks.altarBase);
        }
        if (animationTicks >= 1 && animationTicks <= 24) flameOn(worldObj, xCoord, yCoord, zCoord);
        if (spawnTicks == 0) setActive(false);
    }
        ...
        List<EntityLivingBase> list = worldObj.getEntitiesWithinAABB(
            ...
            AxisAlignedBB
                .getBoundingBox(
                    ...
                    zCoord + 0.5D)
                .expand(6D, 2D, 6D));
        if (active) for (int i = 0; i < list.size(); i++) {
            Entity entity = list.get(i);
            if (entity != null)
                if (entity instanceof EntityLivingBase) if (((EntityLivingBase) entity).getCreatureAttribute()
                    .equals(EnumCreatureAttribute.ARTHROPOD) && !(entity instanceof EntityMobBlock)
                    && !(entity instanceof EntityUmberGolem)) {
