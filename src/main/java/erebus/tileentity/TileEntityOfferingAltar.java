...
public class TileEntityOfferingAltar extends TileEntityBasicInventory {
    ...
    public void popStack() {
        if (!worldObj.isRemote) for (int i = getSizeInventory() - 1; i >= 0; i--) if (inventory[i] != null) {
    ...
    public void addStack(ItemStack stack) {
        ...
        if (inventory[getSizeInventory() - 1] == null)
            for (int i = 0; i < getSizeInventory() - 1; i++) if (inventory[i] == null) {
                addStack(i, stack);
                return;
            }
    }
    ...
    public void updateEntity() {
        ...
        if (output == null) time = 0;
            ...
            if (time == 90 || time == 270 || time == 450) {
                ...
                if (time >= MAX_TIME) worldObj.playAuxSFX(2004, xCoord, yCoord + 1, zCoord, 0);
            }
            if (time >= MAX_TIME) {
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity packet) {
        if (packet.func_148853_f() == 0) readFromNBT(packet.func_148857_g());
    }
