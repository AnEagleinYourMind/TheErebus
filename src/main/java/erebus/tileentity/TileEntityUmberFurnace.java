...
public class TileEntityUmberFurnace extends TileEntityBasicInventory implements IFluidHandler {
    ...
    public ItemStack fillTankWithBucket(ItemStack bucket) {
        if (tank.getFluidAmount() <= tank.getCapacity() - FluidContainerRegistry.BUCKET_VOLUME)
            if (FluidContainerRegistry.isFilledContainer(bucket) && FluidContainerRegistry.getFluidForFilledItem(bucket)
                .getFluid() == FluidRegistry.LAVA) if (bucket.stackSize == 1) {
                    tank.fill(new FluidStack(FluidRegistry.LAVA, FluidContainerRegistry.BUCKET_VOLUME), true);
                    for (FluidContainerData data : FluidContainerRegistry.getRegisteredFluidContainerData())
                        if (data.filledContainer.getItem() == bucket.getItem()
                            && data.filledContainer.getItemDamage() == bucket.getItemDamage())
    ...
    public void updateEntity() {
        ...
        cookTime = COOK_TIME_BASE
            - (int) (COOK_TIME_BASE * 0.8F * ((float) tank.getFluidAmount() / (float) tank.getCapacity()));
            ...
            if (furnaceBurnTime == 0 && canSmelt()) {
                ...
                if (furnaceBurnTime > 0) {
                    ...
                    if (inventory[FUEL_SLOT] != null) {
                        ...
                        if (inventory[FUEL_SLOT].stackSize == 0) inventory[FUEL_SLOT] = inventory[FUEL_SLOT].getItem()
                            .getContainerItem(inventory[FUEL_SLOT]);
                    }
                }
            }
    ...
    private boolean canSmelt() {
        if (inventory[SMELT_SLOT] == null) return false;
        else {
            ItemStack is = FurnaceRecipes.smelting()
                .getSmeltingResult(inventory[SMELT_SLOT]);
            if (is == null) return false;
            if (inventory[RESULT_SLOT] == null) return true;
            if (!inventory[RESULT_SLOT].isItemEqual(is)) return false;
    ...
    private void smeltItem() {
        if (canSmelt()) {
            ...
            if (inventory[RESULT_SLOT] == null) inventory[RESULT_SLOT] = stack.copy();
            else if (inventory[RESULT_SLOT].isItemEqual(stack)) inventory[RESULT_SLOT].stackSize += stack.stackSize;

            tank.drain(FluidContainerRegistry.BUCKET_VOLUME / 10, true);
            inventory[SMELT_SLOT].stackSize--;

            if (inventory[SMELT_SLOT].stackSize <= 0) inventory[SMELT_SLOT] = null;
        }
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        return slot == BUCKET_SLOT ? FluidContainerRegistry.isContainer(is)
            : slot == FUEL_SLOT ? TileEntityFurnace.isItemFuel(is)
                : slot == SMELT_SLOT ? FurnaceRecipes.smelting()
                    .getSmeltingResult(is) != null : false;
    }
    ...
    public int getScaledFluidAmount(float scale) {
        return tank.getFluid() != null ? (int) ((float) tank.getFluid().amount / (float) tank.getCapacity() * scale)
            : 0;
    }
    ...
    public void getGUIData(int id, int value) {
        switch (id) {
            case 1:
                if (tank.getFluid() == null) tank.setFluid(new FluidStack(FluidRegistry.LAVA, value));
                else tank.getFluid().amount = value;
                break;
            case 2:
    ...
    public float getBurnTimeRemainingScaled(int scale) {
        if (currentItemBurnTime == 0) currentItemBurnTime = cookTime;
