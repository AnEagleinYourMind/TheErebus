...
public class TileEntityJarOHoney extends TileEntityGlowingJar {
    ...
    public FluidStack getHoney() {
        if (tank.getFluid() == null) tank.setFluid(new FluidStack(ModFluids.honey, 0));
    ...
    public void setOwner(String name) {
        owner = name;
        if (!worldObj.isRemote) PacketPipeline.sendToAll(getPacket());
    }
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity packet) {
        ...
        if (packet.func_148853_f() == 0) readFromNBT(nbt);
    }
