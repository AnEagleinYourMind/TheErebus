...
public class TileEntityExtenderThingy extends TileEntityBasicInventory {
    ...
    public void updateEntity() {
        ...
        if (dir == null) dir = getDirectionFromMetadata(worldObj.getBlockMetadata(xCoord, yCoord, zCoord));

        Block blockID;
        Block extension = getExtension(dir);
        int index = getIndex(extension);

        if (extending) blockID = extension;
        else {
            blockID = null;
            index--;
        }
        ...
        if (block == null || block.isReplaceable(worldObj, x, y, z) || !extending)
            if (decreaseInventory(blockID)) if (addToInventory(x, y, z)) if (extending) {
                worldObj.setBlock(x, y, z, blockID, getMetaFromDirection(dir), 3);
                worldObj.playSoundEffect(
                    x + 0.5F,
                    y + 0.5F,
                    z + 0.5F,
                    extension.stepSound.getBreakSound(),
                    (extension.stepSound.getVolume() + 1.0F) / 2.0F,
                    extension.stepSound.getPitch() * 0.8F);
            } else {
                ...
                worldObj.playAuxSFXAtEntity(
                    ...
                    y,
                    z,
                    Block.getIdFromBlock(extension) + (worldObj.getBlockMetadata(x, y, z) << 12));
            }
    }
    ...
    private int getIndex(Block extension) {
        ...
        Block block = worldObj.getBlock(x, y, z);
        if (block == null || block.isReplaceable(worldObj, x, y, z) || !extending) return index;
    ...
    private boolean addToInventory(int x, int y, int z) {
        ...
        if (worldObj.isAirBlock(x, y, z) || block.isReplaceable(worldObj, x, y, z)) return true;
        for (int i = 0; i < inventory.length; i++) if (inventory[i] == null) {
            inventory[i] = new ItemStack(block, 1, 0);
            return true;
        } else if (inventory[i].getItem() == Item.getItemFromBlock(block)
    ...
    public boolean isItemValidForSlot(int slot, ItemStack stack) {
        return stack != null && (stack.getItem() == Item.getItemFromBlock(ModBlocks.bambooPole)
            || stack.getItem() == Item.getItemFromBlock(ModBlocks.bambooBridge));
    }
