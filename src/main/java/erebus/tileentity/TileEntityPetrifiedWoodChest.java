...
public class TileEntityPetrifiedWoodChest extends TileEntityBasicInventory {
    ...
    private void func_90009_a(TileEntityPetrifiedWoodChest chest, int side) {
        if (chest.isInvalid()) adjacentChestChecked = false;
        else if (adjacentChestChecked) switch (side) {
            case 0:
                if (adjacentChestZPosition != chest) adjacentChestChecked = false;
                break;
            case 1:
                if (adjacentChestXNeg != chest) adjacentChestChecked = false;
                break;
            case 2:
                if (adjacentChestZNeg != chest) adjacentChestChecked = false;
                break;
            case 3:
                if (adjacentChestXPos != chest) adjacentChestChecked = false;
        }
    }
    ...
    public void checkForAdjacentChests() {
        if (!adjacentChestChecked) {
            ...
            if (func_94044_a(xCoord, yCoord, zCoord + 1))
                adjacentChestZPosition = (TileEntityPetrifiedWoodChest) worldObj
                    .getTileEntity(xCoord, yCoord, zCoord + 1);
            if (adjacentChestZNeg != null) adjacentChestZNeg.func_90009_a(this, 0);
            if (adjacentChestZPosition != null) adjacentChestZPosition.func_90009_a(this, 2);
            if (adjacentChestXPos != null) adjacentChestXPos.func_90009_a(this, 1);
            if (adjacentChestXNeg != null) adjacentChestXNeg.func_90009_a(this, 3);
        }
    }
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote && numUsingPlayers != 0 && (ticksSinceSync + xCoord + yCoord + zCoord) % 200 == 0) {
            ...
            while (iterator.hasNext()) {
                ...
                if (entityplayer.openContainer instanceof ContainerPetrifiedWoodChest) {
                    IInventory iinventory = ((ContainerPetrifiedWoodChest) entityplayer.openContainer)
                        .getLowerChestInventory();

                    if (iinventory == this || iinventory instanceof InventoryLargeChest
                        && ((InventoryLargeChest) iinventory).isPartOfLargeChest(this)) ++numUsingPlayers;
                }
            }
        }
        ...
        if (numUsingPlayers > 0 && lidAngle == 0.0F && adjacentChestZNeg == null && adjacentChestXNeg == null) {
            ...
            if (adjacentChestZPosition != null) d0 += 0.5D;

            if (adjacentChestXPos != null) d1 += 0.5D;
        ...
        if (numUsingPlayers == 0 && lidAngle > 0.0F || numUsingPlayers > 0 && lidAngle < 1.0F) {
            ...
            if (numUsingPlayers > 0) lidAngle += f;
            else lidAngle -= f;
            ...
            if (lidAngle < f2 && f1 >= f2 && adjacentChestZNeg == null && adjacentChestXNeg == null) {
                ...
                if (adjacentChestZPosition != null) d2 += 0.5D;

                if (adjacentChestXPos != null) d0 += 0.5D;
    ...
    public boolean receiveClientEvent(int eventId, int data) {
        if (eventId == 1) {
            ...
            return true;
        } else return super.receiveClientEvent(eventId, data);
    }
