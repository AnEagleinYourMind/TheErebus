...
public class TileEntitySlidingBlockPuzzle extends TileEntity {
    ...
    public void onPuzzleChange() {
        for (int i = 0; i < pieces.length; i++) if (!originalPieces[i].equals(pieces[i])) return;
        ...
        for (ForgeDirection dir : neighbouringDirs) {
            ...
            if (!neighbourOffset.isValid()) continue;
            TileEntitySlidingBlockPuzzle neighbour = Utils.getTileEntity(
                worldObj,
                xCoord + dir.offsetX,
                yCoord + dir.offsetY,
                zCoord + dir.offsetZ,
                TileEntitySlidingBlockPuzzle.class);
            if (neighbour != null) neighbour.onPuzzleChange();
        }
    }

    public boolean handleClick(float hitX, float hitY, float hitZ) {
        if (hitY > 0.5F) switch (facing) {
            case NORTH:
            case SOUTH:
                if (hitX > 0.5F) return handleClick(pieces[0]);
                else return handleClick(pieces[1]);
            case WEST:
            case EAST:
                if (hitZ > 0.5F) return handleClick(pieces[1]);
                else return handleClick(pieces[0]);
            default:
                break;
        }
        else switch (facing) {
            case NORTH:
            case SOUTH:
                if (hitX > 0.5F) return handleClick(pieces[2]);
                else return handleClick(pieces[3]);
            case WEST:
            case EAST:
                if (hitZ > 0.5F) return handleClick(pieces[3]);
                else return handleClick(pieces[2]);
            default:
                break;
        }
        return false;
    ...
    private SlidingPiece getEmptyPieceIfPresent() {
        for (SlidingPiece p : pieces) if (p.isEmpty()) return p;
        return null;
    }
    ...
    private boolean swapPiecesIfPossible(SlidingPiece piece, SlidingPiece emptyPiece) {
        if (piece.x == emptyPiece.x && piece.y != emptyPiece.y || piece.x != emptyPiece.x && piece.y == emptyPiece.y) {
            if (!worldObj.isRemote) SlidingPiece.swapUVs(piece, emptyPiece);
            return true;
        }
        return false;
    }
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity pkt) {
        if (pkt.func_148853_f() == 0) readData(pkt.func_148857_g());
    }
    ...
    private void readData(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < pieces.length; i++) pieces[i] = SlidingPiece.fromArray(nbt.getIntArray("Piece" + i));
    ...
    private NBTTagCompound writeData(NBTTagCompound nbt) {
        if (facing != null) nbt.setByte("Facing", (byte) facing.ordinal());
        if (offset != null) nbt.setIntArray("Offset", offset.toArray());
        if (puzzle != null) nbt.setInteger("Puzzle", puzzle.ordinal());
        for (int i = 0; i < pieces.length; i++)
            if (pieces[i] != null) nbt.setIntArray("Piece" + i, pieces[i].toArray());
        for (int i = 0; i < originalPieces.length; i++)
            if (originalPieces[i] != null) nbt.setIntArray("OriginalPiece" + i, originalPieces[i].toArray());
        return nbt;
    }
    ...
    static {
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            ...
            if (!offsets.isEmpty()) map.put(dir, offsets);
        }
    }
    ...
    public static void createPuzzleAt(World world, int x, int y, int z, ForgeDirection facing, SlidingPuzzle puzzle) {
        ...
        if (offsets == null) throw new IllegalArgumentException("Can't generate sliding block puzzle facing " + facing);
        if (facing == ForgeDirection.WEST) world.setBlock(x, y, z - 2, ModBlocks.completedPuzzle);
        if (facing == ForgeDirection.EAST) world.setBlock(x, y, z + 2, ModBlocks.completedPuzzle);
        if (facing == ForgeDirection.SOUTH) world.setBlock(x - 2, y, z, ModBlocks.completedPuzzle);
        if (facing == ForgeDirection.NORTH) world.setBlock(x + 2, y, z, ModBlocks.completedPuzzle);
        TileEntityCompletedPuzzle compPuzzle = Utils.getTileEntity(world, x, y + 2, z, TileEntityCompletedPuzzle.class);
        if (compPuzzle != null) compPuzzle.setPuzzle(puzzle);

        for (BlockOffset offset : offsets) tiles.add(setBlock(world, x, y, z, offset, facing, puzzle));
        ...
        for (int i = 0; i < tiles.size(); i++) {
            ...
            if (facing == ForgeDirection.SOUTH || facing == ForgeDirection.EAST) {
                ...
                if (tX == 2 && tY == 2) pieces.add(tile.originalPieces[2] = new SlidingPiece(3, -1, -1));
                else pieces.add(tile.originalPieces[2] = new SlidingPiece(3, xx + 1, yy + 1));
            } else {
                ...
                if (tX == 2 && tY == 2) pieces.add(tile.originalPieces[3] = new SlidingPiece(3, -1, -1));
                else pieces.add(tile.originalPieces[3] = new SlidingPiece(3, xx + 1, yy + 1));
            }
        }
        for (TileEntitySlidingBlockPuzzle tile : tiles) {
            ...
            for (int i = 0; i < tile.pieces.length; i++) {
                ...
                int pieceIndex = i;
                if (facing == ForgeDirection.SOUTH || facing == ForgeDirection.EAST) pieceIndex ^= 1;
                tile.pieces[i] = pieces.get(index)
                    .copyUV(pieceIndex);
                pieces.remove(index);
            }
    ...
    private static TileEntitySlidingBlockPuzzle setBlock(World world, int x, int y, int z, BlockOffset offset,
        ForgeDirection facing, SlidingPuzzle puzzle) {
        x += offset.x;
        y += offset.y;
        z += offset.z;
