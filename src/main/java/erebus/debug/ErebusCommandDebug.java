...
public class ErebusCommandDebug extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] params) {
        ...
        if (params.length == 0) {
        ...
        } else if (params[0].equals("gen") && params.length >= 3) try {
            Class<?> cls = Class.forName("erebus.world.feature." + params[1] + ".WorldGen" + params[2]);

            WorldGenerator gen = (WorldGenerator) cls.getDeclaredConstructor()
                .newInstance();
            ChunkCoordinates coords = sender.getPlayerCoordinates();

            if (gen
                .generate(sender.getEntityWorld(), sender.getEntityWorld().rand, coords.posX, coords.posY, coords.posZ))
                sender.addChatMessage(text("command.erebus.generated"));
            else sender.addChatMessage(text("command.erebus.failed"));
        } catch (Throwable t) {
            t.printStackTrace();
            ...
            /*
             * Use this with hot code replace whenever there's something to debug and you want to print stuff out on
             * command. Whoever comes here and needs this - if you think the current code will be useful another time,
             * add a new command for it. If not, just delete it, no hard feelings.
             */
            String response = "";
            ...
            for (Entity entity : list)
                for (EnumCreatureType type : EnumCreatureType.values()) if (entity.isCreatureType(type, true)) {
            ...
            for (Entry<EnumCreatureType, Set<String>> entry : mobs.entrySet()) {
                ...
                for (String s : entry.getValue()) build.append(s)
                    .append(", ");
